...
namespace mach_o
{
	namespace dylib
	{
		...
		template <> const char *
		File<x86_64>::objCInfoSegmentName()
		{
			return "__DATA";
		}
		template <> const char *
		File<arm>::objCInfoSegmentName()
		{
			return "__DATA";
		}
		template <typename A> const char *
		File<A>::objCInfoSegmentName()
		{
			return "__OBJC";
		}

		template <> const char *
		File<x86_64>::objCInfoSectionName()
		{
			return "__objc_imageinfo";
		}
		template <> const char *
		File<arm>::objCInfoSectionName()
		{
			return "__objc_imageinfo";
		}
		template <typename A> const char *
		File<A>::objCInfoSectionName()
		{
			return "__image_info";
		}
			       ...
			       hoistImplicitPublicDylibs, allowSimToMacOSX, addVers), _fileLength(fileLength), _linkeditStartOffset(0)
		{
			...
			for (uint32_t i = 0; i < cmd_count; ++i)
			{
				...
				switch (cmd->cmd())
				{
					...
					case LC_VERSION_MIN_TVOS:
						lcPlatforms.insert(ld::PlatformVersion(ld::platformForLoadCommand(cmd->cmd(), (mach_header *)header),
						                                       ((macho_version_min_command<P> *)cmd)->version()));
						break;
			...
			cmdLinePlatforms.checkDylibCrosslink(lcPlatforms, path, "dylib", internalSDK, indirectDylib, usingBitcode,
			                                     _isUnzipperedTwin, _dylibInstallPath, fromSDK, platformMismatchesAreWarning);
				...
				for (uint32_t i = 0; i < cmd_count; ++i)
				{
					...
					switch (cmd->cmd())
					{
						...
						case LC_REEXPORT_DYLIB:
							...
							{
								throwf("malformed mach-o: LC_*_DYLIB load command has offset (%u) outside its size (%u)", dylibCmd->name_offset(),
								       cmdLength);
							}
		...
		template <typename A>
		void
		File<A>::buildExportHashTableFromSymbolTable(const macho_dysymtab_command<P> *dynamicInfo,
		...
		template <typename A>
		void
		File<A>::buildExportHashTableFromExportInfo(uint32_t exportsOffset, uint32_t exportsSize,
		        const uint8_t *fileContent)
		{
			if (this->_s_logHashtable)
			{
		...
		template <typename A>
		void
		File<A>::addSymbol(const char *name, bool weakDef, bool tlv, pint_t address)
		{
					...
					if (strncmp(symCond, curOSVers, strlen(curOSVers)) == 0)
					{
						...
						if (symName != nullptr)
						{
							...
							else if (strncmp(symAction, "install_name$", 13) == 0)
							{
								...
								if (strcmp(this->_dylibInstallPath,
		...
		class Parser
		{
			public:
				...
				static bool				validFile(const uint8_t *fileContent, bool executableOrDyliborBundle, bool subTypeMustMatch = false,
				                                  uint32_t subType = 0);
				...
				static ld::dylib::File	*
				parse(const uint8_t *fileContent, uint64_t fileLength, const char *path,
		...
		template <typename A>
		ld::Platform
		Parser<A>::findPlatform(const macho_header<P> *header, uint64_t fileLength, uint32_t *minOsVers)
		{
			const uint32_t cmd_count = header->ncmds();
			if (cmd_count == 0)
			{
		...
		template <>
		bool
		Parser<x86>::validFile(const uint8_t *fileContent, bool executableOrDyliborBundle, bool subTypeMustMatch,
		                       uint32_t subType)
		{
		...
		template <>
		bool
		Parser<x86_64>::validFile(const uint8_t *fileContent, bool executableOrDyliborBundle, bool subTypeMustMatch,
		                          uint32_t subType)
		{
			const auto *header = reinterpret_cast<const macho_header<P>*>(fileContent);
		...
		template <>
		bool
		Parser<arm>::validFile(const uint8_t *fileContent, bool executableOrDyliborBundle, bool subTypeMustMatch,
		                       uint32_t subType)
		{
			const auto *header = reinterpret_cast<const macho_header<P>*>(fileContent);
			if (header->magic() != MH_MAGIC)
		...
		template <>
		bool
		Parser<arm64>::validFile(const uint8_t *fileContent, bool executableOrDyliborBundle, bool subTypeMustMatch,
		                         uint32_t subType)
		{
			const auto *header = reinterpret_cast<const macho_header<P>*>(fileContent);
			if (header->magic() != MH_MAGIC_64)
		...
		template <>
		bool
		Parser<arm64_32>::validFile(const uint8_t *fileContent, bool executableOrDyliborBundle, bool subTypeMustMatch,
		                            uint32_t subType)
		{
			const macho_header<P> *header = (const macho_header<P> *)fileContent;
			if (header->magic() != MH_MAGIC)
		...
		bool
		isDylibFile(const uint8_t *fileContent, uint64_t fileLength, cpu_type_t *result, cpu_subtype_t *subResult,
		            ld::Platform *platform, uint32_t *minOsVers)
		{
		...
		static ld::dylib::File *
		parseAsArchitecture(const uint8_t *fileContent, uint64_t fileLength, const char *path,
		      ...
		      bool bundleLoader, bool indirectDylib, bool fromSDK)
		{
			...
			auto file = parseAsArchitecture(fileContent, fileLength, path, modtime, opts, ordinal, bundleLoader, indirectDylib,
			                                fromSDK, opts.architecture(), opts.subArchitecture());
			...
			if (!file && opts.fallbackArchitecture())
			{
				...
				file = parseAsArchitecture(fileContent, fileLength, path, modtime, opts, ordinal, bundleLoader, indirectDylib, fromSDK,
				                           opts.fallbackArchitecture(), opts.fallbackSubArchitecture());
			}
			return file;
		}
