...
namespace generic
{
	namespace dylib
	{
		...
		ExportAtom::ExportAtom(const File &f, const char *nm, const char *im, uint32_t cv, bool weakDef, bool tlv,
		                       uint64_t address)
		...
		const ld::File *
		ExportAtom::file() const
		{
			return _file;
		}
		void
		ExportAtom::setFile(const ld::File *file)
		{
		...
		std::pair<bool, bool>
		File::hasWeakDefinitionImpl(const char *name) const
		{
		...
		bool
		File::hasDefinitionImpl(const char *name) const
		{
			const auto pos = _atoms.find(name);
			if (pos != this->_atoms.end())
			{
		...
		bool
		File::hasWeakDefinition(const char *name) const
		{
		...
		bool
		File::hasDefinition(const char *name) const
		{
			// If we are supposed to ignore this export, then pretend we don't have it.
			if (_ignoreExports.count(name) != 0)
			{
		...
		bool
		File::containsOrReExports(const char *name, AtomAndWeak &atom) const
		{
			if (_ignoreExports.count(name) != 0)
			{
				return false;
		...
		bool
		File::justInTimeforEachAtom(const char *name, ld::File::AtomHandler &handler) const
		{
			// If we are supposed to ignore this export, then pretend we don't have it.
			if (_ignoreExports.count(name) != 0)
			{
			...
			{
				bucket.atom = new ExportAtom(*this, name, bucket.installname, bucket.compat_version, bucket.weakDef, bucket.tlv,
				                             bucket.address);
				_atoms[name] = bucket;
				_providedAtom = true;
				if (_s_logHashtable)
		...
		void
		File::forEachExportedSymbol(void (^handler)(const char *symbolName, bool weakDef)) const
		{
		...
		File *
		File::createSyntheticDylib(const char *installName, uint32_t version) const
		{
			auto result = new File(this->path(), this->modificationTime(), this->ordinal(), _platforms, false, false, false, false,
			                       true);
		...
		void
		File::addExportedSymbol(const ExportAtom *atom)
		{
		...
		void
		File::addExportedSymbol(const char *name, bool weakDef, bool tlv, uint64_t address)
		{
		...
		void
		File::reservedSymbolSpace(size_t size)
		{
			_atoms.reserve(size);
		}
		...
		void
		File::assertNoReExportCycles(ReExportChain *prev) const
		{
		...
		bool
		File::hasReExportedDependentsThatProvidedExportAtom() const
		{
		...
		void
		File::processIndirectLibraries(ld::dylib::File::DylibHandler *handler, bool addImplicitDylibs)
		{
			// only do this once
		...
		bool
		File::isPublicLocation(const char *path) const
		{
		...
		bool
		File::allSymbolsAreWeakImported() const
		{
