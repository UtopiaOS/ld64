...
namespace lto
{
	...
	class InternalAtom : public ld::Atom
	{
		public:
			...
			uint64_t
			objectAddress() const override
			{
				return 0;
			}
			void
			copyRawContent(uint8_t buffer[]) const override { }
			ld::Fixup::iterator
			...
			{
				return &_undefs[_undefs.size()];
			}

			// for adding references to symbols outside bitcode file
			void
			addReference(const char *nm)
			{
	...
	class File : public ld::relocatable::File
	{
		public:
			...
			bool
			justInTimeforEachAtom(const char *name, ld::File::AtomHandler &) const override
			{
				return false;
			}
			uint32_t
			cpuSubType() const override
			{
				return _cpuSubType;
			}
			...
			const char									*
			debugInfoPath() const override
			{
				return _debugInfoPath;
			}
			time_t
			debugInfoModificationTime() const override
			{
				return _debugInfoModTime;
			}
			const std::vector<ld::relocatable::File::Stab>	*
			stabs()	const override
			{
				return NULL;
			}
			bool
			canScatterAtoms() const override
			{
				return true;
			}
			...
			class InternalAtom									&internalAtom()
			{
					return _internalAtom;
			}
			void
			setDebugInfo(ld::relocatable::File::DebugInfoKind k,
	...
	class Atom : public ld::Atom
	{
		public:
			...
			file() const override
			{
				return (_compiledAtom ? _compiledAtom->file() : &_file);
			}
			const ld::File				*
			...
			const char					*
			translationUnitSource() const override
			{
			...
			size() const override
			{
				return (_compiledAtom ? _compiledAtom->size() : 0);
			}
			uint64_t
			objectAddress() const override
			{
				return (_compiledAtom ? _compiledAtom->objectAddress() : 0);
			}
			void
			copyRawContent(uint8_t buffer[]) const override
			{
				if (_compiledAtom)
				{
					_compiledAtom->copyRawContent(buffer);
				}
			}
			const uint8_t				*
			rawContentPointer() const override
			{
				return (_compiledAtom ? _compiledAtom->rawContentPointer() : NULL);
			}
			unsigned long
			contentHash(const class ld::IndirectBindingTable &ibt) const override
			{
				return (_compiledAtom ? _compiledAtom->contentHash(ibt) : 0);
			}
			bool
			canCoalesceWith(const ld::Atom &rhs, const class ld::IndirectBindingTable &ibt) const override
			{
	...
	class Parser
	{
		...
		private:
			...
			static ld::relocatable::File	*parseMachOFile(const uint8_t *p, size_t len, const std::string &path,
			        const OptimizeOptions &options,
			...
			class AtomSyncer : public ld::File::AtomHandler
			{
				public:
					AtomSyncer(std::vector<const char *> &a, std::vector<const ld::Atom *> &na,
						...
						_options(options), _additionalUndefines(a), _newAtoms(na), _llvmAtoms(la), _deadllvmAtoms(dla), _lastProxiedAtom(NULL),
						_lastProxiedFile(NULL) {}
					void		doAtom(const class ld::Atom &) override;
					void
					doFile(const class ld::File &) override { }
	...
	bool
	Parser::validFile(const uint8_t *fileContent, uint64_t fileLength, cpu_type_t architecture, cpu_subtype_t subarch)
	{
	...
	const char *
	Parser::fileKind(const uint8_t *p, uint64_t fileLength)
	{
	...
	File *
	Parser::parse(const uint8_t *fileContent, uint64_t fileLength, const char *path, time_t modTime,
	              ld::File::Ordinal ordinal,
	...
	ld::relocatable::File *
	Parser::parseMachOFile(const uint8_t *p, size_t len, const std::string &path, const OptimizeOptions &options,
	                       ld::File::Ordinal ordinal)
	{
	...
	File::File(const char *pth, time_t mTime, ld::File::Ordinal ordinal, const uint8_t *content, uint32_t contentLength,
	           cpu_type_t arch)
		  ...
		  _debugInfo(ld::relocatable::File::kDebugInfoNone), _cpuSubType(0)
	{
		...
		if (_module == NULL)
		{
			throwf("could not parse object file %s: '%s', using libLTO version '%s'", pth, ::lto_get_error_message(),
			       ::lto_get_version());
		}
		if (log)
		{
			fprintf(stderr, "bitcode file: %s\n", pth);
		}
#if LTO_API_VERSION >= 18
	...
	void
	File::forEachLtoSymbol(void (^handler)(const char *)) const
	{
#if LTO_API_VERSION >= 11
	...
	bool
	File::mergeIntoGenerator(lto_code_gen_t generator, bool useSetModule)
	{
#if LTO_API_VERSION >= 11
		if (sSupportsLocalContext)
		{
...
#if LTO_API_VERSION >= 18
	void
	File::addToThinGenerator(thinlto_code_gen_t generator, int id)
	{
		assert(!_module && "Expected module to be disposed");
	...
	bool
	File::forEachAtom(ld::File::AtomHandler &handler) const
	{
	...
	void
	Atom::setCompiledAtom(const ld::Atom &atom)
	{
...
#if LTO_API_VERSION >= 7
	void
	Parser::ltoDiagnosticHandler(lto_codegen_diagnostic_severity_t severity, const char *message, void *)
	{
		switch (severity)
		{
	...
	void
	Parser::setPreservedSymbols(const std::vector<const ld::Atom *>	&allAtoms,
	                            ld::Internal						&state,
	                            const OptimizeOptions				&options,
	                            CStringToAtom &deadllvmAtoms,
	                            CStringToAtom &llvmAtoms,
	                            lto_code_gen_t generator)
	{
		...
		for (CStringToAtom::iterator it = llvmAtoms.begin(); it != llvmAtoms.end(); ++it)
		{
			...
			if ((atom->scope() == ld::Atom::scopeGlobal) && options.preserveAllGlobals)
			{
				if (logMustPreserve)
				{
					fprintf(stderr, "lto_codegen_add_must_preserve_symbol(%s) because global symbol\n", name);
				}
				::lto_codegen_add_must_preserve_symbol(generator, name);
			}
			else if (nonLLVMRefs.find(name) != nonLLVMRefs.end())
			{
				if (logMustPreserve)
				{
					fprintf(stderr, "lto_codegen_add_must_preserve_symbol(%s) because referenced by a mach-o atom\n", name);
				}
				::lto_codegen_add_must_preserve_symbol(generator, name);
			}
			else if (options.relocatable && hasNonllvmAtoms)
			{
				...
				if (logMustPreserve)
				{
					fprintf(stderr, "lto_codegen_add_must_preserve_symbol(%s) because -r mode disable LTO dead stripping\n", name);
				}
				::lto_codegen_add_must_preserve_symbol(generator, name);
			}
		}
		// <rdar://problem/16165191> tell code generator to preserve initial undefines
		for (std::vector<const char *>::const_iterator it = options.initialUndefines->begin();
		        it != options.initialUndefines->end(); ++it)
		{
			if (logMustPreserve)
			{
				fprintf(stderr, "lto_codegen_add_must_preserve_symbol(%s) because it is an initial undefine\n", *it);
			}
	...
	static lto_codegen_model
	getCodeModel(const OptimizeOptions &options)
	{
	...
	Parser::codegen(const OptimizeOptions &options,
	                ld::Internal			 &state,
	                lto_code_gen_t		 generator,
	                std::string           &object_path)
	{
		...
		if (useSplitAPI)
		{
			...
			{
				throwf("could not do LTO optimization: '%s', using libLTO version '%s'", ::lto_get_error_message(),
				       ::lto_get_version());
			}
	...
	void
	Parser::loadMachO(ld::relocatable::File				*machoFile,
	                  const OptimizeOptions				&options,
	                  ld::File::AtomHandler				&handler,
	                  std::vector<const ld::Atom *>		&newAtoms,
	                  std::vector<const char *>			&additionalUndefines,
	...
	// Full LTO processing
	bool
	Parser::optimizeLTO(const std::vector<File *>				files,
	                    const std::vector<const ld::Atom *>	&allAtoms,
	                    ld::Internal							&state,
	                    const OptimizeOptions					&options,
	                    ld::File::AtomHandler					&handler,
	                    ...
	                    std::vector<const char *>				&additionalUndefines)
	{
		...
		for (auto *f : files)
		{
			...
			if (logBitcodeFiles && !useSetModule)
			{
				fprintf(stderr, "lto_codegen_add_module(%s)\n", f->path());
			}
			if (logBitcodeFiles && useSetModule)
			{
				fprintf(stderr, "lto_codegen_set_module(%s)\n", f->path());
			}
			if (f->mergeIntoGenerator(generator, useSetModule))
			{
				throwf("lto: could not merge in %s because '%s', using libLTO version '%s'", f->path(), ::lto_get_error_message(),
				       ::lto_get_version());
			}
		...
		if (!_s_llvmOptionsProcessed)
		{
			...
			for (const char *opt : *options.llvmOptions)
			{
				if (logExtraOptions)
				{
					fprintf(stderr, "passing option to llvm: %s\n", opt);
				}
		...
		ld::relocatable::File *machoFile = parseMachOFile(machOFile, machOFileLen, object_path, options,
	...
	thinlto_code_gen_t
	Parser::init_thinlto_codegen(const std::vector<File *>           &files,
	                             const std::vector<const ld::Atom *> &allAtoms,
	                             ld::Internal				       &state,
	                             const OptimizeOptions			   &options,
	                             CStringToAtom                      &deadllvmAtoms,
	                             CStringToAtom                      &llvmAtoms)
	{
		...
		for (CStringToAtom::iterator it = llvmAtoms.begin(); it != llvmAtoms.end(); ++it)
		{
			...
			if ((atom->scope() == ld::Atom::scopeGlobal) && options.preserveAllGlobals)
			{
				if (logMustPreserve)
				{
					fprintf(stderr, "thinlto_codegen_add_must_preserve_symbol(%s) because global symbol\n", name);
				}
				::thinlto_codegen_add_must_preserve_symbol(thingenerator, name, strlen(name));
			}
			else if (nonLLVMRefs.find(name) != nonLLVMRefs.end())
			{
				if (logMustPreserve)
				{
					fprintf(stderr, "thinlto_codegen_add_must_preserve_symbol(%s) because referenced from outside of ThinLTO\n", name);
				}
				::thinlto_codegen_add_must_preserve_symbol(thingenerator, name, strlen(name));
			}
			else if (LLVMRefs.find(name) != LLVMRefs.end())
			{
				if (logMustPreserve)
				{
					fprintf(stderr, "thinlto_codegen_add_cross_referenced_symbol(%s) because referenced from another file\n", name);
				}
			...
			else
			{
				if (logMustPreserve)
				{
					fprintf(stderr, "NOT preserving(%s)\n", name);
				}
			}
			// <rdar://problem/16165191> tell code generator to preserve initial undefines
			for (const char *undefName : *options.initialUndefines)
			{
				if (logMustPreserve)
				{
					fprintf(stderr, "thinlto_codegen_add_cross_referenced_symbol(%s) because it is an initial undefine\n", undefName);
				}
	...
	// Full LTO processing
	bool
	Parser::optimizeThinLTO(const std::vector<File *>              &files,
	                        const std::vector<const ld::Atom *>	&allAtoms,
	                        ld::Internal							&state,
	                        const OptimizeOptions					&options,
	                        ld::File::AtomHandler					&handler,
	                        ...
	                        std::vector<const char *>				&additionalUndefines)
	{
		...
		for (auto *f : files)
		{
			if (logBitcodeFiles)
			{
				fprintf(stderr, "thinlto_codegen_add_module(%s)\n", f->path());
			}
		...
		if (options.bitcodeBundle)
		{
			...
			for (unsigned bufID = 0; bufID < numObjects; ++bufID)
			{
				...
				::thinlto_codegen_add_module(thingenerator, strdup(tempMachoPath.c_str()), (const char *)machOFile.Buffer,
				                             machOFile.Size);
			}
		}
		...
		if (numObjects == 0)
		{
			throwf("could not do ThinLTO codegen (thinlto_codegen_process didn't produce any object): '%s', using libLTO version '%s'",
		...
		for (unsigned bufID = 0; bufID < numObjects; ++bufID)
		{
			...
			// parse generated mach-o file into a MachOReader
			ld::relocatable::File *machoFile = parseMachOFile((const uint8_t *)machOFile.Buffer, machOFile.Size, tmp_path, options,
			                                   ordinal);
		...
		throwf("lto: could not use -thinlto because ld was built against a version of libLTO too old (version '%d', >=18 is required)",
		       LTO_API_VERSION);
#endif
	}

	bool
	Parser::optimize(const std::vector<const ld::Atom *>	&allAtoms,
	                 ld::Internal						&state,
	                 const OptimizeOptions				&options,
	                 ld::File::AtomHandler				&handler,
	                 std::vector<const ld::Atom *>		&newAtoms,
	...
	void
	Parser::AtomSyncer::doAtom(const ld::Atom &machoAtom)
	{
		...
		if ((pos != _llvmAtoms.end()) && (machoAtom.scope() != ld::Atom::scopeTranslationUnit))
		{
			...
			if (pos->second->scope() == ld::Atom::scopeLinkageUnit)
			{
				if (log)
				{
					fprintf(stderr, "demote %s to hidden after LTO\n", name);
				}
			        ...
			        pos->second->autoHide() && !machoAtom.autoHide())
			{
				if (log)
				{
					fprintf(stderr, "set %s to auto hide after LTO\n", name);
				}
			...
			if (log)
			{
				fprintf(stderr, "AtomSyncer, mach-o atom %p synced to lto atom %p (name=%s)\n", &machoAtom, pos->second,
				        machoAtom.name());
			}
		}
		else
		{
			...
			if (llvmAtomPos != _deadllvmAtoms.end())
			{
				...
				if (atomFile->isThinLTO() && (machoAtom.scope() == ld::Atom::scopeTranslationUnit))
				{
					...
					if (log)
					{
						fprintf(stderr, "AtomSyncer, static mach-o atom %p not synced to dead lto name=%s\n", &machoAtom, name);
					}
					llvmAtom = nullptr;
				}
			}
			if (llvmAtom != nullptr)
			{
				...
				if (_options.linkerDeadStripping)
				{
					...
					if (log)
					{
						fprintf(stderr, "AtomSyncer, mach-o atom %p matches dead lto atom %p but adding back (name=%s)\n", &machoAtom, llvmAtom,
						        machoAtom.name());
					}
				}
				else
				{
					...
					if (log)
					{
						fprintf(stderr, "AtomSyncer, mach-o atom %p matches dead lto atom %p (name=%s)\n", &machoAtom, llvmAtom,
						        machoAtom.name());
					}
					if (llvmAtom->coalescedAway())
					{
						if (log)
						{
							fprintf(stderr, "AtomSyncer: dead coalesced atom %s\n", machoAtom.name());
						}
						...
						for (ld::Fixup::iterator fit = machoAtom.fixupsBegin(), fend = machoAtom.fixupsEnd(); fit != fend; ++fit)
						{
							switch (fit->kind)
							{
								...
								case ld::Fixup::kindNoneGroupSubordinateLSDA:
									...
									if (log)
									{
										fprintf(stderr, "AtomSyncer: mark coalesced-away subordinate atom %s\n", fit->u.target->name());
									}
									break;
								default:
									break;
			...
			else
			{
				...
				if ((_lastProxiedAtom != NULL) && (_lastProxiedAtom->section() == machoAtom.section()))
				{
					...
					if (log)
					{
						fprintf(stderr, "AtomSyncer, mach-o atom %s is static and being assigned to %s\n", machoAtom.name(),
						        _lastProxiedFile->path());
					}
				}
				if (log)
				{
					fprintf(stderr, "AtomSyncer, mach-o atom %p is totally new (name=%s)\n", &machoAtom, machoAtom.name());
				}
			}
		}
		...
		for (ld::Fixup::iterator fit = machoAtom.fixupsBegin(); fit != machoAtom.fixupsEnd(); ++fit)
		{
			switch (fit->binding)
			{
				...
				case ld::Fixup::bindingByNameUnbound:
					...
					if (log)
					{
						fprintf(stderr, "    adding by-name symbol %s\n", fit->u.name);
					}
					break;
				case ld::Fixup::bindingDirectlyBound:
					...
					if (fit->u.target->scope() != ld::Atom::scopeTranslationUnit)
					{
						...
						if (post != _llvmAtoms.end())
						{
							...
							if (log)
							{
								fprintf(stderr, "    updating direct reference to %p to be ref to %p: %s\n", fit->u.target, t, targetName);
							}
							fit->u.target = t;
						}
						else
						{
							...
							if ((_deadllvmAtoms.find(targetName) != _deadllvmAtoms.end()) && (fit->kind != ld::Fixup::kindNoneFollowOn)
							        && (fit->u.target->scope() != ld::Atom::scopeTranslationUnit))
							{
	...
	class Mutex
	{
		...
		public:
			Mutex()
			{
				pthread_mutex_lock(&lto_lock);
			}
			~Mutex()
			{
				pthread_mutex_unlock(&lto_lock);
			}
	};
	...
	//
	bool
	optimize(const std::vector<const ld::Atom *>	&allAtoms,
	         ld::Internal						&state,
	         const OptimizeOptions				&options,
	         ld::File::AtomHandler				&handler,
	         std::vector<const ld::Atom *>		&newAtoms,
