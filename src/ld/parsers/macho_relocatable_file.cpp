	...
	namespace relocatable
	{
		...
		class File : public ld::relocatable::File
		{
			public:
				...
				virtual bool
				justInTimeforEachAtom(const char *name, ld::File::AtomHandler &) const
				{
					return false;
				}
				virtual const ld::VersionSet						&
				platforms()	const
				{
					return _platforms;
				}
				...
				virtual bool
				objcHasCategoryClassPropertiesField() const
				{
				...
				virtual uint8_t
				cpuSubTypeFlags() const
				{
					return _cpuSubTypeFlags;
				}
				virtual DebugInfoKind
				debugInfo() const
				{
					return _debugInfoKind;
				}
				virtual const std::vector<ld::relocatable::File::Stab>	*
				stabs() const
				{
					return &_stabs;
				}
				virtual bool
				canScatterAtoms() const
				{
					return _canScatterAtoms;
				}
				virtual bool
				hasllvmProfiling() const
				{
					return _hasllvmProfiling;
				}
				virtual const char									*translationUnitSource() const;
				virtual LinkerOptionsList							*
				linkerOptions() const
				{
					return &_linkerOptions;
				}
				virtual const ToolVersionList						&
				toolVersions() const
				{
				...
				virtual uint16_t
				swiftLanguageVersion() const
				{
					return _swiftLanguageVersion;
				}
				virtual ld::Bitcode								*
				getBitcode() const
				{
				...
				virtual const std::vector<AstTimeAndPath>			*
				astFiles() const
				{
					return &_astFiles;
				}
		...
		class Section : public ld::Section
		{
			public:
				...
				virtual
				~Section()					{ }
				class File<A>					&file() const
				{
						return _file;
				}
				const macho_section<P>			*
				machoSection() const
				{
					return _machOSection;
				}
				uint32_t						sectionNum(class Parser<A> &) const;
				virtual ld::Atom::Alignment		alignmentForAddress(pint_t addr);
				virtual ld::Atom::ContentType
				contentType()
				{
					return ld::Atom::typeUnclassified;
				}
				virtual bool
				dontDeadStrip()
				{
					return (this->_machOSection->flags() & S_ATTR_NO_DEAD_STRIP);
				}
				virtual bool
				dontDeadStripIfReferencesLive()
				{
					return ((this->_machOSection != NULL) && (this->_machOSection->flags() & S_ATTR_LIVE_SUPPORT));
				}
				virtual	Atom<A>				*
				findAtomByAddress(pint_t addr)
				{
					return this->findContentAtomByAddress(addr, this->_beginAtoms, this->_endAtoms);
				}
				virtual bool
				addFollowOnFixups() const
				{
					return ! _file.canScatterAtoms();
				}
				...
				contentHash(const class Atom<A> *atom, const ld::IndirectBindingTable &ind) const
				{
					return 0;
				}
				virtual bool
				canCoalesceWith(const class Atom<A> *atom, const ld::Atom &rhs,
				                const ld::IndirectBindingTable &ind) const
				{
					return false;
				}
				virtual	bool
				ignoreLabel(const char *label) const
				{
					return false;
				}
		...
		class CFISection : public Section<A>
		{
			public:
				...
				virtual uint32_t	appendAtoms(class Parser<A> &parser, uint8_t *buffer, struct Parser<A>::LabelAndCFIBreakIterator &it,
				                                const struct Parser<A>::CFI_CU_InfoArrays &);
				virtual void		makeFixups(class Parser<A> &parser, const struct Parser<A>::CFI_CU_InfoArrays &);
				virtual bool
				addFollowOnFixups() const
				...
				class OAS
				{
					public:
						...
						getP(pint_t addr)
						{
							return P::getP(*((pint_t *)mappedAddress(addr)));
						}
		...
		class CUSection : public Section<A>
		{
			public:
				...
				virtual uint32_t
				computeAtomCount(class Parser<A> &parser, struct Parser<A>::LabelAndCFIBreakIterator &it,
				                 const struct Parser<A>::CFI_CU_InfoArrays &)
				{
					return 0;
				}
				virtual uint32_t
				appendAtoms(class Parser<A> &parser, uint8_t *buffer, struct Parser<A>::LabelAndCFIBreakIterator &it,
				            const struct Parser<A>::CFI_CU_InfoArrays &)
				{
					return 0;
				}
		...
		class TentativeDefinitionSection : public Section<A>
		{
			public:
				...
				virtual Atom<A>	*
				findAtomByAddress(typename A::P::uint_t addr)
				{
				...
				virtual uint32_t	appendAtoms(class Parser<A> &parser, uint8_t *buffer,
				                                ...
				                                const struct Parser<A>::CFI_CU_InfoArrays &);
				virtual void
				makeFixups(class Parser<A> &parser, const struct Parser<A>::CFI_CU_InfoArrays &) {}
			private:
				typedef typename A::P::uint_t	pint_t;
				typedef typename A::P			P;
		};
		...
		class AbsoluteSymbolSection : public Section<A>
		{
			public:
				...
				virtual ld::Atom::Alignment
				alignmentForAddress(typename A::P::uint_t addr)
				{
		...
		class ImplicitSizeSection : public Section<A>
		{
			...
			protected:
				...
				virtual ld::Atom::Scope
				scopeAtAddress(Parser<A> &parser, pint_t addr)
				{
		...
		class Literal4Section : public FixedSizeSection<A>
		{
			...
			protected:
				...
				virtual const char				*
				unlabeledAtomName(Parser<A> &, pint_t)
				{
				...
				virtual ld::Atom::Combine
				combine(Parser<A> &, pint_t)
				{
		...
		class NonLazyPointerSection : public FixedSizeSection<A>
		{
			...
			protected:
				...
				contentType()
				{
					return ld::Atom::typeNonLazyPointer;
				}
				virtual ld::Atom::Alignment
				alignmentForAddress(pint_t addr)
				{
		...
		class ObjC1ClassSection : public FixedSizeSection<A>
		{
			...
			protected:
				...
				virtual ld::Atom::Scope
				scopeAtAddress(Parser<A> &, pint_t)
				{
				...
				virtual ld::Atom::SymbolTableInclusion
		...
		class Atom : public ld::Atom
		{
			public:
				...
				virtual const uint8_t						*
				rawContentPointer() const
				{
					return contentPointer();
				}
				virtual unsigned long
				contentHash(const ld::IndirectBindingTable &ind) const
				{
					if (_hash == 0)
					{
						_hash = sect().contentHash(this, ind);
					}
					return _hash;
				}
				virtual bool
				canCoalesceWith(const ld::Atom &rhs, const ld::IndirectBindingTable &ind) const
				{
					return sect().canCoalesceWith(this, rhs, ind);
				}
				virtual ld::Fixup::iterator
				fixupsBegin() const
				{
					return &machofile()._fixups[_fixupsStartIndex];
				}
				virtual ld::Fixup::iterator
				fixupsEnd()	const
				{
					return &machofile()._fixups[_fixupsStartIndex + _fixupsCount];
				}
				virtual ld::Atom::UnwindInfo::iterator
				beginUnwind() const
				{
					return &machofile()._unwindInfos[_unwindInfoStartIndex];
				}
				virtual ld::Atom::UnwindInfo::iterator
				endUnwind()	const
				{
					return &machofile()._unwindInfos[_unwindInfoStartIndex + _unwindInfoCount];
				}
				virtual ld::Atom::LineInfo::iterator
				beginLineInfo() const
				{
					return &machofile()._lineInfos[_lineInfoStartIndex];
				}
				virtual ld::Atom::LineInfo::iterator
				endLineInfo() const
				{
					return &machofile()._lineInfos[_lineInfoStartIndex + _lineInfoCount];
				}
			...
			public:
				...
				machofile() const
				{
					return ((Section<A> *)(this->_section))->file();
				}
				...
				{
					return (_lineInfoCount < ((1 << kLineInfoCountBits) - 1));
				}
				void
				incrementLineInfoCount()
				{
					assert(roomForMoreLineInfoCount());
					++_lineInfoCount;
				}
				void
				incrementFixupCount()
				{
					if (_fixupsCount == ((1 << kFixupCountBits) - 1))
					{
						throwf("too may fixups in %s", name());
					}
					++_fixupsCount;
				}
		...
		template <typename A>
		void
		Atom<A>::setFile(const ld::File *f)
		{
		...
		template <typename A>
		void
		Atom<A>::setFixupsRange(uint32_t startIndex, uint32_t count)
		{
		...
		template <typename A>
		void
		Atom<A>::setUnwindInfoRange(uint32_t startIndex, uint32_t count)
		{
		...
		template <typename A>
		void
		Atom<A>::setLineInfoRange(uint32_t startIndex, uint32_t count)
		{
		...
		template <typename A>
		const uint8_t *
		Atom<A>::contentPointer() const
		{
			const macho_section<P> *sct = this->sect().machoSection();
			if (this->_objAddress > sct->addr() + sct->size())
			{
				throwf("malformed .o file, symbol has address 0x%0llX which is outside range of its section",
				       (uint64_t)this->_objAddress);
			}
		...
		template <typename A>
		void
		Atom<A>::copyRawContent(uint8_t buffer[]) const
		{
			// copy base bytes
		...
		template <>
		void
		Atom<arm>::verifyAlignment(const macho_section<P> &) const
		{
		...
		template <>
		void
		Atom<arm64>::verifyAlignment(const macho_section<P> &sect) const
		{
		...
		template <>
		void
		Atom<arm64_32>::verifyAlignment(const macho_section<P> &sect) const
		{
			if ((this->section().type() == ld::Section::typeCode) && (sect.size() != 0))
			{
				if (((_objAddress % 4) != 0) || (this->alignment().powerOf2 < 2))
		...
		template <typename A>
		void
		Atom<A>::verifyAlignment(const macho_section<P> &) const
		{
		}
		...
		class AliasAtom : public ld::Atom
		{
			public:
				...
				virtual void
				copyRawContent(uint8_t buffer[]) const { }
				virtual ld::Fixup::iterator
				fixupsBegin() const
				{
					return &((ld::Fixup *)&_fixup)[0];
				}
				virtual ld::Fixup::iterator
				fixupsEnd()	const
				{
					return &((ld::Fixup *)&_fixup)[1];
				}
		...
		class Parser
		{
			public:
				...
				static void										findPlatforms(const macho_header<typename A::P> *header,  uint64_t fileLength,
				...
				static ld::relocatable::File					*
				parse(const uint8_t *fileContent, uint64_t fileLength,
				...
				struct FixupInAtom
				{
					FixupInAtom(const SourceLocation &src, ld::Fixup::Cluster c, ld::Fixup::Kind k, Atom<A> *target) :
						fixup(src.offsetInAtom, c, k, target), atom(src.atom)
					{
						src.atom->incrementFixupCount();
					}

					FixupInAtom(const SourceLocation &src, ld::Fixup::Cluster c, ld::Fixup::Kind k, ld::Fixup::TargetBinding b,
					            Atom<A> *target) :
						fixup(src.offsetInAtom, c, k, b, target), atom(src.atom)
					{
						src.atom->incrementFixupCount();
					}

					FixupInAtom(const SourceLocation &src, ld::Fixup::Cluster c, ld::Fixup::Kind k, bool wi, const char *name) :
						fixup(src.offsetInAtom, c, k, wi, name), atom(src.atom)
					{
						src.atom->incrementFixupCount();
					}
					...
					FixupInAtom(const SourceLocation &src, ld::Fixup::Cluster c, ld::Fixup::Kind k, ld::Fixup::TargetBinding b,
					            const char *name) :
						fixup(src.offsetInAtom, c, k, b, name), atom(src.atom)
					{
						src.atom->incrementFixupCount();
					}

					FixupInAtom(const SourceLocation &src, ld::Fixup::Cluster c, ld::Fixup::Kind k, uint64_t addend) :
						fixup(src.offsetInAtom, c, k, addend), atom(src.atom)
					{
						src.atom->incrementFixupCount();
					}
					...
					FixupInAtom(const SourceLocation &src, ld::Fixup::Cluster c, ld::Fixup::Kind k, ld::Fixup::AuthData authData) :
						fixup(src.offsetInAtom, c, k, authData), atom(src.atom)
					{
						src.atom->incrementFixupCount();
					}
#endif
					...
					FixupInAtom(const SourceLocation &src, ld::Fixup::Cluster c, ld::Fixup::Kind k) :
						fixup(src.offsetInAtom, c, k, (uint64_t)0), atom(src.atom)
					{
						src.atom->incrementFixupCount();
					}
				...
				void
				addFixup(const SourceLocation &src, ld::Fixup::Cluster c, ld::Fixup::Kind k, Atom<A> *target)
				{
					_allFixups.push_back(FixupInAtom(src, c, k, target));
				}

				void
				addFixup(const SourceLocation &src, ld::Fixup::Cluster c, ld::Fixup::Kind k, ld::Fixup::TargetBinding b,
				         Atom<A> *target)
				{
				...
				void
				addFixup(const SourceLocation &src, ld::Fixup::Cluster c, ld::Fixup::Kind k, bool wi, const char *name)
				{
				...
				void
				addFixup(const SourceLocation &src, ld::Fixup::Cluster c, ld::Fixup::Kind k, uint64_t addend)
				{
...
#if SUPPORT_ARCH_arm64e
				void
				addFixup(const SourceLocation &src, ld::Fixup::Cluster c, ld::Fixup::Kind k, ld::Fixup::AuthData authData)
				{
				...
				void
				addFixup(const SourceLocation &src, ld::Fixup::Cluster c, ld::Fixup::Kind k)
				{
				...
				firstMachOSection()
				{
					return _sectionsStart;
				}
				const macho_section<P>							*machOSectionFromSectionIndex(uint32_t index);
				uint32_t
				machOSectionCount()
				{
					return _machOSectionsCount;
				}
				uint32_t
				undefinedStartIndex()
				{
					return _undefinedStartIndex;
				}
				uint32_t
				undefinedEndIndex()
				{
					return _undefinedEndIndex;
				}
				void
				addFixup(FixupInAtom f)
				{
					_allFixups.push_back(f);
				}
				...
				{
					return _tentativeDefinitionCount;
				}
				uint32_t
				absoluteSymbolCount()
				{
					return _absoluteSymbolCount;
				}
				...
				hasStubsSection()
				{
					return (_stubsSectionNum != 0);
				}
				unsigned int
				stubsSectionNum()
				{
					return _stubsSectionNum;
				}
				...
				{
					return _warnUnwindConversionProblems;
				}
				bool
				hasDataInCodeLabels()
				{
					return _hasDataInCodeLabels;
				}
				bool
				keepDwarfUnwind()
				{
					return _keepDwarfUnwind;
				}
				bool
				forceDwarfConversion()
				{
					return _forceDwarfConversion;
				}
				bool
				verboseOptimizationHints()
				{
					return _verboseOptimizationHints;
				}
				bool
				neverConvertDwarf()
				{
					return _neverConvertDwarf;
				}
				bool
				armUsesZeroCostExceptions()
				{
					return _armUsesZeroCostExceptions;
				}
				uint8_t
				maxDefaultCommonAlignment()
				{
					return _maxDefaultCommonAlignment;
				}


				macho_data_in_code_entry<P>					*
				dataInCodeStart()
				{
					return _dataInCodeStart;
				}
				macho_data_in_code_entry<P>					*
				dataInCodeEnd()
				{
					return _dataInCodeEnd;
				}
				const uint8_t									*
				optimizationHintsStart()
				{
					return _lohStart;
				}
				const uint8_t									*
			...
			private:
				...
				struct MachOSectionAndSectionClass
				{
					...
					static int
					sorter(const void *l, const void *r)
					{
		...
		template <>
		bool
		Parser<x86>::validFile(const uint8_t *fileContent, bool, cpu_subtype_t)
		{
		...
		template <>
		bool
		Parser<x86_64>::validFile(const uint8_t *fileContent, bool, cpu_subtype_t)
		{
			const macho_header<P> *header = (const macho_header<P> *)fileContent;
		...
		template <>
		bool
		Parser<arm>::validFile(const uint8_t *fileContent, bool subtypeMustMatch, cpu_subtype_t subtype)
		{
			const macho_header<P> *header = (const macho_header<P> *)fileContent;
			if (header->magic() != MH_MAGIC)
			{
		...
		template <>
		bool
		Parser<arm64>::validFile(const uint8_t *fileContent, bool subtypeMustMatch, cpu_subtype_t subtype)
		{
			const macho_header<P> *header = (const macho_header<P> *)fileContent;
			if (header->magic() != MH_MAGIC_64)
			{
		...
		template <>
		bool
		Parser<arm64_32>::validFile(const uint8_t *fileContent, bool subtypeMustMatch, cpu_subtype_t subtype)
		{
			const macho_header<P> *header = (const macho_header<P> *)fileContent;
			if (header->magic() != MH_MAGIC)
			{
		...
		template <>
		const char *
		Parser<x86>::fileKind(const uint8_t *fileContent)
		{
			const macho_header<P> *header = (const macho_header<P> *)fileContent;
			if (header->magic() != MH_MAGIC)
			{
		...
		template <>
		const char *
		Parser<x86_64>::fileKind(const uint8_t *fileContent)
		{
			const macho_header<P> *header = (const macho_header<P> *)fileContent;
			if (header->magic() != MH_MAGIC_64)
			{
		...
		template <>
		const char *
		Parser<arm>::fileKind(const uint8_t *fileContent)
		{
			const macho_header<P> *header = (const macho_header<P> *)fileContent;
			if (header->magic() != MH_MAGIC)
			{
		...
		template <>
		const char *
		Parser<arm64>::fileKind(const uint8_t *fileContent)
		{
			const macho_header<P> *header = (const macho_header<P> *)fileContent;
			if (header->magic() != MH_MAGIC_64)
			{
		...
		template <>
		const char *
		Parser<arm64_32>::fileKind(const uint8_t *fileContent)
		{
			const macho_header<P> *header = (const macho_header<P> *)fileContent;
			if (header->magic() != MH_MAGIC)
			{
		...
		template <typename A>
		bool
		Parser<A>::hasObjC2Categories(const uint8_t *fileContent)
		{
			const macho_header<P> *header = (const macho_header<P> *)fileContent;
			const uint32_t cmd_count = header->ncmds();
			const macho_load_command<P> *const cmds = (macho_load_command<P> *)((char *)header + sizeof(macho_header<P>));
			const macho_load_command<P> *const cmdsEnd = (macho_load_command<P> *)((char *)header + sizeof(
			            macho_header<P>) + header->sizeofcmds());
		...
		template <typename A>
		bool
		Parser<A>::hasObjC1Categories(const uint8_t *fileContent)
		{
			const macho_header<P> *header = (const macho_header<P> *)fileContent;
			const uint32_t cmd_count = header->ncmds();
			const macho_load_command<P> *const cmds = (macho_load_command<P> *)((char *)header + sizeof(macho_header<P>));
		...
		template <typename A>
		bool
		Parser<A>::getNonLocalSymbols(const uint8_t *fileContent, std::vector<const char *> &syms)
		{
			const macho_header<P> *header = (const macho_header<P> *)fileContent;
			const uint32_t cmd_count = header->ncmds();
			const macho_load_command<P> *const cmds = (macho_load_command<P> *)((char *)header + sizeof(macho_header<P>));
		...
		template <typename A>
		int
		Parser<A>::pointerSorter(const void *l, const void *r)
		{
		...
		template <typename A>
		typename A::P::uint_t
		Parser<A>::LabelAndCFIBreakIterator::peek(Parser<A> &parser, pint_t startAddr, pint_t endAddr)
		{
			pint_t symbolAddr;
		...
		template <typename A>
		bool
		Parser<A>::LabelAndCFIBreakIterator::next(Parser<A> &parser, const Section<A> &sect, uint32_t sectNum, pint_t startAddr,
		        pint_t endAddr,
		...
		template <>
		typename arm::P::uint_t
		Parser<arm>::realAddr(typename arm::P::uint_t addr)
		{
			return addr & (-2);
		}
		...
		template <typename A>
		typename A::P::uint_t
		Parser<A>::realAddr(typename A::P::uint_t addr)
		{
			return addr;
		}
...
#define STACK_ALLOC_IF_SMALL(_type, _name, _actual_count, _maxCount) \
	...
	if ( _name == NULL ) \
		...
		template <typename A>
		ld::relocatable::File *
		Parser<A>::parse(const ParserOptions &opts)
		{
			// create file object
		...
		template <> uint8_t
		Parser<x86>::loadCommandSizeMask()
		{
			return 0x03;
		}
		template <> uint8_t
		Parser<x86_64>::loadCommandSizeMask()
		{
			return 0x07;
		}
		template <> uint8_t
		Parser<arm>::loadCommandSizeMask()
		{
			return 0x03;
		}
		template <> uint8_t
		Parser<arm64>::loadCommandSizeMask()
		{
			return 0x07;
		}
#if SUPPORT_ARCH_arm64_32
		template <> uint8_t
		Parser<arm64_32>::loadCommandSizeMask()
		{
			return 0x03;
		}
#endif
		...
		template <typename A>
		bool
		Parser<A>::parseLoadCommands(const ld::VersionSet &cmdLinePlatforms, bool internalSDK)
		{
			...
			for (uint32_t i = 0; i < cmd_count; ++i)
			{
				...
				switch (cmd->cmd())
				{
					...
					case LC_BUILD_VERSION:
					{
						...
						const macho_build_tool_version<P> *entry = (macho_build_tool_version<P> *)((uint8_t *)cmd + sizeof(
						            macho_build_version_command<P>));
		...
		template <typename A>
		void
		Parser<A>::findPlatforms(const macho_header<P> *header, uint64_t fileLength, ld::VersionSet &platformsFound)
		{
			const uint32_t cmd_count = header->ncmds();
			if (cmd_count == 0)
			{
		...
		template <typename A>
		void
		Parser<A>::appendAliasAtoms(uint8_t *p)
		{
			for (uint32_t i = 0; i < this->_symbolCount; ++i)
			{
				const macho_nlist<P> &sym =	symbolFromIndex(i);
		...
		template <typename A>
		int
		Parser<A>::sectionIndexSorter(void *extra, const void *l, const void *r)
		{
		...
		template <typename A>
		void
		Parser<A>::makeSortedSectionsArray(uint32_t array[])
		{
			...
			if (log)
			{
				...
				{
					fprintf(stderr, "0x%08llX %s %s\n", _sectionsStart[i].addr(), _sectionsStart[i].segname(),
					        _sectionsStart[i].sectname());
				}
			}
			...
			if (log)
			{
				...
				for (unsigned int i = 0; i < _machOSectionsCount; ++i)
				{
					fprintf(stderr, "0x%08llX %s %s\n", _sectionsStart[array[i]].addr(), _sectionsStart[array[i]].segname(),
					        _sectionsStart[array[i]].sectname());
				}
			}
		}
		...
		template <typename A>
		int
		Parser<A>::symbolIndexSorter(void *extra, const void *l, const void *r)
		{
		...
		template <typename A>
		void
		Parser<A>::makeSortedSymbolsArray(uint32_t array[], const uint32_t sectionArray[])
		{
			const bool log = false;
			uint32_t *p = array;
			for (uint32_t i = 0; i < this->_symbolCount; ++i)
			...
			if (log)
			{
				...
				{
					fprintf(stderr, "0x%09llX symIndex=%d sectNum=%2d, %s\n", symbolFromIndex(array[i]).n_value(), array[i],
					        symbolFromIndex(array[i]).n_sect(), nameFromSymbol(symbolFromIndex(array[i])));
				}
			}
		}
		...
		Parser<A>::makeSections()
		{
			...
			uint8_t machOSectsStorage[sizeof(MachOSectionAndSectionClass<P>) * (_machOSectionsCount +
			...
			for (uint32_t i = 0; i < _machOSectionsCount; ++i)
			{
				...
				if ((sect->offset() + sect->size() > _fileLength) && (sectionType != S_ZEROFILL)
			...
			{
				if (cmdlineSect != NULL)
					_file->_bitcode = std::unique_ptr<ld::Bitcode>(new ld::ClangBitcode(&_fileContent[bitcodeSect->offset()],
					                  bitcodeSect->size(),
					                  &_fileContent[cmdlineSect->offset()], cmdlineSect->size()));
				else if (swiftCmdlineSect != NULL)
					_file->_bitcode = std::unique_ptr<ld::Bitcode>(new ld::SwiftBitcode(&_fileContent[bitcodeSect->offset()],
					                  bitcodeSect->size(),
			...
			{
				_file->_bitcode = std::unique_ptr<ld::Bitcode>(new ld::BundleBitcode(&_fileContent[bundleSect->offset()],
				                  bundleSect->size()));
			}
			else if (bitcodeAsm)
			{
		...
		template <typename A>
		Section<A> *
		Parser<A>::sectionForAddress(typename A::P::uint_t addr)
		{
		...
		template <typename A>
		Section<A> *
		Parser<A>::sectionForNum(unsigned int num)
		{
			for (uint32_t i = 0; i < _file->_sectionsArrayCount; ++i)
			{
				const macho_section<typename A::P> *sect = _file->_sectionsArray[i]->machoSection();
		...
		template <typename A>
		Atom<A> *
		Parser<A>::findAtomByAddress(pint_t addr)
		{
		...
		template <typename A>
		Atom<A> *
		Parser<A>::findAtomByAddressOrNullIfStub(pint_t addr)
		{
			if (hasStubsSection() && (_stubsMachOSection->addr() <= addr)
			        && (addr < (_stubsMachOSection->addr() + _stubsMachOSection->size())))
			{
				return NULL;
			}
		...
		template <typename A>
		Atom<A> *
		Parser<A>::findAtomByAddressOrLocalTargetOfStub(pint_t addr, uint32_t *offsetInAtom)
		{
			if (hasStubsSection() && (_stubsMachOSection->addr() <= addr)
			        && (addr < (_stubsMachOSection->addr() + _stubsMachOSection->size())))
			{
				...
				assert((this->combineFromSymbol(sym) != ld::Atom::combineByName)
				       || (this->scopeFromSymbol(sym) != ld::Atom::scopeGlobal));
				*offsetInAtom = 0;
		...
		template <typename A>
		Atom<A> *
		Parser<A>::findAtomByName(const char *name)
		{
			uint8_t *p = _file->_atomsArray;
			for (int i = _file->_atomsArrayCount; i > 0; --i)
			{
		...
		template <typename A>
		void
		Parser<A>::findTargetFromAddress(pint_t addr, TargetDesc &target)
		{
			if (hasStubsSection() && (_stubsMachOSection->addr() <= addr)
			        && (addr < (_stubsMachOSection->addr() + _stubsMachOSection->size())))
			{
		...
		template <typename A>
		void
		Parser<A>::findTargetFromAddress(pint_t baseAddr, pint_t addr, TargetDesc &target)
		{
		...
		template <typename A>
		void
		Parser<A>::findTargetFromAddressAndSectionNum(pint_t addr, unsigned int sectNum, TargetDesc &target)
		{
			if (sectNum == R_ABS)
			{
		...
		template <typename A>
		void
		Parser<A>::addDtraceExtraInfos(const SourceLocation &src, const char *providerName)
		{
		...
		template <typename A>
		const char *
		Parser<A>::scanSymbolTableForAddress(uint64_t addr)
		{
		...
		template <typename A>
		void
		Parser<A>::addFixups(const SourceLocation &src, ld::Fixup::Kind setKind, const TargetDesc &target)
		{
			...
			if (target.atom != NULL)
			{
				...
				else if ((target.atom->combine() == ld::Atom::combineByNameAndContent)
				         || (target.atom->combine() == ld::Atom::combineByNameAndReferences))
				{
		...
		template <typename A>
		void
		Parser<A>::addFixups(const SourceLocation &src, ld::Fixup::Kind kind, const TargetDesc &target,
		                     const TargetDesc &picBase)
		{
		...
		template <typename A>
		uint32_t
		TentativeDefinitionSection<A>::computeAtomCount(class Parser<A> &parser,
		        struct Parser<A>::LabelAndCFIBreakIterator &it,
		        const struct Parser<A>::CFI_CU_InfoArrays &)
		{
		...
		template <typename A>
		uint32_t
		TentativeDefinitionSection<A>::appendAtoms(class Parser<A> &parser, uint8_t *p,
		        struct Parser<A>::LabelAndCFIBreakIterator &it,
		        const struct Parser<A>::CFI_CU_InfoArrays &)
		{
		...
		template <typename A>
		uint32_t
		AbsoluteSymbolSection<A>::computeAtomCount(class Parser<A> &parser,
		        struct Parser<A>::LabelAndCFIBreakIterator &it,
		        const struct Parser<A>::CFI_CU_InfoArrays &)
		{
		...
		template <typename A>
		uint32_t
		AbsoluteSymbolSection<A>::appendAtoms(class Parser<A> &parser, uint8_t *p,
		                                      struct Parser<A>::LabelAndCFIBreakIterator &it,
		                                      const struct Parser<A>::CFI_CU_InfoArrays &)
		{
			this->_beginAtoms = (Atom<A> *)p;
		...
		template <typename A>
		Atom<A> *
		AbsoluteSymbolSection<A>::findAbsAtomForValue(typename A::P::uint_t value)
		{
		...
		template <typename A>
		uint32_t
		Parser<A>::indirectSymbol(uint32_t indirectIndex)
		{
		...
		template <typename A>
		const macho_nlist<typename A::P> &
		Parser<A>::symbolFromIndex(uint32_t index)
		{
		...
		template <typename A>
		const macho_section<typename A::P>	*
		Parser<A>::machOSectionFromSectionIndex(uint32_t index)
		{
		...
		template <typename A>
		uint32_t
		Parser<A>::symbolIndexFromIndirectSectionAddress(pint_t addr, const macho_section<P> *sect)
		{
		...
		template <typename A>
		const char *
		Parser<A>::nameFromSymbol(const macho_nlist<P> &sym)
		{
		...
		template <typename A>
		ld::Atom::Scope
		Parser<A>::scopeFromSymbol(const macho_nlist<P> &sym)
		{
			if ((sym.n_type() & N_EXT) == 0)
			{
				return ld::Atom::scopeTranslationUnit;
		...
		template <typename A>
		ld::Atom::Definition
		Parser<A>::definitionFromSymbol(const macho_nlist<P> &sym)
		{
		...
		template <typename A>
		ld::Atom::Combine
		Parser<A>::combineFromSymbol(const macho_nlist<P> &sym)
		{
		...
		template <typename A>
		ld::Atom::SymbolTableInclusion
		Parser<A>::inclusionFromSymbol(const macho_nlist<P> &sym)
		{
		...
		template <typename A>
		bool
		Parser<A>::dontDeadStripFromSymbol(const macho_nlist<P> &sym)
		{
		...
		template <typename A>
		bool
		Parser<A>::isThumbFromSymbol(const macho_nlist<P> &sym)
		{
		...
		template <typename A>
		bool
		Parser<A>::weakImportFromSymbol(const macho_nlist<P> &sym)
		{
		...
		template <typename A>
		bool
		Parser<A>::resolverFromSymbol(const macho_nlist<P> &sym)
		{
		...
		template <typename A>
		bool
		Parser<A>::altEntryFromSymbol(const macho_nlist<P> &sym)
		{
		...
		template <typename A>
		bool
		Parser<A>::coldFromSymbol(const macho_nlist<P> &sym)
		{
		...
		template <typename A>
		bool
		Parser<A>::skip_form(const uint8_t **offset, const uint8_t *end, uint64_t form,
		...
		template <typename A>
		const char *
		Parser<A>::getStrxString(uint64_t idx, bool dwarf64)
		{
		...
		template <typename A>
		const char *
		Parser<A>::getDwarfString(uint64_t form, const uint8_t *&di, bool dwarf64)
		{
			uint32_t offset;
		...
		template <typename A>
		uint64_t
		Parser<A>::getDwarfOffset(uint64_t form, const uint8_t *&di, bool dwarf64)
		{
		...
		template <typename A>
		bool
		Parser<A>::isConstFunStabs(const char *stabStr)
		{
			const char *colon;
					...
					if ((stmtList != (uint64_t) -1) && (stmtList < _file->_dwarfDebugLineSect->size()))
					{
						...
						if (lines != NULL)
						{
							while (line_next(lines, &result, line_stop_pc))
							{
								...
								else if (result.end_of_sequence && (curAtom == NULL) && (this->findAtomByAddress(0) != NULL)
								         && (result.pc == this->findAtomByAddress(0)->size()))
								{
		...
		Parser<A>::parseStabs()
		{
			...
			for (uint32_t symbolIndex = 0; symbolIndex < _symbolCount; ++symbolIndex)
			{
				...
				if ((type & N_STAB) != 0)
				{
					_file->_debugInfoKind = (_hasUUID ? ld::relocatable::File::kDebugInfoStabsUUID :
		...
		template <typename A>
		bool
		Parser<A>::read_comp_unit(const char **name, const char **comp_dir,
		                          uint64_t *stmt_list)
		{
		...
		template <typename A>
		const char *
		File<A>::translationUnitSource() const
		{
		...
		template <typename A>
		bool
		File<A>::forEachAtom(ld::File::AtomHandler &handler) const
		{
			handler.doFile(*this);
		...
		template <typename A>
		const char *
		Section<A>::makeSegmentName(const macho_section<typename A::P> *sect)
		{
		...
		template <typename A>
		const char *
		Section<A>::makeSectionName(const macho_section<typename A::P> *sect)
		{
		...
		template <typename A>
		bool
		Section<A>::readable(const macho_section<typename A::P> *sect)
		{
			return true;
		}
		...
		template <typename A>
		bool
		Section<A>::writable(const macho_section<typename A::P> *sect)
		{
		...
		template <typename A>
		bool
		Section<A>::exectuable(const macho_section<typename A::P> *sect)
		{
			// mach-o .o files do not contain segment permissions
			// we just know TEXT is special
			return (strcmp(sect->segname(), "__TEXT") == 0);
		...
		template <typename A>
		ld::Section::Type
		Section<A>::sectionType(const macho_section<typename A::P> *sect)
		{
			switch (sect->flags() & SECTION_TYPE)
			{
				case S_ZEROFILL:
		...
		template <typename A>
		Atom<A> *
		Section<A>::findContentAtomByAddress(pint_t addr, class Atom<A> *start, class Atom<A> *end)
		{
		...
		template <typename A>
		ld::Atom::Alignment
		Section<A>::alignmentForAddress(pint_t addr)
		{
		...
		template <typename A>
		uint32_t
		Section<A>::sectionNum(class Parser<A> &parser) const
		{
		...
		template <>
		uint32_t
		CFISection<arm>::cfiCount(Parser<arm> &parser)
		{
		...
		template <typename A>
		uint32_t
		CFISection<A>::cfiCount(Parser<A> &parser)
		{
			// create ObjectAddressSpace object for use by libunwind
			OAS oas(*this, (uint8_t *)this->file().fileContent() + this->_machOSection->offset());
			return libunwind::CFI_Parser<OAS>::getCFICount(oas,
		...
		template <typename A>
		void
		CFISection<A>::warnFunc(void *ref, uint64_t funcAddr, const char *msg)
		{
		...
		template <>
		bool
		CFISection<x86_64>::needsRelocating()
		{
			return true;
		}
		...
		template <>
		bool
		CFISection<arm64>::needsRelocating()
		{
			return true;
		}
		...
		template <>
		bool
		CFISection<arm64_32>::needsRelocating()
		{
			return true;
		}
#endif
		...
		template <>
		void
		CFISection<x86_64>::cfiParse(class Parser<x86_64> &parser, uint8_t *buffer,
		                             ...
		                             uint32_t &count, const pint_t cuStarts[], uint32_t cuCount)
		{
			...
			const macho_relocation_info<P> *relocs = (macho_relocation_info<P> *)(file().fileContent() +
			        this->_machOSection->reloff());
		...
		template <>
		void
		CFISection<x86>::cfiParse(class Parser<x86> &parser, uint8_t *buffer,
		...
		template <>
		void
		CFISection<arm>::cfiParse(class Parser<arm> &parser, uint8_t *buffer,
		...
		template <>
		void
		CFISection<arm64>::cfiParse(class Parser<arm64> &parser, uint8_t *buffer,
		...
		template <>
		void
		CFISection<arm64_32>::cfiParse(class Parser<arm64_32> &parser, uint8_t *buffer,
		...
		template <typename A>
		uint32_t
		CFISection<A>::computeAtomCount(class Parser<A> &parser,
		                                struct Parser<A>::LabelAndCFIBreakIterator &it,
		...
		template <typename A>
		uint32_t
		CFISection<A>::appendAtoms(class Parser<A> &parser, uint8_t *p,
		                           struct Parser<A>::LabelAndCFIBreakIterator &it,
		                           const struct Parser<A>::CFI_CU_InfoArrays &cfis)
		{
			this->_beginAtoms = (Atom<A> *)p;
		...
		template <> bool
		CFISection<x86_64>::bigEndian()
		{
			return false;
		}
		template <> bool
		CFISection<x86>::bigEndian()
		{
			return false;
		}
		template <> bool
		CFISection<arm>::bigEndian()
		{
			return false;
		}
		template <> bool
		CFISection<arm64>::bigEndian()
		{
			return false;
		}
#if SUPPORT_ARCH_arm64_32
		template <> bool
		CFISection<arm64_32>::bigEndian()
		{
			return false;
		}
#endif
		...
		template <>
		void
		CFISection<x86_64>::addCiePersonalityFixups(class Parser<x86_64> &parser, const CFI_Atom_Info *cieInfo)
		{
		...
		template <>
		void
		CFISection<x86>::addCiePersonalityFixups(class Parser<x86> &parser, const CFI_Atom_Info *cieInfo)
		{
			uint8_t personalityEncoding = cieInfo->u.cieInfo.personality.encodingOfTargetAddress;
		...
		template <>
		void
		CFISection<arm64>::addCiePersonalityFixups(class Parser<arm64> &parser, const CFI_Atom_Info *cieInfo)
		{
			uint8_t personalityEncoding = cieInfo->u.cieInfo.personality.encodingOfTargetAddress;
			if (personalityEncoding == 0x9B)
			{
		...
		template <>
		void
		CFISection<arm64_32>::addCiePersonalityFixups(class Parser<arm64_32> &parser, const CFI_Atom_Info *cieInfo)
		{
			uint8_t personalityEncoding = cieInfo->u.cieInfo.personality.encodingOfTargetAddress;
			if (personalityEncoding == 0x9B)
			{
		...
		template <>
		void
		CFISection<arm>::addCiePersonalityFixups(class Parser<arm> &parser, const CFI_Atom_Info *cieInfo)
		{
			uint8_t personalityEncoding = cieInfo->u.cieInfo.personality.encodingOfTargetAddress;
			if ((personalityEncoding == 0x9B) || (personalityEncoding == 0x90))
			{
		...
		template <typename A>
		void
		CFISection<A>::addCiePersonalityFixups(class Parser<A> &parser, const CFI_Atom_Info *cieInfo)
		{
		...
		template <typename A>
		void
		CFISection<A>::makeFixups(class Parser<A> &parser, const struct Parser<A>::CFI_CU_InfoArrays &cfis)
		{
		...
		template <typename A>
		const void	 *
		CFISection<A>::OAS::mappedAddress(pint_t addr)
		{
		...
		template <typename A>
		uint64_t
		CFISection<A>::OAS::getULEB128(pint_t &logicalAddr, pint_t end)
		{
		...
		template <typename A>
		int64_t
		CFISection<A>::OAS::getSLEB128(pint_t &logicalAddr, pint_t end)
		{
			uintptr_t size = (end - logicalAddr);
			libunwind::LocalAddressSpace::pint_t laddr = (libunwind::LocalAddressSpace::pint_t)mappedAddress(logicalAddr);
			libunwind::LocalAddressSpace::pint_t sladdr = laddr;
		...
		template <typename A>
		typename A::P::uint_t
		CFISection<A>::OAS::getEncodedP(pint_t &addr, pint_t end, uint8_t encoding)
		{
		...
		template <>
		const char *
		CUSection<x86_64>::personalityName(class Parser<x86_64> &parser, const macho_relocation_info<x86_64::P> *reloc)
		{
			if (reloc->r_extern())
			{
				assert((reloc->r_type() == X86_64_RELOC_UNSIGNED)
			...
			else
			{
				...
				assert((parser.sectionForAddress(personalityAddr)->type() == ld::Section::typeCode)
		...
		template <>
		const char *
		CUSection<x86>::personalityName(class Parser<x86> &parser, const macho_relocation_info<x86::P> *reloc)
		{
			if (reloc->r_extern())
			{
		...
		template <>
		const char *
		CUSection<arm64>::personalityName(class Parser<arm64> &parser, const macho_relocation_info<arm64::P> *reloc)
		{
			if (reloc->r_extern())
			{
				assert((reloc->r_type() == ARM64_RELOC_UNSIGNED)
				       && "wrong reloc type on personality column in __compact_unwind section");
				const macho_nlist<P> &sym = parser.symbolFromIndex(reloc->r_symbolnum());
				return parser.nameFromSymbol(sym);
			}
			else
			{
				...
				assert((personalitySection->type() == ld::Section::typeCode)
				       && "personality column in __compact_unwind section is not pointer to function");
				// atoms may not be constructed yet, so scan symbol table for labels
				const char *name = parser.scanSymbolTableForAddress(personalityAddr);
				return name;
		...
		template <>
		const char *
		CUSection<arm64_32>::personalityName(class Parser<arm64_32> &parser, const macho_relocation_info<arm64_32::P> *reloc)
		{
			if (reloc->r_extern())
			{
				assert((reloc->r_type() == ARM64_RELOC_UNSIGNED)
		...
		template <>
		const char *
		CUSection<arm>::personalityName(class Parser<arm> &parser, const macho_relocation_info<arm::P> *reloc)
		{
			if (reloc->r_extern())
			{
		...
		template <typename A>
		const char *
		CUSection<A>::personalityName(class Parser<A> &parser, const macho_relocation_info<P> *reloc)
		{
			return NULL;
		}
		...
		template <>
		bool
		CUSection<x86>::encodingMeansUseDwarf(compact_unwind_encoding_t enc)
		{
		...
		template <>
		bool
		CUSection<x86_64>::encodingMeansUseDwarf(compact_unwind_encoding_t enc)
		{
		...
		template <>
		bool
		CUSection<arm>::encodingMeansUseDwarf(compact_unwind_encoding_t enc)
		{
		...
		template <>
		bool
		CUSection<arm64>::encodingMeansUseDwarf(compact_unwind_encoding_t enc)
		{
		...
		template <>
		bool
		CUSection<arm64_32>::encodingMeansUseDwarf(compact_unwind_encoding_t enc)
		{
			return ((enc & UNWIND_ARM64_MODE_MASK) == UNWIND_ARM64_MODE_DWARF);
		}
#endif
		...
		template <typename A>
		int
		CUSection<A>::infoSorter(const void *l, const void *r)
		{
		...
		template <typename A>
		void
		CUSection<A>::parse(class Parser<A> &parser, uint32_t cnt, Info array[])
		{
			// walk section content and copy to Info array
			const macho_compact_unwind_entry<P> *const entries = (macho_compact_unwind_entry<P> *)(
			            this->file().fileContent() + this->_machOSection->offset());
			...
			if (this->_machOSection->reloff() + this->_machOSection->nreloc()*sizeof(macho_relocation_info<P>) >
			        parser.fileLength())
			{
				throw "relocations beyond end of file";
			}
			const macho_relocation_info<P> *relocs = (macho_relocation_info<P> *)(this->file().fileContent() +
			        this->_machOSection->reloff());
			const macho_relocation_info<P> *relocsEnd = &relocs[this->_machOSection->nreloc()];
			for (const macho_relocation_info<P> *reloc = relocs; reloc < relocsEnd; ++reloc)
			{
				...
				if (reloc->r_extern())
				{
					...
					if ((reloc->r_address() % sizeof(macho_compact_unwind_entry<P>)) ==
					        macho_compact_unwind_entry<P>::personalityFieldOffset())
					{
					...
					else if ((reloc->r_address() % sizeof(macho_compact_unwind_entry<P>)) ==
					         macho_compact_unwind_entry<P>::lsdaFieldOffset())
					{
						uint32_t entryIndex = reloc->r_address() / sizeof(macho_compact_unwind_entry<P>);
						...
						else
						{
							warning("unexpected extern relocation to lsda in __compact_unwind section");
					         ...
					         macho_compact_unwind_entry<P>::codeStartFieldOffset())
					{
						uint32_t entryIndex = reloc->r_address() / sizeof(macho_compact_unwind_entry<P>);
		...
		template <typename A>
		void
		CUSection<A>::makeFixups(class Parser<A> &parser, const struct Parser<A>::CFI_CU_InfoArrays &cus)
		{
		...
		template <typename A>
		bool
		SymboledSection<A>::dontDeadStrip()
		{
			switch (_type)
			{
		...
		template <typename A>
		uint32_t
		SymboledSection<A>::computeAtomCount(class Parser<A> &parser,
		                                     struct Parser<A>::LabelAndCFIBreakIterator &it,
		                                     const struct Parser<A>::CFI_CU_InfoArrays &)
		{
		...
		template <typename A>
		uint32_t
		SymboledSection<A>::appendAtoms(class Parser<A> &parser, uint8_t *p,
		                                struct Parser<A>::LabelAndCFIBreakIterator &it,
		                                const struct Parser<A>::CFI_CU_InfoArrays &)
		{
			this->_beginAtoms = (Atom<A> *)p;
		...
		template <>
		ld::Atom::SymbolTableInclusion
		ImplicitSizeSection<arm64>::symbolTableInclusion()
		{
		...
		template <>
		ld::Atom::SymbolTableInclusion
		ImplicitSizeSection<arm64_32>::symbolTableInclusion()
		{
			return ld::Atom::symbolTableInWithRandomAutoStripLabel;
		}
#endif
		...
		template <typename A>
		ld::Atom::SymbolTableInclusion
		ImplicitSizeSection<A>::symbolTableInclusion()
		{
			return ld::Atom::symbolTableNotIn;
		}
		...
		template <typename A>
		uint32_t
		ImplicitSizeSection<A>::computeAtomCount(class Parser<A> &parser,
		        struct Parser<A>::LabelAndCFIBreakIterator &it,
		        const struct Parser<A>::CFI_CU_InfoArrays &)
		{
			uint32_t count = 0;
			...
			if (it.fileHasOverlappingSymbols && (sect->size() != 0)
			        && (this->combine(parser, startAddr) == ld::Atom::combineByNameAndContent))
			{
				...
				for (uint32_t i = 0; i < it.sortedSymbolCount; ++i)
				{
					...
					if (!ignore && !prevIgnore && (symbolAddr == prevSymbolAddr) && (prevSymbolSectNum == symbolSectNum)
					        && (symbolSectNum == this->sectionNum(parser)))
					{
						++count;
					}
		...
		template <typename A>
		uint32_t
		ImplicitSizeSection<A>::appendAtoms(class Parser<A> &parser, uint8_t *p,
		                                    struct Parser<A>::LabelAndCFIBreakIterator &it,
		                                    const struct Parser<A>::CFI_CU_InfoArrays &)
		{
			this->_beginAtoms = (Atom<A> *)p;
		...
		template <typename A>
		bool
		Literal4Section<A>::ignoreLabel(const char *label) const
		{
		...
		template <typename A>
		unsigned long
		Literal4Section<A>::contentHash(const class Atom<A> *atom, const ld::IndirectBindingTable &ind) const
		{
		...
		template <typename A>
		bool
		Literal4Section<A>::canCoalesceWith(const class Atom<A> *atom, const ld::Atom &rhs,
		                                    const ld::IndirectBindingTable &ind) const
		{
		...
		template <typename A>
		bool
		Literal8Section<A>::ignoreLabel(const char *label) const
		{
			return (label[0] == 'L') || (label[0] == 'l');
		}
		...
		template <typename A>
		unsigned long
		Literal8Section<A>::contentHash(const class Atom<A> *atom, const ld::IndirectBindingTable &ind) const
		{
#if __LP64__
		...
		template <typename A>
		bool
		Literal8Section<A>::canCoalesceWith(const class Atom<A> *atom, const ld::Atom &rhs,
		                                    const ld::IndirectBindingTable &ind) const
		{
		...
		template <typename A>
		bool
		Literal16Section<A>::ignoreLabel(const char *label) const
		{
			return (label[0] == 'L') || (label[0] == 'l');
		}
		...
		template <typename A>
		unsigned long
		Literal16Section<A>::contentHash(const class Atom<A> *atom, const ld::IndirectBindingTable &ind) const
		{
			unsigned long hash = 5381;
			const uint8_t *byteContent = atom->contentPointer();
			for (int i = 0; i < 16; ++i)
		...
		template <typename A>
		bool
		Literal16Section<A>::canCoalesceWith(const class Atom<A> *atom, const ld::Atom &rhs,
		                                     const ld::IndirectBindingTable &ind) const
		{
		...
		template <typename A>
		typename A::P::uint_t
		CStringSection<A>::elementSizeAtAddress(pint_t addr)
		{
			const macho_section<P> *sect = this->machoSection();
		...
		template <typename A>
		bool
		CStringSection<A>::useElementAt(Parser<A> &parser, struct Parser<A>::LabelAndCFIBreakIterator &it, pint_t addr)
		{
			return true;
		}
		...
		template <typename A>
		bool
		CStringSection<A>::ignoreLabel(const char *label) const
		{
			return (label[0] == 'L') || (label[0] == 'l');
		}
		...
		template <typename A>
		Atom<A> *
		CStringSection<A>::findAtomByAddress(pint_t addr)
		{
		...
		template <typename A>
		unsigned long
		CStringSection<A>::contentHash(const class Atom<A> *atom, const ld::IndirectBindingTable &ind) const
		{
			unsigned long hash = 5381;
		...
		template <typename A>
		bool
		CStringSection<A>::canCoalesceWith(const class Atom<A> *atom, const ld::Atom &rhs,
		                                   const ld::IndirectBindingTable &ind) const
		{
		...
		template <>
		ld::Fixup::Kind
		NonLazyPointerSection<x86>::fixupKind()
		{
		...
		template <>
		ld::Fixup::Kind
		NonLazyPointerSection<arm>::fixupKind()
		{
			return ld::Fixup::kindStoreLittleEndian32;
		}
		...
		template <>
		ld::Fixup::Kind
		NonLazyPointerSection<arm64>::fixupKind()
		{
		...
		template <>
		ld::Fixup::Kind
		NonLazyPointerSection<arm64_32>::fixupKind()
		{
			return ld::Fixup::kindStoreLittleEndian32;
		}
#endif
		...
		template <>
		void
		NonLazyPointerSection<x86_64>::makeFixups(class Parser<x86_64> &parser,
		        const struct Parser<x86_64>::CFI_CU_InfoArrays &)
		{
		...
		template <typename A>
		void
		NonLazyPointerSection<A>::makeFixups(class Parser<A> &parser, const struct Parser<A>::CFI_CU_InfoArrays &)
		{
		...
		template <typename A>
		ld::Atom::Combine
		NonLazyPointerSection<A>::combine(Parser<A> &parser, pint_t addr)
		{
			const macho_section<P> *sect = this->machoSection();
			uint32_t symIndex = parser.symbolIndexFromIndirectSectionAddress(addr, sect);
			if (symIndex == INDIRECT_SYMBOL_LOCAL)
		...
		template <typename A>
		const char *
		NonLazyPointerSection<A>::targetName(const class Atom<A> *atom, const ld::IndirectBindingTable &ind)
		{
		...
		template <typename A>
		unsigned long
		NonLazyPointerSection<A>::contentHash(const class Atom<A> *atom, const ld::IndirectBindingTable &ind) const
		{
			assert(atom->combine() == ld::Atom::combineByNameAndReferences);
		...
		template <typename A>
		bool
		NonLazyPointerSection<A>::canCoalesceWith(const class Atom<A> *atom, const ld::Atom &rhs,
		...
		template <typename A>
		ld::Atom::Scope
		NonLazyPointerSection<A>::scopeAtAddress(Parser<A> &parser, pint_t addr)
		{
			const macho_section<P> *sect = this->machoSection();
			uint32_t symIndex = parser.symbolIndexFromIndirectSectionAddress(addr, sect);
			if (symIndex == INDIRECT_SYMBOL_LOCAL)
		...
		template <typename A>
		ld::Atom::Combine
		TLVPointerSection<A>::combine(Parser<A> &parser, pint_t addr)
		{
			return ld::Atom::combineByNameAndReferences;
		}
		...
		template <>
		void
		TLVPointerSection<arm>::makeFixups(class Parser<arm> &parser, const struct Parser<arm>::CFI_CU_InfoArrays &)
		{
		...
		template <typename A>
		void
		TLVPointerSection<A>::makeFixups(class Parser<A> &parser, const struct Parser<A>::CFI_CU_InfoArrays &)
		{
		...
		template <typename A>
		const char *
		TLVPointerSection<A>::targetName(const class Atom<A> *atom, const ld::IndirectBindingTable &ind, bool *isStatic)
		{
			assert(atom->combine() == ld::Atom::combineByNameAndReferences);
			assert(atom->fixupCount() == 1);
			*isStatic = false;
		...
		template <typename A>
		unsigned long
		TLVPointerSection<A>::contentHash(const class Atom<A> *atom, const ld::IndirectBindingTable &ind) const
		{
			assert(atom->combine() == ld::Atom::combineByNameAndReferences);
			unsigned long hash = 9508;
			bool isStatic;
		...
		template <typename A>
		bool
		TLVPointerSection<A>::canCoalesceWith(const class Atom<A> *atom, const ld::Atom &rhs,
		                                      const ld::IndirectBindingTable &indirectBindingTable) const
		{
		...
		template <typename A>
		const uint8_t *
		CFStringSection<A>::targetContent(const class Atom<A> *atom, const ld::IndirectBindingTable &ind,
		                                  ContentType *ct, unsigned int *count)
		{
			*ct = contentUnknown;
		...
		template <typename A>
		unsigned long
		CFStringSection<A>::contentHash(const class Atom<A> *atom, const ld::IndirectBindingTable &ind) const
		{
		...
		template <typename A>
		bool
		CFStringSection<A>::canCoalesceWith(const class Atom<A> *atom, const ld::Atom &rhs,
		                                    const ld::IndirectBindingTable &indirectBindingTable) const
		{
			if (atom == &rhs)
			{
		...
		template <typename A>
		typename A::P::uint_t
		ObjC1ClassSection<A>::elementSizeAtAddress(pint_t addr)
		{
		...
		template <typename A>
		const char *
		ObjC1ClassSection<A>::unlabeledAtomName(Parser<A> &parser, pint_t addr)
		{
		...
		template <typename A>
		const char *
		ObjC2ClassRefsSection<A>::targetClassName(const class Atom<A> *atom, const ld::IndirectBindingTable &ind) const
		{
			assert(atom->fixupCount() == 1);
			ld::Fixup::iterator fit = atom->fixupsBegin();
		...
		template <typename A>
		unsigned long
		ObjC2ClassRefsSection<A>::contentHash(const class Atom<A> *atom, const ld::IndirectBindingTable &ind) const
		{
		...
		template <typename A>
		bool
		ObjC2ClassRefsSection<A>::canCoalesceWith(const class Atom<A> *atom, const ld::Atom &rhs,
		        const ld::IndirectBindingTable &indirectBindingTable) const
		{
			assert(this->type() == rhs.section().type());
			const Atom<A> *rhsAtom = dynamic_cast<const Atom<A>*>(&rhs);
		...
		template <typename A>
		const char *
		Objc1ClassReferences<A>::targetCString(const class Atom<A> *atom, const ld::IndirectBindingTable &ind) const
		{
		...
		template <typename A>
		const char *
		PointerToCStringSection<A>::targetCString(const class Atom<A> *atom, const ld::IndirectBindingTable &ind) const
		{
			assert(atom->fixupCount() == 1);
			ld::Fixup::iterator fit = atom->fixupsBegin();
			const ld::Atom *targetAtom = NULL;
		...
		template <typename A>
		unsigned long
		PointerToCStringSection<A>::contentHash(const class Atom<A> *atom,
		                                        const ld::IndirectBindingTable &indirectBindingTable) const
		{
		...
		template <typename A>
		bool
		PointerToCStringSection<A>::canCoalesceWith(const class Atom<A> *atom, const ld::Atom &rhs,
		        const ld::IndirectBindingTable &indirectBindingTable) const
		{
			assert(this->type() == rhs.section().type());
		...
		template <typename A>
		unsigned long
		UTF16StringSection<A>::contentHash(const class Atom<A> *atom, const ld::IndirectBindingTable &ind) const
		{
			unsigned long hash = 5381;
		...
		template <typename A>
		bool
		UTF16StringSection<A>::canCoalesceWith(const class Atom<A> *atom, const ld::Atom &rhs,
		                                       const ld::IndirectBindingTable &ind) const
		{
		...
		template <>
		uint32_t
		Section<x86_64>::x86_64PcRelOffset(uint8_t r_type)
		{
			switch (r_type)
			{
		...
		template <>
		bool
		Section<x86_64>::addRelocFixup(class Parser<x86_64> &parser, const macho_relocation_info<P> *reloc)
		{
			const macho_section<P> *sect = this->machoSection();
			if (sect == NULL)
			{
				...
				case X86_64_RELOC_SUBTRACTOR:
					...
					else
					{
						...
						useDirectBinding = (toTarget.atom->scope() == ld::Atom::scopeTranslationUnit)
						                   || ((toTarget.atom->combine() == ld::Atom::combineByNameAndContent)
						                       || (toTarget.atom->combine() == ld::Atom::combineByNameAndReferences));
					}
					if (useDirectBinding)
					{
						if ((toTarget.atom->combine() == ld::Atom::combineByNameAndContent)
						        || (toTarget.atom->combine() == ld::Atom::combineByNameAndReferences))
						{
							parser.addFixup(src, ld::Fixup::k1of4, ld::Fixup::kindSetTargetAddress, ld::Fixup::bindingByContentBound,
							                toTarget.atom);
						}
						else
						{
		...
		template <>
		bool
		Section<x86>::addRelocFixup(class Parser<x86> &parser, const macho_relocation_info<P> *reloc)
		{
			const macho_section<P> *sect = this->machoSection();
			uint32_t srcAddr;
		...
		template <>
		bool
		Section<arm>::addRelocFixup(class Parser<arm> &parser, const macho_relocation_info<P> *reloc)
		{
			const macho_section<P> *sect = this->machoSection();
			bool result = false;
			uint32_t srcAddr;
			...
			if ((reloc->r_address() & R_SCATTERED) == 0)
			{
				...
				if (reloc->r_extern())
				{
					const macho_nlist<P> &targetSymbol = parser.symbolFromIndex(reloc->r_symbolnum());
					// use direct reference for local symbols
					if (((targetSymbol.n_type() & N_TYPE) == N_SECT) && (((targetSymbol.n_type() & N_EXT) == 0)
					        || (parser.nameFromSymbol(targetSymbol)[0] == 'L')))
					{
				...
				switch (sreloc->r_type())
				{
					...
					case ARM_RELOC_LOCAL_SECTDIFF:
					{
						...
						if (addend < 0)
						{
							...
							else if ((targetAtom->combine() == ld::Atom::combineByNameAndContent)
							         || (targetAtom->combine() == ld::Atom::combineByNameAndReferences))
							{
		...
		template <>
		bool
		Section<arm64>::addRelocFixup(class Parser<arm64> &parser, const macho_relocation_info<P> *reloc)
		{
			bool result = false;
			...
			if (reloc->r_extern())
			{
				...
				if (((sym.n_type() & N_TYPE) == N_SECT) && (((sym.n_type() & N_EXT) == 0) || (symbolName[0] == 'L')
				        || (symbolName[0] == 'l')))
				{
					// use direct reference for local symbols
					parser.findTargetFromAddressAndSectionNum(sym.n_value(), sym.n_sect(), target);
				...
				else if (((sym.n_type() & N_TYPE) == N_SECT) && (src.atom->_objAddress <= sym.n_value())
				         && (sym.n_value() < (src.atom->_objAddress + src.atom->size())))
				{
			...
			switch (reloc->r_type())
			{
				...
				case ARM64_RELOC_BRANCH26:
					...
					else
					{
						...
						if (encodedAddend != 0)
						{
							...
							{
								warning("branch26 instruction at 0x%08X has embedded addend. ARM64_RELOC_ADDEND should be used instead",
								        reloc->r_address());
				...
				case ARM64_RELOC_PAGE21:
					...
					if (encodedAddend != 0)
					{
						if (prefixRelocAddend == 0)
						{
							warning("adrp instruction at 0x%08X has embedded addend. ARM64_RELOC_ADDEND should be used instead",
							        reloc->r_address());
							target.addend = encodedAddend;
						}
						else
				...
				case ARM64_RELOC_PAGEOFF12:
					...
					if (encodedAddend != 0)
					{
						if (prefixRelocAddend == 0)
						{
							warning("pageoff12 instruction at 0x%08X has embedded addend. ARM64_RELOC_ADDEND should be used instead",
							        reloc->r_address());
							target.addend = encodedAddend;
						}
						else
		...
		template <>
		bool
		Section<arm64_32>::addRelocFixup(class Parser<arm64_32> &parser, const macho_relocation_info<P> *reloc)
		{
			bool result = false;
		...
		template <typename A>
		bool
		ObjC1ClassSection<A>::addRelocFixup(class Parser<A> &parser, const macho_relocation_info<P> *reloc)
		{
			// inherited
		...
		template <>
		bool
		ObjC1ClassSection<x86>::addRelocFixup(class Parser<x86> &parser, const macho_relocation_info<x86::P> *reloc)
		{
		...
		template <typename A>
		bool
		Objc1ClassReferences<A>::addRelocFixup(class Parser<A> &parser, const macho_relocation_info<P> *reloc)
		{
			// inherited
		...
		template <>
		bool
		Objc1ClassReferences<x86>::addRelocFixup(class Parser<x86> &parser, const macho_relocation_info<x86::P> *reloc)
		{
		...
		template <>
		void
		Section<arm64>::addLOH(class Parser<arm64> &parser, int kind, int count, const uint64_t addrs[])
		{
			switch (kind)
			{
		...
		template <>
		void
		Section<arm64_32>::addLOH(class Parser<arm64_32> &parser, int kind, int count, const uint64_t addrs[])
		{
			switch (kind)
			{
				case LOH_ARM64_ADRP_ADRP:
		...
		template <typename A>
		void
		Section<A>::addLOH(class Parser<A> &parser, int kind, int count, const uint64_t addrs[])
		{
		}

		template <typename A>
		void
		Section<A>::makeFixups(class Parser<A> &parser, const struct Parser<A>::CFI_CU_InfoArrays &)
		{
			const macho_section<P> *sect = this->machoSection();
			...
			else if (this->type() == ld::Section::typeCode)
			{
				...
				for (Atom<A> *p = _beginAtoms; p < end; ++p)
				{
					typename Parser<A>::SourceLocation src(p, 0);
					Atom<A> *nextAtom = &p[1];
					if ((p->symbolTableInclusion() == ld::Atom::symbolTableIn)
					        && (nextAtom->symbolTableInclusion() == ld::Atom::symbolTableNotIn))
					{
						parser.addFixup(src, ld::Fixup::k1of1, ld::Fixup::kindNoneFollowOn, nextAtom);
					}
				}
		...
		//
		bool
		isObjectFile(const uint8_t *fileContent, uint64_t fileLength, const ParserOptions &opts)
		{
			switch (opts.architecture)
			{
				case CPU_TYPE_X86_64:
		...
		//
		bool
		isObjectFile(const uint8_t *fileContent, uint64_t fileLength, cpu_type_t *result, cpu_subtype_t *subResult,
		             ld::VersionSet &platformsFound)
		{
			if (mach_o::relocatable::Parser<x86_64>::validFile(fileContent))
			{
		...
		//
		const char *
		archName(const uint8_t *fileContent)
		{
			if (mach_o::relocatable::Parser<x86_64>::validFile(fileContent))
			{
		...
		//
		bool
		getNonLocalSymbols(const uint8_t *fileContent, std::vector<const char *> &syms)
		{
			if (mach_o::relocatable::Parser<x86_64>::validFile(fileContent))
			{
