...
namespace textstub
{
	namespace dylib
	{
		...
		template <> bool
		File<x86>::useSimulatorVariant()
		{
			return true;
		}
		template <> bool
		File<x86_64>::useSimulatorVariant()
		{
			return true;
		}
		template <typename A> bool
		File<A>::useSimulatorVariant()
		{
			return false;
		}


		static ld::VersionSet
		mapPlatform(tapi::Platform platform, bool useSimulatorVariant)
		{
...
#error "unsupported libtapi API version"
			...
			init(_interface, opts, buildingForSimulator, indirectDylib, linkingFlatNamespace,
			     linkingMainExecutable, path, platforms, targetInstallPath, usingBitcode, internalSDK, fromSDK,
			     platformMismatchesAreWarning);
		}
		...
		template<typename A>
		void
		File<A>::init(tapi::LinkerInterfaceFile *file, const Options *opts, bool buildingForSimulator,
		              bool indirectDylib, bool linkingFlatNamespace, bool linkingMainExecutable,
		              ...
		              bool usingBitcode, bool internalSDK, bool fromSDK, bool platformMismatchesAreWarning)
		{
			...
			this->_parentUmbrella = file->getParentFrameworkName().empty() ? nullptr : strdup(
			                            file->getParentFrameworkName().c_str());
			...
			this->_hasPublicInstallName = file->hasAllowableClients() ? false : this->isPublicLocation(
			                                  file->getInstallName().c_str());
		...
		template <typename A>
		void
		File<A>::buildExportHashTable(const tapi::LinkerInterfaceFile *file)
		{
		...
		template <typename A>
		void
		File<A>::processIndirectLibraries(ld::dylib::File::DylibHandler *handler, bool addImplicitDylibs)
		{
			if (_interface)
			{
		...
		class Parser
		{
			public:
				...
				parse(const char *path, const uint8_t *fileContent,
				...
				static ld::dylib::File	*
				parse(const char *path, tapi::LinkerInterfaceFile *file, time_t mTime,
				      ld::File::Ordinal ordinal, const Options &opts,
				      bool indirectDylib, bool fromSDK, cpu_type_t architecture, cpu_subtype_t subArchitecture)
				{
		                    ...
		                    cpu_type_t architecture, cpu_subtype_t subArchitecture)
		{
			switch (architecture)
			{
				...
				case CPU_TYPE_X86_64:
					return Parser<x86_64>::parse(path, fileContent, fileLength, modTime, ordinal, opts, indirectDylib, fromSDK,
					                             architecture, subArchitecture);
#endif
#if SUPPORT_ARCH_i386
				case CPU_TYPE_I386:
					return Parser<x86>::parse(path, fileContent, fileLength, modTime, ordinal, opts, indirectDylib, fromSDK, architecture,
					                          subArchitecture);
#endif
#if SUPPORT_ARCH_arm_any
				case CPU_TYPE_ARM:
					return Parser<arm>::parse(path, fileContent, fileLength, modTime, ordinal, opts, indirectDylib, fromSDK, architecture,
					                          subArchitecture);
#endif
#if SUPPORT_ARCH_arm64
				case CPU_TYPE_ARM64:
					return Parser<arm64>::parse(path, fileContent, fileLength, modTime, ordinal, opts, indirectDylib, fromSDK, architecture,
					                            subArchitecture);
#endif
#if SUPPORT_ARCH_arm64_32
				case CPU_TYPE_ARM64_32:
					return Parser<arm64_32>::parse(path, fileContent, fileLength, modTime, ordinal, opts, indirectDylib, fromSDK,
					                               architecture, subArchitecture);
#endif
				default:
		...
		static ld::dylib::File *
		parseAsArchitecture(const char *path, tapi::LinkerInterfaceFile *file, time_t modTime,
...
#if SUPPORT_ARCH_arm64_32
				case CPU_TYPE_ARM64_32:
					return Parser<arm64_32>::parse(path, file, modTime, ordinal, opts, indirectDylib, fromSDK, architecture,
					                               subArchitecture);
#endif
				default:
					throwf("unsupported architecture for tbd file");
		      ...
		      bool bundleLoader, bool indirectDylib, bool fromSDK)
		{
			...
			try
			{
				return parseAsArchitecture(fileContent, fileLength, path, modtime, ordinal, opts, bundleLoader, indirectDylib, fromSDK,
		...
		ld::dylib::File *
		parse(const char *path, tapi::LinkerInterfaceFile *file, time_t modTime,
		      ld::File::Ordinal ordinal, const Options &opts, bool indirectDylib, bool fromSDK)
		{
			try
			{
				return parseAsArchitecture(path, file, modTime, ordinal, opts, indirectDylib, fromSDK, opts.architecture(),
			...
			warning("architecture %s not present in TBD %s, attempting fallback", opts.architectureName(), path);
			return parseAsArchitecture(path, file, modTime, ordinal, opts, indirectDylib, fromSDK, opts.fallbackArchitecture(),
			                           opts.fallbackSubArchitecture());
		}
