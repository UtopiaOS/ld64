...
namespace archive
{
	...
	class Parser
	{
		public:
			...
			static bool
			validFile(const uint8_t *fileContent, uint64_t fileLength,
	...
	class File : public ld::archive::File
	{
		public:
			...
			subFileCount() const
			{
				return _archiveFilelength / sizeof(ar_hdr);
			}
		...
		private:
			friend bool isArchiveFile(const uint8_t *fileContent, uint64_t fileLength, ld::Platform *platform,
	...
	template <typename A>
	bool
	File<A>::Entry::hasLongName() const
	{
	...
	template <typename A>
	unsigned int
	File<A>::Entry::getLongNameSpace() const
	{
		char *endptr;
	...
	template <typename A>
	void
	File<A>::Entry::getName(char *buf, int bufsz) const
	{
		if (this->hasLongName())
		{
	...
	template <typename A>
	time_t
	File<A>::Entry::modificationTime() const
	{
		char temp[14];
	...
	template <typename A>
	uint32_t
	File<A>::Entry::contentSize() const
	{
		char temp[12];
	...
	template <typename A>
	const class File<A>::Entry *
		File<A>::Entry::next() const
	{
	...
	template <> cpu_type_t
	File<x86>::architecture()
	{
		return CPU_TYPE_I386;
	}
	template <> cpu_type_t
	File<x86_64>::architecture()
	{
	...
	template <typename A>
	bool
	File<A>::validMachOFile(const uint8_t *fileContent, uint64_t fileLength, const mach_o::relocatable::ParserOptions &opts)
	{
	...
	template <typename A>
	bool
	File<A>::validLTOFile(const uint8_t *fileContent, uint64_t fileLength, const mach_o::relocatable::ParserOptions &opts)
	{
		if (fileLength < 32)
		{
			return false;
	...
	template <typename A>
	bool
	File<A>::validFile(const uint8_t *fileContent, uint64_t fileLength, const mach_o::relocatable::ParserOptions &opts)
	{
	...
	template <>
	bool
	File<x86>::memberHasObjCCategories(const Entry *member) const
	{
		if (_objc2ABI)
		{
	...
	template <typename A>
	bool
	File<A>::memberHasObjCCategories(const Entry *member) const
	{
	...
	template <typename A>
	typename File<A>::MemberState &
	File<A>::makeObjectFileForMember(const Entry *member) const
	{
	...
	template <typename A>
	bool
	File<A>::loadMember(MemberState &state, ld::File::AtomHandler &handler, const char *format, ...) const
	{
	...
	template <typename A>
	bool
	File<A>::justInTimeforEachAtom(const char *name, ld::File::AtomHandler &handler) const
	{
	...
	class CheckIsDataSymbolHandler : public ld::File::AtomHandler
	{
		public:
			...
			virtual void
			doAtom(const class ld::Atom &atom)
			{
	...
	template <typename A>
	bool
	File<A>::justInTimeDataOnlyforEachAtom(const char *name, ld::File::AtomHandler &handler) const
	{
		// in force load case, all members already loaded
		if (_alreadyLoadedAll)
		{
	...
	template <typename A>
	void
	File<A>::dumpTableOfContents()
	{
		for (unsigned int i = 0; i < _tableOfContentCount; ++i)
		{
			const struct ranlib *e = &_tableOfContents[i];
			printf("%s in %s\n", &_tableOfContentStrings[E::get32(e->ran_un.ran_strx)],
			       ((Entry *)&_archiveFileContent[E::get32(e->ran_off)])->name());
		}
	}
	...
	bool
	isArchiveFile(const uint8_t *fileContent, uint64_t fileLength, ld::Platform *platform, const char **archiveArchName)
	{
		if (strncmp((const char *)fileContent, "!<arch>\n", 8) != 0)
		{
			return false;
