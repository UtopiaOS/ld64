...
namespace ld
{
	...
	const Platform
	basePlatform(Platform platform)
	{
		switch (platform)
		{
	...
	static void
	versionToString(uint32_t value, char buffer[32])
	{
		if (value & 0xFF)
		{
	...
	void
	VersionSet::checkObjectCrosslink(const VersionSet &objectPlatforms, const std::string &targetPath, bool internalSDK,
	...
	void
	VersionSet::checkDylibCrosslink(const VersionSet &dylibPlatforms, const std::string &targetPath,
	...
	void
	forEachSupportedPlatform(void (^handler)(const PlatformInfo &info, bool &stop))
	{
		bool stop = false;
		for (const PlatformInfo &info : sAllSupportedPlatforms)
		{
	...
	Platform
	platformForLoadCommand(uint32_t lc, const mach_header *mh)
	{
	...
	Platform
	platformFromBuildVersion(uint32_t plat, const mach_header *mh)
	{
		bool isIntel = ((mh->cputype & ~CPU_ARCH_MASK) == CPU_TYPE_I386);
	...
	Platform
	platformFromName(const char *platformName)
	{
	...
	const char *
	nameFromPlatform(Platform platform)
	{
		for (const PlatformInfo &info : sAllSupportedPlatforms)
		{
			if (info.platform == platform)
