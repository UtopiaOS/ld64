	...
	namespace tool
	{
		...
		class IgnoredFile : public ld::File
		{
			public:
				...
				virtual bool
				forEachAtom(AtomHandler &) const
				{
					return false;
				};
				virtual bool
				justInTimeforEachAtom(const char *name, AtomHandler &) const
				{
					return false;
				};
		};
		...
		DSOHandleAtom DSOHandleAtom::_s_atomExecutable("__mh_execute_header", ld::Atom::scopeGlobal,
		...
		DSOHandleAtom DSOHandleAtom::_s_atomBundle("__mh_bundle_header", ld::Atom::scopeLinkageUnit,
		        ld::Atom::symbolTableNotIn);
		DSOHandleAtom DSOHandleAtom::_s_atomDyld("__mh_dylinker_header", ld::Atom::scopeLinkageUnit,
		        ld::Atom::symbolTableNotIn);
		DSOHandleAtom DSOHandleAtom::_s_atomObjectFile("__mh_object_header", ld::Atom::scopeLinkageUnit,
		        ld::Atom::symbolTableNotIn);
		DSOHandleAtom DSOHandleAtom::_s_atomPreload("__mh_preload_header", ld::Atom::scopeLinkageUnit,
		        ld::Atom::symbolTableNotIn, _s_section_preload);
		DSOHandleAtom DSOHandleAtom::_s_atomPreloadDSO("___dso_handle", ld::Atom::scopeLinkageUnit, ld::Atom::symbolTableNotIn,
		        _s_section_text);
		...
		const char *
		InputFiles::extractFileInfo(const uint8_t *p, unsigned len, const char *path, ld::Platform &platform)
		{
		...
		ld::File *
		InputFiles::makeFile(const Options::FileInfo &info, bool indirectDylib)
		{
			...
			objResult = lto::parse(p, len, info.path, info.modTime, info.ordinal, _options.architecture(),
			                       _options.subArchitecture(), _options.logAllFiles(), _options.verboseOptimizationHints());
			if (objResult != NULL)
			{
				OSAtomicAdd64(len, &_totalObjectSize);
				OSAtomicIncrement32(&_totalObjectLoaded);
			...
			switch (_options.outputKind())
			{
				...
				case Options::kDynamicBundle:
					dylibResult = mach_o::dylib::parse(p, len, info.path, info.modTime, _options, info.ordinal, info.options.fBundleLoader,
					                                   indirectDylib, fromSDK);
					if (dylibResult != NULL)
					{
						return dylibResult;
					}
					dylibResult = textstub::dylib::parse(p, len, info.path, info.modTime, _options, info.ordinal,
					                                     info.options.fBundleLoader, indirectDylib, fromSDK);
					if (dylibResult != NULL)
					{
						return dylibResult;
					}
			...
			if (lto::archName((uint8_t *)p, len) != NULL)
			{
				if (lto::libLTOisLoaded())
				{
					...
					throwf("lto file was built for %s which is not the architecture being linked (%s): %s", fileArchName,
					       _options.architectureName(), info.path);
				}
				else
				{
			...
			else
			{
				if (isFatFile)
				{
					throwf("file is universal (%u slices) but does not contain the %s architecture: %s", sliceCount,
					       _options.architectureName(), info.path);
				}
				else
				{
		...
		void
		InputFiles::logDylib(ld::File *file, bool indirect, bool speculative)
		{
		...
		void
		InputFiles::logArchive(ld::File *file) const
		{
		...
		void
		InputFiles::logTraceInfo(const char *format, ...) const
		{
		...
		ld::dylib::File *
		InputFiles::findDylib(const char *installPath, const ld::dylib::File *fromDylib, bool speculative)
		{
			...
			else
			{
				...
				for (std::vector<Options::DylibOverride>::const_iterator dit = _options.dylibOverrides().begin();
				        dit != _options.dylibOverrides().end(); ++dit)
				{
		...
		void
		InputFiles::markExplicitlyLinkedDylibs()
		{
		...
		bool
		InputFiles::frameworkAlreadyLoaded(const char *path, const char *frameworkName)
		{
		...
		bool
		InputFiles::libraryAlreadyLoaded(const char *path)
		{
			for (ld::File *file : _inputFiles)
			{
				if (strcmp(path, file->path()) == 0)
		...
		void
		InputFiles::addLinkerOptionLibraries(ld::Internal &state, ld::File::AtomHandler &handler)
		{
		...
		void
		InputFiles::createIndirectDylibs()
		{
			...
			while (lastMapSize != _allDylibs.size())
			{
				...
				std::sort(unprocessedDylibs.begin(), unprocessedDylibs.end(), [](const ld::dylib::File * lhs,
				          const ld::dylib::File * rhs)
				{
		...
		void
		InputFiles::createOpaqueFileSections()
		{
		...
		void
		InputFiles::checkDylibClientRestrictions(ld::dylib::File *dylib)
		{
...
#if HAVE_PTHREADS
		void
		InputFiles::startThread(void (*threadFunc)(InputFiles *)) const
		{
			pthread_t thread;
			pthread_attr_t attr;
		...
		InputFiles::parseWorkerThread()
		{
			...
			do
			{
				...
				else
				{
					...
					{
						if (((strstr(msg, "architecture") != NULL)  || (strstr(msg, "attempting to link") != NULL))
						        && !_options.errorOnOtherArchFiles())
						{
		...
		void
		InputFiles::parseWorkerThread(InputFiles *inputFiles)
		{
			inputFiles->parseWorkerThread();
		}
#endif
		...
		ld::File *
		InputFiles::addDylib(ld::dylib::File *reader, const Options::FileInfo &info)
		{
				...
				while (fileMap.size() > 0)
				{
					...
					if (_s_logPThreads)
					{
						printf("pipeline listener: %s slot=%d, _parseCursor=%d, _availableInputFiles = %d remaining = %ld\n", path_buf,
						       inputInfo->inputFileSlot, _parseCursor, _availableInputFiles, fileMap.size() - 1);
					}
					pthread_mutex_unlock(&_parseLock);
					fileMap.erase(it);
				}
		...
		void
		InputFiles::waitForInputFiles(InputFiles *inputFiles)
		{
			inputFiles->waitForInputFiles();
		}
#endif
		...
		void
		InputFiles::forEachInitialAtom(ld::File::AtomHandler &handler, ld::Internal &state)
		{
		...
		bool
		InputFiles::searchLibraries(const char *name, bool searchDylibs, bool searchArchives, bool dataSymbolOnly,
		                            ld::File::AtomHandler &handler) const
		{
		...
		bool
		InputFiles::searchWeakDefInDylib(const char *name) const
		{
		...
		static bool
		vectorContains(const std::vector<ld::dylib::File *> &vec, ld::dylib::File *key)
		{
		...
		{
			bool
			operator()(const ld::dylib::File *left, const ld::dylib::File *right)
			{
		...
		void
		InputFiles::dylibs(ld::Internal &state)
		{
			bool dylibsOK = false;
			switch (_options.outputKind())
			{
			...
			if ((state.dylibs.size() == 0) && _options.needsEntryPointLoadCommand()
			        && !_options.platforms().contains(ld::Platform::driverKit))
			{
		...
		void
		InputFiles::archives(ld::Internal &state)
		{
