...
namespace ld
{
	namespace tool
	{
		...
		size_t
		SymbolTable::ContentFuncs::operator()(const ld::Atom *atom) const
		{
			return atom->contentHash(*_s_indirectBindingTable);
		}

		bool
		SymbolTable::ContentFuncs::operator()(const ld::Atom *left, const ld::Atom *right) const
		{
		...
		size_t
		SymbolTable::CStringHashFuncs::operator()(const ld::Atom *atom) const
		{
			return atom->contentHash(*_s_indirectBindingTable);
		}

		bool
		SymbolTable::CStringHashFuncs::operator()(const ld::Atom *left, const ld::Atom *right) const
		{
		...
		size_t
		SymbolTable::UTF16StringHashFuncs::operator()(const ld::Atom *atom) const
		{
			return atom->contentHash(*_s_indirectBindingTable);
		}

		bool
		SymbolTable::UTF16StringHashFuncs::operator()(const ld::Atom *left, const ld::Atom *right) const
		{
			if (left == right)
			{
				return true;
		...
		size_t
		SymbolTable::ReferencesHashFuncs::operator()(const ld::Atom *atom) const
		{
			return atom->contentHash(*_s_indirectBindingTable);
		}

		bool
		SymbolTable::ReferencesHashFuncs::operator()(const ld::Atom *left, const ld::Atom *right) const
		{
		...
		void
		SymbolTable::addDuplicateSymbolError(const char *name, const ld::Atom *atom)
		{
			addDuplicateSymbol(_duplicateSymbolErrors, name, atom);
		}

		void
		SymbolTable::addDuplicateSymbolWarning(const char *name, const ld::Atom *atom)
		{
			addDuplicateSymbol(_duplicateSymbolWarnings, name, atom);
		}

		void
		SymbolTable::addDuplicateSymbol(DuplicateSymbols &dups, const char *name, const ld::Atom *atom)
		{
		...
		void
		SymbolTable::checkDuplicateSymbols() const
		{
			// print duplicate errors
			bool foundDuplicate = false;
			for (DuplicateSymbols::const_iterator symbolIt = _duplicateSymbolErrors.begin();
			        symbolIt != _duplicateSymbolErrors.end(); symbolIt++)
			{
			...
			for (DuplicateSymbols::const_iterator symbolIt = _duplicateSymbolWarnings.begin();
			        symbolIt != _duplicateSymbolWarnings.end(); symbolIt++)
			{
		...
		class NameCollisionResolution
		{
			public:
				NameCollisionResolution(const ld::Atom &a, const ld::Atom &b, Options::Treatment duplicates,
				                        const Options &options) : _atomA(a), _atomB(b), _options(options), _reportDuplicate(false), _duplicates(duplicates)
				{
					pickAtom();
				}
				...
				{
					return _reportDuplicate && (_duplicates == Options::Treatment::kError);
				}
				bool
				reportDuplicateWarning()
				{
					return _reportDuplicate && (_duplicates == Options::Treatment::kWarning);
				}
			...
			private:
				...
				void
				pickAtom(const ld::Atom &atom)
				{
					_chosen = &atom;    // primitive to set which atom is picked
				}
				void
				pickAtomA()
				{
					pickAtom(_atomA);    // primitive to pick atom A
				}
				void
				pickAtomB()
				{
					pickAtom(_atomB);    // primitive to pick atom B
				}
				...
				void
				pickCommonsMode(const ld::Atom &dylib, const ld::Atom &proxy)
				{
		...
		bool
		SymbolTable::addByName(const ld::Atom &newAtom, Options::Treatment duplicates)
		{
			bool useNew = true;
		...
		bool
		SymbolTable::addByContent(const ld::Atom &newAtom)
		{
			bool useNew = true;
		...
		bool
		SymbolTable::addByReferences(const ld::Atom &newAtom)
		{
			bool useNew = true;
			const ld::Atom *existingAtom;
		...
		bool
		SymbolTable::add(const ld::Atom &atom, Options::Treatment duplicates)
		{
		...
		void
		SymbolTable::markCoalescedAway(const ld::Atom *atom)
		{
		...
		{
			bool
			operator()(const char *i, const char *j)
			{
				if (i == NULL)
				{
					return true;
		...
		void
		SymbolTable::undefines(std::vector<const char *> &undefs)
		{
		...
		void
		SymbolTable::tentativeDefs(std::vector<const char *> &tents)
		{
			// return all names in _byNameTable that have no associated atom
			for (NameToSlot::iterator it = _byNameTable.begin(); it != _byNameTable.end(); ++it)
			{
		...
		void
		SymbolTable::mustPreserveForBitcode(std::unordered_set<const char *> &syms)
		{
			// return all names in _byNameTable that have no associated atom
		...
		bool
		SymbolTable::hasName(const char *name)
		{
		...
		// find existing or create new slot
		SymbolTable::IndirectBindingSlot
		SymbolTable::findSlotForContent(const ld::Atom *atom, const ld::Atom **existingAtom)
		{
		...
		// find existing or create new slot
		SymbolTable::IndirectBindingSlot
		SymbolTable::findSlotForReferences(const ld::Atom *atom, const ld::Atom **existingAtom)
		{
		...
		const char	*
		SymbolTable::indirectName(IndirectBindingSlot slot) const
		{
		...
		const ld::Atom *
		SymbolTable::indirectAtom(IndirectBindingSlot slot) const
		{
			assert(slot < _indirectBindingTable.size());
		...
		void
		SymbolTable::removeDeadUndefs(std::vector<const ld::Atom *> &allAtoms, const std::unordered_set<const ld::Atom *> &keep)
		{
