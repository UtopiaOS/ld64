		...
		namespace stubs
		{
			...
			class Pass
			{
				public:
					...
					void
					addAtom(const ld::Atom &atom)
					{
						_internal->addAtom(atom);
					}
					bool
					usingCompressedLINKEDIT() const
					{
						return _compressedLINKEDIT;
					}
					ld::Internal				*
					internal()
					{
...
#include "stub_arm64_32.hpp"
			...
			const ld::Atom *
			Pass::stubableFixup(const ld::Fixup *fixup, ld::Internal &state)
			{
			...
			ld::Atom *
			Pass::makeStub(const ld::Atom &target, bool weakImport)
			{
				...
				if (usingCompressedLINKEDIT() && !forLazyDylib && !_options.noLazyBinding())
				{
					...
					{
						throwf("symbol dyld_stub_binder not found (normally in libSystem.dylib).  Needed to perform lazy binding to function %s",
						       target.name());
					}
				}
				switch (_architecture)
				{
					...
					case CPU_TYPE_ARM:
						...
						else if (_options.makeChainedFixups())
						{
							...
							{
								return new ld::passes::stubs::arm::NonLazyStubPICAtom(*this, target, stubToGlobalWeakDef, stubToResolver, weakImport,
								        usingDataConst);
							}
						}
						else if (usingCompressedLINKEDIT() && !forLazyDylib)
					...
					case CPU_TYPE_ARM64:
						...
						if ((_options.subArchitecture() == CPU_SUBTYPE_ARM64E) && _options.useAuthenticatedStubs())
						{
							...
							{
								return new ld::passes::stubs::arm64e::StubAtom(*this, target, stubToGlobalWeakDef, stubToResolver, weakImport,
								        usingDataConst);
							}
							else
							{
			...
			void
			Pass::verifyNoResolverFunctions(ld::Internal &state)
			{
			...
			void
			Pass::process(ld::Internal &state)
			{
				...
				if (!_options.makeCompressedDyldInfo() && !_options.makeThreadedStartsSection() && !_options.makeChainedFixups()
				        && (state.classicBindingHelper == NULL) && (_options.outputKind() != Options::kKextBundle))
				{
