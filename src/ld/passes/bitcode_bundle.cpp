	...
	namespace passes
	{
		namespace bitcode_bundle
		{
			...
			class FileHandler
			{
				...
				public:
					virtual void
					populateMustPreserveSymbols(BitcodeObfuscator *_obfuscator)    { }
					virtual void obfuscateAndWriteToPath(BitcodeObfuscator *_obfuscator, const char *path);
					virtual const char *
					compressionMethod()
					{
						return XAR_OPT_VAL_NONE;    // no compression by default
					}
					xar_file_t
					getXARFile()
					{
			...
			static void
			ltoDiagnosticHandler(lto_codegen_diagnostic_severity_t severity, const char *message, void *)
			{
				switch (severity)
				{
			...
			void
			BitcodeObfuscator::addMustPreserveSymbols(const char *name)
			{
				::lto_codegen_add_must_preserve_symbol(_obfuscator, name);
			}

			void
			BitcodeObfuscator::bitcodeHideSymbols(ld::Bitcode *bc, const char *filePath, const char *outputPath)
			{
			}

			void
			BitcodeObfuscator::writeSymbolMap(const char *outputPath)
			{
				(*_lto_write_reverse_map)(_obfuscator, outputPath);
			}

			const char *
			BitcodeObfuscator::lookupHiddenName(const char *symbol)
			{
				return (*_lto_lookup_hidden_name)(_obfuscator, symbol);
			}

			void
			BitcodeObfuscator::addAsmSymbolsToMustPreserve(lto_module_t module)
			{
			...
			void
			BundleHandler::copyXARProp(xar_file_t src, xar_file_t dst)
			{
			...
			void
			BundleHandler::populateMustPreserveSymbols(BitcodeObfuscator *obfuscator)
			{
				// init the handler
				if (_xar == NULL)
				{
			...
			void
			BitcodeHandler::populateMustPreserveSymbols(BitcodeObfuscator *obfuscator)
			{
				initFile();
			...
			void
			ObjectHandler::populateMustPreserveSymbols(BitcodeObfuscator *obfuscator)
			{
				initFile();
			...
			void
			BundleHandler::obfuscateAndWriteToPath(BitcodeObfuscator *obfuscator, const char *path)
			{
				// init the handler
				if (_xar == NULL)
				{
			...
			void
			BitcodeHandler::obfuscateAndWriteToPath(BitcodeObfuscator *obfuscator, const char *path)
			{
				initFile();
			...
			void
			SymbolListHandler::obfuscateAndWriteToPath(BitcodeObfuscator *obfuscator, const char *path)
			{
				initFile();
			...
			void
			FileHandler::obfuscateAndWriteToPath(BitcodeObfuscator *obfuscator, const char *path)
			{
				initFile();
				int f = ::open(path, O_WRONLY | O_CREAT, S_IRUSR | S_IWUSR);
			...
			BitcodeBundle::doPass()
			{
				...
				for (auto &obj : _state.filesWithBitcode)
				{
					...
					if (ld::LLVMBitcode *llvmbc = dynamic_cast<ld::LLVMBitcode *>(obj->getBitcode()))
					{
						...
						{
							bcFile = xar_add_frombuffer(x, NULL, outFilePath, (char *)const_cast<uint8_t *>(llvmbc->getContent()),
							                            llvmbc->getSize());
						}
						if (bcFile == NULL)
						{
				...
				if (_options.orderedSymbolsCount() > 0)
				{
					...
					xar_file_t ordersFile = xar_add_frombuffer(x, NULL, "file.order", const_cast<char *>(orderFile.data()),
					                        orderFile.size());
					if (ordersFile == NULL)
					{
