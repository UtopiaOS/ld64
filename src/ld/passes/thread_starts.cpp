		...
		namespace thread_starts
		{
			...
			class ThreadStartsAtom : public ld::Atom
			{
				public:
					...
					name() const
					{
						return "thread starts";
					}
					virtual uint64_t
					size() const
					{
						return 4 + (_numThreadStarts * 4);
					}
					virtual uint64_t
					objectAddress() const
					{
			...
			class ChainStartsAtom : public ld::Atom
			{
				public:
					...
					size() const
					{
						return offsetof(dyld_chained_starts_offsets, chain_starts[_chainStartsCount]);
					}
					virtual uint64_t
					objectAddress() const
					{
			...
			static uint32_t
			threadStartsCountInSection(std::vector<uint64_t> &fixupAddressesInSection)
			{
			...
			static uint32_t
			processSections(ld::Internal &state, uint64_t minAlignment)
			{
				uint32_t numThreadStarts = 0;
			...
			static uint32_t
			countChains(ld::Internal &state, uint32_t pointerFormat)
			{
				uint32_t count = 0;
