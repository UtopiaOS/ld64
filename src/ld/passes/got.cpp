...
namespace ld
{
	namespace passes
	{
		namespace got
		{
				...
				public:
					GOTEntryAtom(ld::Internal &internal, const ld::Atom *target, bool weakImport, bool weakDef, bool is64)
						: ld::Atom(weakDef ? _s_sectionWeak : _s_section, ld::Atom::definitionRegular, ld::Atom::combineNever,
						  ...
						  _is64(is64)
					{
						_fixup.weakImport = weakImport;
						internal.addAtom(*this);
					}
				...
				public:
					GOTAuthEntryAtom(ld::Internal &internal, const ld::Atom *target, bool weakImport, bool weakDef)
						: ld::Atom(weakDef ? _s_sectionWeak : _s_section, ld::Atom::definitionRegular, ld::Atom::combineNever,
						  ...
						  _fixup1(0, ld::Fixup::k1of2, ld::Fixup::kindSetAuthData, (ld::Fixup::AuthData)
					{
			...
			static bool
			gotFixup(const Options &opts, ld::Internal &internal, const ld::Atom *targetOfGOT, const ld::Atom *fixupAtom,
					...
					case ld::Fixup::kindStoreTargetAddressARM64GOTLoadPageOff12:
						...
						*optimizable = true;
						...
						if (targetOfGOT->scope() == ld::Atom::scopeGlobal)
						{
							...
							if (((targetOfGOT->definition() == ld::Atom::definitionRegular)
							        || (targetOfGOT->definition() == ld::Atom::definitionProxy)) && (targetOfGOT->combine() == ld::Atom::combineByName))
							{
			...
			struct GotMapEntry
			{
				...
				bool
				operator<(const GotMapEntry &other) const
				{
					if (atom != other.atom)
					{
			...
			doPass(const Options &opts, ld::Internal &internal)
			{
				...
				for (ld::Internal::FinalSection *sect : internal.sections)
				{
					...
					for (const ld::Atom *atom : sect->atoms)
					{
						...
						if (target != NULL)
						{
							if (log)
							{
								fprintf(stderr, "found existing got entry to %s\n", target->name());
							}
				...
				for (std::vector<ld::Internal::FinalSection *>::iterator sit = internal.sections.begin();
				        sit != internal.sections.end(); ++sit)
				{
					...
					for (std::vector<const ld::Atom *>::iterator ait = sect->atoms.begin();  ait != sect->atoms.end(); ++ait)
					{
						...
						for (ld::Fixup::iterator fit = atom->fixupsBegin(), end = atom->fixupsEnd(); fit != end; ++fit)
						{
							...
							if (optimizable)
							{
								...
								if (log)
								{
									fprintf(stderr, "optimized GOT usage in %s to %s\n", atom->name(), targetOfGOT->name());
								}
								switch (fit->binding)
								{
									case ld::Fixup::bindingsIndirectlyBound:
							...
							else
							{
								...
								if (log)
								{
									fprintf(stderr, "found GOT use in %s\n", atom->name());
								}
								if (!atomUsesGOT)
								{
				...
				for (auto &entry : gotMap)
				{
					if (entry.second == NULL)
					{
						...
						if (entry.first.isPersonalityFn && (opts.supportsAuthenticatedPointers()))
						{
							...
							if (log)
							{
								fprintf(stderr, "making new GOT slot for %s, gotMap[%p] = %p\n", entry.first.atom->name(), entry.first.atom,
								        entry.second);
							}
							continue;
						}
						...
						if (!optimizable)
						{
							...
							switch (fitThatSetTarget->binding)
							{
								...
								case ld::Fixup::bindingDirectlyBound:
									if (log)
									{
										fprintf(stderr, "updating GOT use in %s to %s\n", atom->name(), targetOfGOT->name());
									}
