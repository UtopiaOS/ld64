	...
	namespace passes
	{
		namespace huge
		{
			...
			doPass(const Options &opts, ld::Internal &state)
			{
				        ...
				        ++sit)
				{
					...
					for (std::vector<const ld::Atom *>::iterator ait = sect->atoms.begin(); ait != sect->atoms.end(); ++ait)
					{
						...
						if ((address > 0x7FFFFFFFLL) && !sect->isSectionHidden())
						{
							...
							if (log)
							{
								fprintf(stderr, "atom: %s is >2GB (0x%09llX), so enabling huge mode\n", atom->name(), address);
							}
							break;
						}
						address += atom->size();
				...
				ld::Internal::FinalSection *hugeSection = state.getFinalSection(*new ld::Section("__DATA", "__huge",
					...
					if (sect->type() == ld::Section::typeZeroFill)
					{
						...
						for (std::vector<const ld::Atom *>::iterator ait = sect->atoms.begin(); ait != sect->atoms.end(); ++ait)
						{
							...
							if (atom->size() > 1024 * 1024)
							{
								...
								if (log)
								{
									fprintf(stderr, "moved to __huge: %s, size=%llu\n", atom->name(), atom->size());
								}
