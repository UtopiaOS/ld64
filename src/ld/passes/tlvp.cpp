...
namespace ld
{
	namespace passes
	{
		namespace tlvp
		{
				...
				public:
					TLVEntryAtom(ld::Internal &internal, const ld::Atom *target, bool weakImport, unsigned ptrSize)
						: ld::Atom(_s_section, ld::Atom::definitionRegular, ld::Atom::combineNever,
						  ...
						  _fixup(0, ld::Fixup::k1of1, (ptrSize == 8) ? ld::Fixup::kindStoreTargetAddressLittleEndian64 :
			...
			static bool
			optimizable(const Options &opts, const ld::Atom *targetOfTLV)
			{
			...
			doPass(const Options &opts, ld::Internal &internal)
			{
					...
					for (std::vector<const ld::Atom *>::iterator ait = sect->atoms.begin(); ait != sect->atoms.end(); ++ait)
					{
						...
						for (ld::Fixup::iterator fit = atom->fixupsBegin(), end = atom->fixupsEnd(); fit != end; ++fit)
						{
							...
							if (fit->lastInCluster() && (ref.fixupWithTLVStore != NULL))
							{
								...
								if (log)
									fprintf(stderr, "found reference to TLV at %s+0x%X to %s\n",
				...
				for (std::map<const ld::Atom *, bool>::iterator it = weakImportMap.begin(); it != weakImportMap.end(); ++it)
				{
					...
					if (pos == variableToPointerMap.end())
					{
						if (log)
						{
							fprintf(stderr, "make TLV pointer for %s\n", it->first->name());
						}
				...
				for (std::vector<TlVReferenceCluster>::iterator it = references.begin(); it != references.end(); ++it)
				{
					if (it->optimizable)
					{
						...
						if (log)
						{
							fprintf(stderr, "optimizing load of TLV to %s into an LEA\n", it->targetOfTLV->name());
						}
					...
					else
					{
						...
						if (log)
						{
							fprintf(stderr, "updating load of TLV to %s to load from TLV pointer\n", it->targetOfTLV->name());
						}
