...
typedef uint8_t *(*createdof_func_t)(cpu_type_t, unsigned int, const char *[], unsigned int, const char *[],
                                     const char *[], uint64_t offsetsInDOF[], size_t *size);
		...
		namespace dtrace
		{
			...
			class File : public ld::File
			{
				public:
					...
					virtual bool
					forEachAtom(AtomHandler &h) const
					{
						h.doAtom(_atom);
						return true;
					}
					...
					void
					reserveFixups(unsigned int count)
					{
						_atom._fixups.reserve(count);
					}
					void
					addSectionFixup(const ld::Fixup &f)
					{
						_atom._fixups.push_back(f);
					}
					ld::Atom					&
					atom()
					{
			...
			doPass(const Options &opts, ld::Internal &internal)
			{
				...
				for (ProviderToProbes::iterator pit = providerToProbes.begin(); pit != providerToProbes.end(); ++pit, ++dofIndex)
				{
					...
					uint8_t *p = (*pCreateDOF)(opts.architecture(), typeCount, typeNames, probeCount, probeNames, funtionNames,
					                           offsetsInDOF, &dofSectionSize);
					if (p != NULL)
					{
