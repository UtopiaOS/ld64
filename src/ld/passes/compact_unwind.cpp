		...
		namespace compact_unwind
		{
			...
			class UnwindInfoAtom : public ld::Atom
			{
				public:
					...
					name() const
					{
						return "compact unwind info";
					}
					virtual uint64_t
					size() const
					{
						return _headerSize + _pagesSize;
					}
					virtual uint64_t
					objectAddress() const
					{
				  ...
				  _pagesForDelete(NULL), _pageAlignedPages(NULL), _pages(NULL), _pagesSize(0), _header(NULL), _headerSize(0)
			{
				...
				const unsigned int entriesPerRegularPage = (4096 - sizeof(unwind_info_regular_second_level_page_header)) / sizeof(
				...
				for (std::map<const ld::Atom *, unsigned int>::iterator it = personalityIndexMap.begin();
				        it != personalityIndexMap.end(); ++it)
				{
				...
				macho_unwind_info_section_header_index_entry<P> *indexTable = (macho_unwind_info_section_header_index_entry<P> *)
				        &_header[indexSectionOffset];
				uint32_t refOffset;
				for (unsigned int i = 0; i < secondLevelPageCount; ++i)
				{
					...
					indexTable[i].set_lsdaIndexArraySectionOffset(lsdaIndexOffsetMap[secondLevelFirstFuncs[reverseIndex]] +
			...
			template <typename A>
			void
			UnwindInfoAtom<A>::copyRawContent(uint8_t buffer[]) const
			{
			...
			template <>
			bool
			UnwindInfoAtom<x86>::encodingMeansUseDwarf(compact_unwind_encoding_t enc)
			{
			...
			template <>
			bool
			UnwindInfoAtom<x86_64>::encodingMeansUseDwarf(compact_unwind_encoding_t enc)
			{
			...
			template <>
			bool
			UnwindInfoAtom<arm64>::encodingMeansUseDwarf(compact_unwind_encoding_t enc)
			{
			...
			template <>
			bool
			UnwindInfoAtom<arm64_32>::encodingMeansUseDwarf(compact_unwind_encoding_t enc)
			{
				return ((enc & UNWIND_ARM64_MODE_MASK) == UNWIND_ARM64_MODE_DWARF);
			}
#endif
			...
			template <>
			bool
			UnwindInfoAtom<arm>::encodingMeansUseDwarf(compact_unwind_encoding_t enc)
			{
			...
			template <>
			bool
			UnwindInfoAtom<x86>::encodingCannotBeMerged(compact_unwind_encoding_t enc)
			{
			...
			template <>
			bool
			UnwindInfoAtom<x86_64>::encodingCannotBeMerged(compact_unwind_encoding_t enc)
			{
			...
			template <typename A>
			bool
			UnwindInfoAtom<A>::encodingCannotBeMerged(compact_unwind_encoding_t enc)
			{
				return false;
			}
			...
			template <typename A>
			void
			UnwindInfoAtom<A>::compressDuplicates(const std::vector<UnwindEntry> &entries, std::vector<UnwindEntry> &uniqueEntries)
			{
				...
				if (_s_log)
					fprintf(stderr, "compressDuplicates() entries.size()=%lu, uniqueEntries.size()=%lu\n",
					        entries.size(), uniqueEntries.size());
			}

			template <typename A>
			void
			UnwindInfoAtom<A>::makePersonalityIndexes(std::vector<UnwindEntry> &entries,
			        std::map<const ld::Atom *, uint32_t> &personalityIndexMap)
			{
				...
				if (_s_log)
				{
					fprintf(stderr, "makePersonalityIndexes() %lu personality routines used\n", personalityIndexMap.size());
				}
			}
			...
			template <typename A>
			void
			UnwindInfoAtom<A>::findCommonEncoding(const std::vector<UnwindEntry> &entries,
			                                      std::map<compact_unwind_encoding_t, unsigned int> &commonEncodings)
			{
				...
				{
					for (std::map<compact_unwind_encoding_t, unsigned int>::iterator euit = encodingsUsed.begin();
					        euit != encodingsUsed.end(); ++euit)
					{
				...
				if (_s_log)
				{
					fprintf(stderr, "findCommonEncoding() %lu common encodings found\n", commonEncodings.size());
				}
			}
			...
			template <typename A>
			void
			UnwindInfoAtom<A>::makeLsdaIndex(const std::vector<UnwindEntry> &entries, std::vector<LSDAEntry> &lsdaIndex,
			                                 std::map<const ld::Atom *, uint32_t> &lsdaIndexOffsetMap)
			{
				for (std::vector<UnwindEntry>::const_iterator it = entries.begin(); it != entries.end(); ++it)
				{
				...
				if (_s_log)
				{
					fprintf(stderr, "makeLsdaIndex() %lu LSDAs found\n", lsdaIndex.size());
				}
			}
			...
			template <>
			void
			UnwindInfoAtom<x86>::addCompressedAddressOffsetFixup(uint32_t offset, const ld::Atom *func, const ld::Atom *fromFunc)
			{
			...
			template <>
			void
			UnwindInfoAtom<x86_64>::addCompressedAddressOffsetFixup(uint32_t offset, const ld::Atom *func, const ld::Atom *fromFunc)
			{
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k1of3, ld::Fixup::kindSetTargetAddress, func));
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k2of3, ld::Fixup::kindSubtractTargetAddress, fromFunc));
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k3of3, ld::Fixup::kindStoreLittleEndianLow24of32));
			...
			template <>
			void
			UnwindInfoAtom<arm64>::addCompressedAddressOffsetFixup(uint32_t offset, const ld::Atom *func, const ld::Atom *fromFunc)
			{
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k1of3, ld::Fixup::kindSetTargetAddress, func));
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k2of3, ld::Fixup::kindSubtractTargetAddress, fromFunc));
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k3of3, ld::Fixup::kindStoreLittleEndianLow24of32));
			...
			template <>
			void
			UnwindInfoAtom<arm64_32>::addCompressedAddressOffsetFixup(uint32_t offset, const ld::Atom *func,
			        const ld::Atom *fromFunc)
			{
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k1of3, ld::Fixup::kindSetTargetAddress, func));
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k2of3, ld::Fixup::kindSubtractTargetAddress, fromFunc));
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k3of3, ld::Fixup::kindStoreLittleEndianLow24of32));
			...
			template <>
			void
			UnwindInfoAtom<arm>::addCompressedAddressOffsetFixup(uint32_t offset, const ld::Atom *func, const ld::Atom *fromFunc)
			{
				if (fromFunc->isThumb())
				{
			...
			template <>
			void
			UnwindInfoAtom<x86>::addCompressedEncodingFixup(uint32_t offset, const ld::Atom *fde)
			{
			...
			template <>
			void
			UnwindInfoAtom<x86_64>::addCompressedEncodingFixup(uint32_t offset, const ld::Atom *fde)
			{
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k1of2, ld::Fixup::kindSetTargetSectionOffset, fde));
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k2of2, ld::Fixup::kindStoreLittleEndianLow24of32));
			}
			...
			template <>
			void
			UnwindInfoAtom<arm64>::addCompressedEncodingFixup(uint32_t offset, const ld::Atom *fde)
			{
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k1of2, ld::Fixup::kindSetTargetSectionOffset, fde));
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k2of2, ld::Fixup::kindStoreLittleEndianLow24of32));
			}
			...
			template <>
			void
			UnwindInfoAtom<arm64_32>::addCompressedEncodingFixup(uint32_t offset, const ld::Atom *fde)
			{
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k1of2, ld::Fixup::kindSetTargetSectionOffset, fde));
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k2of2, ld::Fixup::kindStoreLittleEndianLow24of32));
			}
			...
			template <>
			void
			UnwindInfoAtom<arm>::addCompressedEncodingFixup(uint32_t offset, const ld::Atom *fde)
			{
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k1of2, ld::Fixup::kindSetTargetSectionOffset, fde));
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k2of2, ld::Fixup::kindStoreLittleEndianLow24of32));
			}
			...
			template <>
			void
			UnwindInfoAtom<x86>::addRegularAddressFixup(uint32_t offset, const ld::Atom *func)
			{
			...
			template <>
			void
			UnwindInfoAtom<x86_64>::addRegularAddressFixup(uint32_t offset, const ld::Atom *func)
			{
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k1of2, ld::Fixup::kindSetTargetImageOffset, func));
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k2of2, ld::Fixup::kindStoreLittleEndian32));
			}
			...
			template <>
			void
			UnwindInfoAtom<arm64>::addRegularAddressFixup(uint32_t offset, const ld::Atom *func)
			{
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k1of2, ld::Fixup::kindSetTargetImageOffset, func));
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k2of2, ld::Fixup::kindStoreLittleEndian32));
			}
			...
			template <>
			void
			UnwindInfoAtom<arm64_32>::addRegularAddressFixup(uint32_t offset, const ld::Atom *func)
			{
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k1of2, ld::Fixup::kindSetTargetImageOffset, func));
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k2of2, ld::Fixup::kindStoreLittleEndian32));
			}
			...
			template <>
			void
			UnwindInfoAtom<arm>::addRegularAddressFixup(uint32_t offset, const ld::Atom *func)
			{
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k1of2, ld::Fixup::kindSetTargetImageOffset, func));
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k2of2, ld::Fixup::kindStoreLittleEndian32));
			}
			...
			template <>
			void
			UnwindInfoAtom<x86>::addRegularFDEOffsetFixup(uint32_t offset, const ld::Atom *fde)
			{
			...
			template <>
			void
			UnwindInfoAtom<x86_64>::addRegularFDEOffsetFixup(uint32_t offset, const ld::Atom *fde)
			{
				_fixups.push_back(ld::Fixup(offset + 4, ld::Fixup::k1of2, ld::Fixup::kindSetTargetSectionOffset, fde));
				_fixups.push_back(ld::Fixup(offset + 4, ld::Fixup::k2of2, ld::Fixup::kindStoreLittleEndianLow24of32));
			}
			...
			template <>
			void
			UnwindInfoAtom<arm64>::addRegularFDEOffsetFixup(uint32_t offset, const ld::Atom *fde)
			{
				_fixups.push_back(ld::Fixup(offset + 4, ld::Fixup::k1of2, ld::Fixup::kindSetTargetSectionOffset, fde));
				_fixups.push_back(ld::Fixup(offset + 4, ld::Fixup::k2of2, ld::Fixup::kindStoreLittleEndianLow24of32));
			}
			...
			template <>
			void
			UnwindInfoAtom<arm64_32>::addRegularFDEOffsetFixup(uint32_t offset, const ld::Atom *fde)
			{
				_fixups.push_back(ld::Fixup(offset + 4, ld::Fixup::k1of2, ld::Fixup::kindSetTargetSectionOffset, fde));
				_fixups.push_back(ld::Fixup(offset + 4, ld::Fixup::k2of2, ld::Fixup::kindStoreLittleEndianLow24of32));
			}
			...
			template <>
			void
			UnwindInfoAtom<arm>::addRegularFDEOffsetFixup(uint32_t offset, const ld::Atom *fde)
			{
				_fixups.push_back(ld::Fixup(offset + 4, ld::Fixup::k1of2, ld::Fixup::kindSetTargetSectionOffset, fde));
				_fixups.push_back(ld::Fixup(offset + 4, ld::Fixup::k2of2, ld::Fixup::kindStoreLittleEndianLow24of32));
			}
			...
			template <>
			void
			UnwindInfoAtom<x86>::addImageOffsetFixup(uint32_t offset, const ld::Atom *targ)
			{
			...
			template <>
			void
			UnwindInfoAtom<x86_64>::addImageOffsetFixup(uint32_t offset, const ld::Atom *targ)
			{
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k1of2, ld::Fixup::kindSetTargetImageOffset, targ));
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k2of2, ld::Fixup::kindStoreLittleEndian32));
			}
			...
			template <>
			void
			UnwindInfoAtom<arm64>::addImageOffsetFixup(uint32_t offset, const ld::Atom *targ)
			{
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k1of2, ld::Fixup::kindSetTargetImageOffset, targ));
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k2of2, ld::Fixup::kindStoreLittleEndian32));
			}
			...
			template <>
			void
			UnwindInfoAtom<arm64_32>::addImageOffsetFixup(uint32_t offset, const ld::Atom *targ)
			{
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k1of2, ld::Fixup::kindSetTargetImageOffset, targ));
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k2of2, ld::Fixup::kindStoreLittleEndian32));
			}
			...
			template <>
			void
			UnwindInfoAtom<arm>::addImageOffsetFixup(uint32_t offset, const ld::Atom *targ)
			{
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k1of2, ld::Fixup::kindSetTargetImageOffset, targ));
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k2of2, ld::Fixup::kindStoreLittleEndian32));
			}
			...
			template <>
			void
			UnwindInfoAtom<x86>::addImageOffsetFixupPlusAddend(uint32_t offset, const ld::Atom *targ, uint32_t addend)
			{
			...
			template <>
			void
			UnwindInfoAtom<x86_64>::addImageOffsetFixupPlusAddend(uint32_t offset, const ld::Atom *targ, uint32_t addend)
			{
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k1of3, ld::Fixup::kindSetTargetImageOffset, targ));
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k2of3, ld::Fixup::kindAddAddend, addend));
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k3of3, ld::Fixup::kindStoreLittleEndian32));
			...
			template <>
			void
			UnwindInfoAtom<arm64>::addImageOffsetFixupPlusAddend(uint32_t offset, const ld::Atom *targ, uint32_t addend)
			{
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k1of3, ld::Fixup::kindSetTargetImageOffset, targ));
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k2of3, ld::Fixup::kindAddAddend, addend));
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k3of3, ld::Fixup::kindStoreLittleEndian32));
			...
			template <>
			void
			UnwindInfoAtom<arm64_32>::addImageOffsetFixupPlusAddend(uint32_t offset, const ld::Atom *targ, uint32_t addend)
			{
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k1of3, ld::Fixup::kindSetTargetImageOffset, targ));
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k2of3, ld::Fixup::kindAddAddend, addend));
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k3of3, ld::Fixup::kindStoreLittleEndian32));
			...
			template <>
			void
			UnwindInfoAtom<arm>::addImageOffsetFixupPlusAddend(uint32_t offset, const ld::Atom *targ, uint32_t addend)
			{
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k1of3, ld::Fixup::kindSetTargetImageOffset, targ));
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k2of3, ld::Fixup::kindAddAddend, addend));
				_fixups.push_back(ld::Fixup(offset, ld::Fixup::k3of3, ld::Fixup::kindStoreLittleEndian32));
			...
			template <typename A>
			unsigned int
			UnwindInfoAtom<A>::makeRegularSecondLevelPage(const std::vector<UnwindEntry> &uniqueInfos, uint32_t pageSize,
			        unsigned int endIndex, uint8_t *&pageEnd)
			{
				const unsigned int maxEntriesPerPage = (pageSize - sizeof(unwind_info_regular_second_level_page_header)) / sizeof(
				        unwind_info_regular_second_level_entry);
				...
				macho_unwind_info_regular_second_level_page_header<P> *page = (macho_unwind_info_regular_second_level_page_header<P> *)
				        pageStart;
				...
				macho_unwind_info_regular_second_level_entry<P> *entryTable = (macho_unwind_info_regular_second_level_entry<P> *)(
				            pageStart + page->entryPageOffset());
			...
			template <typename A>
			unsigned int
			UnwindInfoAtom<A>::makeCompressedSecondLevelPage(const std::vector<UnwindEntry> &uniqueInfos,
			        const std::map<compact_unwind_encoding_t, unsigned int> commonEncodings,
			        uint32_t pageSize, unsigned int endIndex, uint8_t *&pageEnd)
			{
				if (_s_log)
				{
					fprintf(stderr, "makeCompressedSecondLevelPage(pageSize=%u, endIndex=%u)\n", pageSize, endIndex);
				}
				...
				while (canDo && (index >= 0))
				{
					...
					if (pos != commonEncodings.end())
					{
						...
						if (_s_log)
						{
							fprintf(stderr, "makeCompressedSecondLevelPage(): funcIndex=%d, re-use commonEncodings[%d]=0x%08X\n", index,
							        encodingIndex, info.encoding);
						}
					}
					else
					{
						...
						if (ppos != pageSpecificEncodings.end())
						{
							...
							if (_s_log)
							{
								fprintf(stderr, "makeCompressedSecondLevelPage(): funcIndex=%d, re-use pageSpecificEncodings[%d]=0x%08X\n", index,
						...
						else
						{
							...
							if (encodingIndex <= 255)
							{
								...
								if (_s_log)
								{
									fprintf(stderr, "makeCompressedSecondLevelPage(): funcIndex=%d, pageSpecificEncodings[%d]=0x%08X\n", index,
									        encodingIndex, encoding);
								}
							}
							else
							{
								canDo = false; // case 3)
								if (_s_log)
									fprintf(stderr, "end of compressed page with %u entries, %lu custom encodings because too many custom encodings\n",
					...
					if (funcOffsetWithInPage > 0x00FFFF00)
					{
						...
						if (_s_log)
						{
							fprintf(stderr, "can't use compressed page with %u entries because function offset too big\n", entryCount);
						}
					}
					...
					if ((pageSpecificEncodings.size() + entryCount) > space4)
					{
						...
						if (_s_log)
						{
							fprintf(stderr, "end of compressed page with %u entries because full\n", entryCount);
						}
					}
				...
				{
					const int regularEntriesPerPage = (pageSize - sizeof(unwind_info_regular_second_level_page_header)) / sizeof(
					                                      unwind_info_regular_second_level_entry);
				...
				for (std::map<uint32_t, unsigned int>::const_iterator it = pageSpecificEncodings.begin();
				        it != pageSpecificEncodings.end(); ++it)
				{
					E::set32(encodingsArray[it->second - commonEncodings.size()], it->first);
				}
				if (_s_log)
				{
					fprintf(stderr, "compressed page with %u entries, %lu custom encodings\n", entryCount, pageSpecificEncodings.size());
				}
				// update pageEnd;
				pageEnd = pageStart;
			...
			static uint64_t
			calculateEHFrameSize(ld::Internal &state)
			{
				bool allCIEs = true;
				uint64_t size = 0;
			...
			static void
			getAllUnwindInfos(const ld::Internal &state, std::vector<UnwindEntry> &entries)
			{
				uint64_t address = 0;
				for (std::vector<ld::Internal::FinalSection *>::const_iterator sit = state.sections.begin();
			...
			static void
			makeFinalLinkedImageCompactUnwindSection(const Options &opts, ld::Internal &state)
			{
			...
			class CompactUnwindAtom : public ld::Atom
			{
				public:
					...
					size() const
					{
						return sizeof(macho_compact_unwind_entry<P>);
					}
					virtual uint64_t
					objectAddress() const
					{
			...
			template <> ld::Fixup::Kind CompactUnwindAtom<x86>::_s_pointerStoreKind =
			...
			template <> ld::Fixup::Kind CompactUnwindAtom<x86_64>::_s_pointerStoreKind =
			...
			template <> ld::Fixup::Kind CompactUnwindAtom<arm64>::_s_pointerStoreKind =
			    ld::Fixup::kindStoreTargetAddressLittleEndian64;
#endif
#if SUPPORT_ARCH_arm64_32
			template <> ld::Fixup::Kind CompactUnwindAtom<arm64_32>::_s_pointerKind = ld::Fixup::kindStoreLittleEndian32;
			template <> ld::Fixup::Kind CompactUnwindAtom<arm64_32>::_s_pointerStoreKind =
			    ld::Fixup::kindStoreTargetAddressLittleEndian32;
#endif
			template <> ld::Fixup::Kind CompactUnwindAtom<arm>::_s_pointerKind = ld::Fixup::kindStoreLittleEndian32;
			template <> ld::Fixup::Kind CompactUnwindAtom<arm>::_s_pointerStoreKind =
			    ld::Fixup::kindStoreTargetAddressLittleEndian32;
			...
			{
				_fixups.push_back(ld::Fixup(macho_compact_unwind_entry<P>::codeStartFieldOffset(), ld::Fixup::k1of3,
				                            ld::Fixup::kindSetTargetAddress, funcAtom));
				_fixups.push_back(ld::Fixup(macho_compact_unwind_entry<P>::codeStartFieldOffset(), ld::Fixup::k2of3,
				                            ld::Fixup::kindAddAddend, _startOffset));
				...
				for (ld::Fixup::iterator fit = funcAtom->fixupsBegin(), end = funcAtom->fixupsEnd(); fit != end; ++fit)
				{
					switch (fit->kind)
					{
						case ld::Fixup::kindNoneGroupSubordinatePersonality:
							...
							_fixups.push_back(ld::Fixup(macho_compact_unwind_entry<P>::personalityFieldOffset(), ld::Fixup::k1of1,
							                            _s_pointerStoreKind, state.indirectBindingTable[fit->u.bindingIndex]));
							break;
						case ld::Fixup::kindNoneGroupSubordinateLSDA:
							assert(fit->binding == ld::Fixup::bindingDirectlyBound);
							_fixups.push_back(ld::Fixup(macho_compact_unwind_entry<P>::lsdaFieldOffset(), ld::Fixup::k1of1, _s_pointerStoreKind,
							                            fit->u.target));
							break;
						default:
							break;
			...
			template <typename A>
			void
			CompactUnwindAtom<A>::copyRawContent(uint8_t buffer[]) const
			{
			...
			static void
			makeCompactUnwindAtom(const Options &opts, ld::Internal &state, const ld::Atom *atom,
			...
			static void
			makeRelocateableCompactUnwindSection(const Options &opts, ld::Internal &state)
			{
