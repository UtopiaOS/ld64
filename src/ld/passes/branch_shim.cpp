...
namespace ld
{
	namespace passes
	{
		namespace branch_shim
		{
			...
			class Thumb2ToArmShimAtom : public ld::Atom
			{
				public:
					...
					copyRawContent(uint8_t buffer[]) const
					{
						...
						if (_s_log)
						{
							fprintf(stderr, "3 Thumb2 instruction shim to jump to %s\n", _target->name());
						}
			...
			class Thumb1ToArmShimAtom : public ld::Atom
			{
				public:
					...
					copyRawContent(uint8_t buffer[]) const
					{
						...
						if (_s_log)
						{
							fprintf(stderr, "6 Thumb1 instruction shim to jump to %s\n", _target->name());
						}
			...
			class ARMtoThumbShimAtom : public ld::Atom
			{
				public:
					...
					copyRawContent(uint8_t buffer[]) const
					{
						...
						if (_s_log)
						{
							fprintf(stderr, "4 ARM instruction shim to jump to %s\n", _target->name());
						}
			...
			class NoPICARMtoThumbShimAtom : public ld::Atom
			{
				public:
					...
					copyRawContent(uint8_t buffer[]) const
					{
						...
						if (_s_log)
						{
							fprintf(stderr, "3 ARM instruction shim to jump to %s\n", _target->name());
						}
			...
			static void
			extractTarget(ld::Fixup::iterator fixup, ld::Internal &state, const ld::Atom **target)
			{
