	...
	namespace passes
	{
		namespace branch_island
		{
			...
			class TargetAndOffsetComparor
			{
				public:
					bool
					operator()(const TargetAndOffset &left, const TargetAndOffset &right) const
					{
			...
			class ARM64BranchIslandAtom : public ld::Atom
			{
				public:
					...
					{
						if (_s_log)
							fprintf(stderr, "%p: ARM64 branch island to final target %s\n",
					...
					fixupsBegin() const
					{
						return (ld::Fixup *)&_fixup1;
					}
					virtual ld::Fixup::iterator
					fixupsEnd()	const
					{
						return &((ld::Fixup *)&_fixup2)[1];
					}
			...
			class ARMtoARMBranchIslandAtom : public ld::Atom
			{
				public:
					...
					{
						if (_s_log)
							fprintf(stderr, "%p: ARM-to-ARM branch island to final target %s\n",
							        this, finalTarget.atom->name());
					}
			...
			class ARMtoThumb1BranchIslandAtom : public ld::Atom
			{
				public:
					...
					{
						if (_s_log)
							fprintf(stderr, "%p: ARM-to-thumb1 branch island to final target %s\n",
							        this, finalTarget.atom->name());
					}
			...
			class Thumb2toThumbBranchIslandAtom : public ld::Atom
			{
				public:
					...
					{
						if (_s_log)
							fprintf(stderr, "%p: Thumb-to-thumb branch island to final target %s\n",
							        this, finalTarget.atom->name());
					}
			...
			class Thumb2toThumbBranchAbsoluteIslandAtom : public ld::Atom
			{
				public:
					...
					{
						if (_s_log)
							fprintf(stderr, "%p: Thumb-to-thumb absolute branch island to final target %s\n",
							        this, finalTarget.atom->name());
					}
			...
			class NoPicARMtoThumbMBranchIslandAtom : public ld::Atom
			{
				public:
					...
					{
						if (_s_log)
							fprintf(stderr, "%p: NoPIC ARM-to-Thumb branch island to final target %s\n",
							        this, finalTarget.atom->name());
					}
			...
			static ld::Atom *
			makeBranchIsland(const Options &opts, ld::Fixup::Kind kind, int islandRegion, const ld::Atom *nextTarget,
			...
			static uint64_t
			textSizeWhenMightNeedBranchIslands(const Options &opts, bool seenThumbBranch)
			{
			...
			static uint64_t
			maxDistanceBetweenIslands(const Options &opts, bool seenThumbBranch)
			{
				switch (opts.architecture())
				{
					case CPU_TYPE_ARM:
			...
			static void
			makeIslandsForSection(const Options &opts, ld::Internal &state, ld::Internal::FinalSection *textSection,
			                      unsigned stubCount)
			{
				...
				if (_s_log)
				{
					fprintf(stderr, "ld: section %s size=%llu, might need branch islands\n", textSection->sectionName(), totalTextSize);
				}
				...
				const uint64_t kBetweenRegions = maxDistanceBetweenIslands(opts,
				                                 hasThumbBranches); // place regions of islands every 14MB in __text section
				std::vector<const ld::Atom *>
				branchIslandInsertionPoints; // atoms in the atom list after which branch islands will be inserted
				...
				if ((insertionPoint != NULL)
				        && (insertionPoint->sectionOffset() + insertionPoint->size() > previousIslandEndAddr + (kBetweenRegions - 0x100000)))
				{
					branchIslandInsertionPoints.push_back(insertionPoint);
				}
				...
				for (int i = 0; i < kIslandRegionsCount; ++i)
				{
					...
					if (_s_log)
					{
						fprintf(stderr, "ld: branch islands will be inserted at 0x%08llX after %s\n", regionAddresses[i],
						        branchIslandInsertionPoints[i]->name());
					}
				}
						...
						if (haveBranch)
						{
							...
							if (crossSectionBranch && ((displacement > kBranchLimit) || (displacement < (-kBranchLimit))))
							{
								...
								if (pos == region->end())
								{
									...
									if (_s_log)
										fprintf(stderr, "added absolute branching island %p %s, displacement=%lld\n",
								...
								if (_s_log)
								{
									fprintf(stderr, "using island %p %s for branch to %s from %s\n", island, island->name(), target->name(), atom->name());
								}
							...
							else if (displacement > kBranchLimit)
							{
								...
								if (_s_log)
									fprintf(stderr, "need forward branching island srcAdr=0x%08llX, dstAdr=0x%08llX, target=%s\n",
								...
								for (int i = kIslandRegionsCount - 1; i >= 0 ; --i)
								{
									...
									if ((srcAddr < islandRegionAddr) && ((islandRegionAddr <= dstAddr)))
									{
										...
										if (pos == region->end())
										{
											...
											if (_s_log)
											{
												fprintf(stderr, "added forward branching island %p %s to region %d for %s\n", island, island->name(), i, atom->name());
											}
								...
								if (_s_log)
								{
									fprintf(stderr, "using island %p %s for branch to %s from %s\n", nextTarget, nextTarget->name(), target->name(),
									        atom->name());
								}
							...
							else if (displacement < (-kBranchLimit))
							{
								...
								for (int i = 0; i < kIslandRegionsCount ; ++i)
								{
									...
									if ((dstAddr < islandRegionAddr) && (islandRegionAddr <= srcAddr))
									{
										if (_s_log)
										{
											fprintf(stderr, "need backward branching island srcAdr=0x%08llX, dstAdr=0x%08llX, target=%s\n", srcAddr, dstAddr,
											        target->name());
										}
										AtomToIsland::iterator pos = region->find(finalTargetAndOffset);
										if (pos == region->end())
										{
											...
											if (_s_log)
											{
												fprintf(stderr, "added back branching island %p %s to region %d for %s\n", island, island->name(), i, atom->name());
											}
											regionsIslands[i]->push_back(island);
											state.atomToSection[island] = textSection;
											++islandCount;
								...
								if (_s_log)
								{
									fprintf(stderr, "using back island %p %s for %s\n", prevTarget, prevTarget->name(), atom->name());
								}
				...
				if (islandCount > 0)
				{
					if (_s_log)
					{
						fprintf(stderr, "ld: %u branch islands required in %u regions\n", islandCount, kIslandRegionsCount);
					}
			...
			static void
			buildAddressMap(const Options &opts, ld::Internal &state)
			{
				...
				if (log)
				{
					fprintf(stderr, "buildAddressMap()\n");
				}
				        ...
				        ++sit)
				{
					...
					for (std::vector<const ld::Atom *>::iterator ait = sect->atoms.begin(); ait != sect->atoms.end(); ++ait)
					{
						...
						if (log)
						{
							fprintf(stderr, "    0x%08llX atom=%p, name=%s\n", sect->address + offset, atom, atom->name());
						}
