...
namespace ld
{
	namespace passes
	{
		namespace order
		{
			...
			bool
			Layout::Comparer::operator()(const ld::Atom *left, const ld::Atom *right)
			{
				if (left == right)
				{
					return false;
			...
			bool
			Layout::possibleToOrder(const ld::Internal::FinalSection *sect)
			{
			...
			const ld::Atom *
			Layout::findAtom(const Options::OrderedSymbol &orderedSymbol)
			{
			...
			const ld::Atom *
			Layout::follower(const ld::Atom *atom)
			{
								...
								else
								{
									if (_followOnStarts[followOnAtom] == followOnAtom)
									{
										...
										while (true)
										{
											...
											if (_s_log)
											{
												fprintf(stderr, "  adjust start for %s -> %s\n", a->name(), _followOnStarts[atom]->name());
											}
			...
			void
			Layout::buildOrdinalOverrideMap()
			{
				...
				for (Options::OrderedSymbolsIterator it = _options.orderedSymbolsBegin(); it != _options.orderedSymbolsEnd(); ++it)
				{
					...
					if (atom != NULL)
					{
						...
						if (start != _followOnStarts.end())
						{
							...
							for (const ld::Atom *nextAtom = start->second; nextAtom != NULL; nextAtom = _followOnNexts[nextAtom])
							{
								...
								if (pos == _ordinalOverrideMap.end())
								{
									...
									if (_s_log)
									{
										fprintf(stderr, "override ordinal %u assigned to %s in cluster from %s\n", index, nextAtom->name(),
										        nextAtom->safeFilePath());
									}
								}
								else
								{
									if (_s_log)
										fprintf(stderr, "could not order %s as %u because it was already laid out earlier by %s as %u\n",
						...
						else
						{
							...
							if (_s_log)
							{
								fprintf(stderr, "override ordinal %u assigned to %s from %s\n", index, atom->name(), atom->safeFilePath());
							}
						}
						++matchCount;
					}
