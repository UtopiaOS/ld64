...
#define OBJC_IMAGE_HAS_CATEGORY_CLASS_PROPERTIES	(1<<6)
			...
			template <typename A> ld::Section ObjCImageInfoAtom<A>::_s_sectionABI1("__OBJC", "__image_info",
			        ld::Section::typeUnclassified);
			template <typename A> ld::Section ObjCImageInfoAtom<A>::_s_sectionABI2("__DATA", "__objc_imageinfo",
			        ld::Section::typeUnclassified);
				...
				public:
					SelRefAtom(ld::Internal &state, const ld::Atom *target, bool is64)
						: ld::Atom(_s_section, ld::Atom::definitionRegular, ld::Atom::combineNever,
						           ...
						           ld::Atom::symbolTableInWithRandomAutoStripLabel, false, false, false,
						           (is64 ? ld::Atom::Alignment(3) : ld::Atom::Alignment(2))),
						  _fixup(0, ld::Fixup::k1of1, (is64 ? ld::Fixup::kindStoreTargetAddressLittleEndian64 :
						                               ld::Fixup::kindStoreTargetAddressLittleEndian32), target),
						  _target(target),
						  _is64(is64)
					{
						state.addAtom(*this);
			...
			class CategoryNameAtom : public ld::Atom
			{
				public:
					...
					name() const
					{
						return "objc merged category name";
					}
					virtual uint64_t
					size() const
					{
						return _categoryName.size() + 1;
					}
					virtual uint64_t
					objectAddress() const
					{
			...
			class MethodListAtom : public ld::Atom
			{
				public:
					...
					fixupsBegin() const
					{
						return (ld::Fixup *)&_fixups[0];
					}
					virtual ld::Fixup::iterator
					fixupsEnd()	const
					{
						return (ld::Fixup *)&_fixups[_fixups.size()];
					}
				...
				private:
					...
					void									appendMethod(uint32_t methodIndex, const MethodEntryInfo &method, ld::Internal &state,
			...
			class ProtocolListAtom : public ld::Atom
			{
				public:
					...
					size() const
					{
						return (_protocolCount + 1) * sizeof(pint_t);
					}
					virtual uint64_t
					objectAddress() const
					{
			...
			class PropertyListAtom : public ld::Atom
			{
				public:
					...
					name() const
					{
						return "objc merged property list";
					}
					virtual uint64_t
					size() const
					{
						return _propertyCount * 2 * sizeof(pint_t) + 8;
					}
					virtual uint64_t
					objectAddress() const
					{
			...
			class ObjCOverlayAtom : public ld::Atom
			{
				public:
					...
					contentHash(const class ld::IndirectBindingTable &ibt) const
					{
						return _atom->contentHash(ibt);
					}
					virtual bool
					canCoalesceWith(const ld::Atom &rhs, const class ld::IndirectBindingTable &ibt) const
					{
			...
			{
				public:
					static const ld::Atom	*getPointerInContent(ld::Internal &state, const ld::Atom *contentAtom, unsigned int offset,
					        uint64_t *addend = NULL, bool *isAuthPtr = NULL);
			...
			template <typename A>
			const ld::Atom *
			ObjCData<A>::getPointerInContent(ld::Internal &state, const ld::Atom *contentAtom, unsigned int offset,
			                                 uint64_t *addend, bool *isAuthPtr)
			{
				...
				for (ld::Fixup::iterator fit = contentAtom->fixupsBegin(); fit != contentAtom->fixupsEnd(); ++fit)
				{
					if ((fit->offsetInAtom == offset) && (fit->kind != ld::Fixup::kindNoneFollowOn)
					        && (fit->kind != ld::Fixup::kindNoneGroupSubordinate))
					{
						switch (fit->binding)
						{
			...
			template <typename A>
			void
			ObjCData<A>::setPointerInContent(ld::Internal &state, const ld::Atom *contentAtom,
			                                 unsigned int offset, const ld::Atom *newAtom)
			{
				for (ld::Fixup::iterator fit = contentAtom->fixupsBegin(); fit != contentAtom->fixupsEnd(); ++fit)
				{
...
#define SET_RO_FIELD(state, classROAtom, field, valueAtom) \
	ObjCData<A>::setPointerInContent(state, getROData(state, classAtom), offsetof(ROContent, field), valueAtom)
			...
			class Category : public ObjCData<A>
			{
				public:
					...
					static void				setClassMethods(ld::Internal &state, const ld::Atom *&categoryAtom, const ld::Atom *methodListAtom,
					                                        bool usesAuthPtrs, bool &categoryIsNowOverlay, std::set<const ld::Atom *> &deadAtoms);
					static void 			setInstanceProperties(ld::Internal &state, const ld::Atom *&categoryAtom,
					        const ld::Atom *propertyListAtom,
			...
			class MethodList : public ObjCData<A>
			{
				public:
					static uint32_t
					count(ld::Internal &state, const ld::Atom *methodListAtom)
					{
					...
					static uint32_t
					elementSize(ld::Internal &state, const ld::Atom *methodListAtom)
					{
						const uint32_t *methodListData = (uint32_t *)(methodListAtom->rawContentPointer());
					...
					static bool
					usesRelativeMethodList(ld::Internal &state, const ld::Atom *methodListAtom)
					{
						const uint32_t *methodListData = (uint32_t *)(methodListAtom->rawContentPointer());
			...
			template <typename A>
			const ld::Atom	*
			Category<A>::getName(ld::Internal &state, const ld::Atom *contentAtom)
			{
			...
			template <typename A>
			const ld::Atom	*
			Category<A>::getClass(ld::Internal &state, const ld::Atom *contentAtom, uint64_t &addend)
			{
			...
			template <typename A>
			const ld::Atom	*
			Category<A>::getInstanceMethods(ld::Internal &state, const ld::Atom *contentAtom, bool *isAuthPtr)
			{
			...
			template <typename A>
			const ld::Atom	*
			Category<A>::getClassMethods(ld::Internal &state, const ld::Atom *contentAtom, bool *isAuthPtr)
			{
			...
			template <typename A>
			bool
			Category<A>::usesRelMethodLists(ld::Internal &state, const ld::Atom *categoryAtom)
			{
			...
			template <typename A>
			const ld::Atom	*
			Category<A>::getProtocols(ld::Internal &state, const ld::Atom *contentAtom)
			{
			...
			template <typename A>
			const ld::Atom	*
			Category<A>::getInstanceProperties(ld::Internal &state, const ld::Atom *contentAtom)
			{
			...
			template <typename A>
			const ld::Atom	*
			Category<A>::getClassProperties(ld::Internal &state, const ld::Atom *contentAtom)
			{
			...
			template <typename A>
			bool
			Category<A>::hasCategoryClassPropertiesField(const ld::Atom *contentAtom)
			{
				// Only specially-marked files have this field.
			...
			template <typename A>
			void
			Category<A>::setInstanceMethods(ld::Internal &state, const ld::Atom *&categoryAtom, const ld::Atom *methodListAtom,
			                                bool useAuthPtrs, bool &categoryIsNowOverlay, std::set<const ld::Atom *> &deadAtoms)
			{
			...
			template <typename A>
			void
			Category<A>::setClassMethods(ld::Internal &state, const ld::Atom *&categoryAtom, const ld::Atom *methodListAtom,
			                             bool useAuthPtrs, bool &categoryIsNowOverlay, std::set<const ld::Atom *> &deadAtoms)
			{
				// if the base class does not already have a method list, we need to create an overlay
				bool needAuthPtrToMethodList = useAuthPtrs && (strcmp(methodListAtom->section().sectionName(), "__objc_methlist") == 0);
			...
			template <typename A>
			void
			Category<A>::setProtocols(ld::Internal &state, const ld::Atom *&categoryAtom,
			                          const ld::Atom *protocolListAtom, bool &categoryIsNowOverlay,
			                          std::set<const ld::Atom *> &deadAtoms)
			{
			...
			template <typename A>
			void
			Category<A>::setInstanceProperties(ld::Internal &state, const ld::Atom *&categoryAtom, const ld::Atom *methodListAtom,
			                                   bool &categoryIsNowOverlay, std::set<const ld::Atom *> &deadAtoms)
			{
			...
			template <typename A>
			void
			Category<A>::setClassProperties(ld::Internal &state, const ld::Atom *&categoryAtom, const ld::Atom *methodListAtom,
			                                bool &categoryIsNowOverlay, std::set<const ld::Atom *> &deadAtoms)
			{
				// if the base category does not already have a property list, we need to create an overlay
			...
			class ProtocolList : public ObjCData<A>
			{
				public:
					static uint32_t
					count(ld::Internal &state, const ld::Atom *protocolListAtom)
					{
			...
			template <typename A>
			const ld::Atom	*
			Class<A>::getMetaClass(ld::Internal &state, const ld::Atom *classAtom)
			{
			...
			template <typename A>
			const ld::Atom	*
			Class<A>::getROData(ld::Internal &state, const ld::Atom *classAtom)
			{
			...
			template <typename A>
			const ld::Atom	*
			Class<A>::getName(ld::Internal &state, const ld::Atom *classAtom)
			{
			...
			template <typename A>
			const ld::Atom	*
			Class<A>::getInstanceMethodList(ld::Internal &state, const ld::Atom *classAtom, bool *isAuthPtr)
			{
			...
			template <typename A>
			bool
			Class<A>::usesRelMethodLists(ld::Internal &state, const ld::Atom *classAtom)
			{
			...
			template <typename A>
			const ld::Atom	*
			Class<A>::getInstanceProtocolList(ld::Internal &state, const ld::Atom *classAtom)
			{
			...
			template <typename A>
			const ld::Atom	*
			Class<A>::getInstancePropertyList(ld::Internal &state, const ld::Atom *classAtom)
			{
			...
			template <typename A>
			const ld::Atom	*
			Class<A>::getClassMethodList(ld::Internal &state, const ld::Atom *classAtom)
			{
			...
			template <typename A>
			const ld::Atom	*
			Class<A>::getClassPropertyList(ld::Internal &state, const ld::Atom *classAtom)
			{
			...
			template <typename A>
			void
			Class<A>::setInstanceMethodList(ld::Internal &state, const ld::Atom *classAtom,
			                                const ld::Atom *methodListAtom, bool useAuthPtrs, std::set<const ld::Atom *> &deadAtoms)
			{
				// if the base class does not already have a method list, we need to create an overlay
				bool needAuthPtrToMethodList = useAuthPtrs && (strcmp(methodListAtom->section().sectionName(), "__objc_methlist") == 0);
			...
			template <typename A>
			void
			Class<A>::setInstanceProtocolList(ld::Internal &state, const ld::Atom *classAtom,
			                                  const ld::Atom *protocolListAtom, std::set<const ld::Atom *> &deadAtoms)
			{
			...
			template <typename A>
			void
			Class<A>::setClassProtocolList(ld::Internal &state, const ld::Atom *classAtom,
			                               const ld::Atom *protocolListAtom, std::set<const ld::Atom *> &deadAtoms)
			{
			...
			template <typename A>
			void
			Class<A>::setInstancePropertyList(ld::Internal &state, const ld::Atom *classAtom,
			                                  const ld::Atom *propertyListAtom, std::set<const ld::Atom *> &deadAtoms)
			{
			...
			template <typename A>
			void
			Class<A>::setClassMethodList(ld::Internal &state, const ld::Atom *classAtom,
			                             const ld::Atom *methodListAtom, bool useAuthPtrs, std::set<const ld::Atom *> &deadAtoms)
			{
			...
			template <typename A>
			void
			Class<A>::setClassPropertyList(ld::Internal &state, const ld::Atom *classAtom,
			                               const ld::Atom *propertyListAtom, std::set<const ld::Atom *> &deadAtoms)
			{
...
#undef SET_RO_FIELD
			...
			template <typename A>
			void
			ObjCOverlayAtom<A>::addFixupAtOffset(uint32_t offset, bool isAuthPtr)
			{
				// remove any fixups from original atom at this location
				_fixups.erase(std::remove_if(_fixups.begin(), _fixups.end(), [offset](const ld::Fixup & f)
				{
					return f.offsetInAtom == offset;
				}), _fixups.end());
				if (isAuthPtr)
				{
#if SUPPORT_ARCH_arm64e
			...
			template <typename A>
			void
			ClassROOverlayAtom<A>::addMethodListFixup(bool isAuthPtr)
			{
			...
			template <typename A>
			void
			ClassROOverlayAtom<A>::addProtocolListFixup()
			{
			...
			template <typename A>
			void
			ClassROOverlayAtom<A>::addPropertyListFixup()
			{
			...
			template <typename A>
			void
			CategoryOverlayAtom<A>::addNameFixup()
			{
			...
			template <typename A>
			void
			CategoryOverlayAtom<A>::addInstanceMethodListFixup(bool isAuthPtr)
			{
			...
			template <typename A>
			void
			CategoryOverlayAtom<A>::addClassMethodListFixup(bool isAuthPtr)
			{
			...
			template <typename A>
			void
			CategoryOverlayAtom<A>::addProtocolListFixup()
			{
			...
			template <typename A>
			void
			CategoryOverlayAtom<A>::addInstancePropertyListFixup()
			{
			...
			template <typename A>
			void
			CategoryOverlayAtom<A>::addClassPropertyListFixup()
			{
			...
			template <typename A>
			bool
			OptimizeCategories<A>::hasName(ld::Internal &state, const std::vector<const ld::Atom *> *categories)
			{
			...
			template <typename A>
			bool
			OptimizeCategories<A>::hasInstanceMethods(ld::Internal &state, const std::vector<const ld::Atom *> *categories)
			{
			...
			template <typename A>
			bool
			OptimizeCategories<A>::hasClassMethods(ld::Internal &state, const std::vector<const ld::Atom *> *categories)
			{
				if (categories == nullptr)
				{
					return false;
			...
			template <typename A>
			bool
			OptimizeCategories<A>::hasProtocols(ld::Internal &state, const std::vector<const ld::Atom *> *categories)
			{
				if (categories == nullptr)
				{
					return false;
			...
			template <typename A>
			bool
			OptimizeCategories<A>::hasInstanceProperties(ld::Internal &state, const std::vector<const ld::Atom *> *categories)
			{
				if (categories == nullptr)
				{
					return false;
			...
			template <typename A>
			bool
			OptimizeCategories<A>::hasClassProperties(ld::Internal &state, const std::vector<const ld::Atom *> *categories)
			{
				if (categories == nullptr)
				{
					return false;
			...
			static const ld::Atom *
			getFollowOnAtom(const ld::Atom *atom)
			{
			...
			static const ld::Atom *
			fixClassAliases(const ld::Atom *classAtom, uint64_t &addend)
			{
			...
			{
				bool
				operator()(const Atom *left, const Atom *right)
				{
...
#if 0
			static bool
			compSelRefs(const ld::Atom *l, const ld::Atom *r)
			{
			...
			template <typename A>
			void
			OptimizeCategories<A>::doit(const Options &opts, ld::Internal &state, bool haveCategoriesWithoutClassPropertyStorage)
			{
				std::set<const ld::Atom *> deadAtoms;
					...
					if (sect->type() == ld::Section::typeObjC2CategoryList)
					{
						...
						for (const ld::Atom *catListAtom : sect->atoms)
						{
							...
							if (log)
							{
								...
								if (log)
								{
									fprintf(stderr, "category: %p %s on %s\n", categoryAtom, catName, className);
								}
							}
						}
					}
				...
				if (!classDefAtoms.empty())
				{
					...
					for (const ld::Atom *classAtom : orderedClasses)
					{
						...
						if (log)
						{
							fprintf(stderr, "updating method lists in class %s\n", className);
						}
						...
						if (needToRewriteMethodList || categoriesHaveInstanceMethods)
						{
							...
							{
								const ld::Atom *newInstanceMethodListAtom = new MethodListAtom<A>(state, baseInstanceMethodListAtom, methodListFormat,
								        MethodListAtom<A>::classMethodList,
						...
						if (needToRewriteMethodList || categoriesHaveClassMethods)
						{
							...
							{
								const ld::Atom *newClassMethodListAtom = new MethodListAtom<A>(state, baseClassMethodListAtom, methodListFormat,
								        MethodListAtom<A>::classMethodList,
						...
						if (OptimizeCategories<A>::hasProtocols(state, categories))
						{
							...
							const ProtocolListAtom<A> *newProtocolListAtom = new ProtocolListAtom<A>(state, baseProtocolListAtom, className,
							        categories, deadAtoms);
						...
						if (OptimizeCategories<A>::hasInstanceProperties(state, categories))
						{
							...
							const ld::Atom *newPropertyListAtom = new PropertyListAtom<A>(state, basePropertyListAtom, categories, deadAtoms,
							        PropertyListAtom<A>::PropertyKind::InstanceProperties);
						...
						if (OptimizeCategories<A>::hasClassProperties(state, categories))
						{
							...
							const ld::Atom *newPropertyListAtom = new PropertyListAtom<A>(state, basePropertyListAtom, categories, deadAtoms,
							        PropertyListAtom<A>::PropertyKind::ClassProperties);
				...
				if (!externalClassAtoms.empty())
				{
					...
					for (const ld::Atom *externalClassAtom : externalClassAtoms)
					{
						...
						{
							const ld::Atom *newInstanceMethodListAtom = new MethodListAtom<A>(state, nullptr, methodListFormat,
							        MethodListAtom<A>::categoryMethodList,
							...
							Category<A>::setInstanceMethods(state, categoryAtom, newInstanceMethodListAtom, usesAuthPtrs, categoryIsNowOverlay,
							                                deadAtoms);
						}
						// if category has class methods, replace method list format
						if (OptimizeCategories<A>::hasClassMethods(state, &categories))
						{
							const ld::Atom *newClassMethodListAtom = new MethodListAtom<A>(state, nullptr, methodListFormat,
							        MethodListAtom<A>::categoryMethodList,
							...
							Category<A>::setClassMethods(state, categoryAtom, newClassMethodListAtom, usesAuthPtrs, categoryIsNowOverlay,
							                             deadAtoms);
						}
						// if any category adds protocols, generate new merged protocol list, and replace
						if (OptimizeCategories<A>::hasProtocols(state, &categories))
						{
							const ProtocolListAtom<A> *newProtocolListAtom = new ProtocolListAtom<A>(state, nullptr, onClassName, &categories,
							        deadAtoms);
						...
						if (OptimizeCategories<A>::hasInstanceProperties(state, &categories))
						{
							const ld::Atom *newPropertyListAtom = new PropertyListAtom<A>(state, nullptr, &categories, deadAtoms,
							        PropertyListAtom<A>::PropertyKind::InstanceProperties);
			...
			template <typename A>
			static void
			forEachMethod(ld::Internal &state, const ld::Atom *categoryMethodListAtom,
			              void (^callback)(const MethodEntryInfo &method))
			{
				...
				for (ld::Fixup::iterator fit = categoryMethodListAtom->fixupsBegin(); fit != categoryMethodListAtom->fixupsEnd(); ++fit)
				{
					...
					else
					{
						...
						if ((entryOffset == 0) && (fit->clusterSize == Fixup::k1of1))
						{
							...
							assert((methodNameAtom->contentType() == ld::Atom::typeCString)
							       || (strcmp(methodNameAtom->section().segmentName(), "__TEXT") == 0));
							methods[methodIndex].methodName         = (char *)(methodNameAtom->rawContentPointer());
						...
						else if ((entryOffset == ptrSize) && (fit->clusterSize == Fixup::k1of1))
						{
							...
							assert((methods[methodIndex].typeAtom->contentType() == ld::Atom::typeCString)
							       || (strcmp(methods[methodIndex].typeAtom->section().segmentName(), "__TEXT") == 0));
						}
...
#if SUPPORT_ARCH_arm64e
						else if (!isProtocolList && (entryOffset == 2 * ptrSize) && (fit->clusterSize == Fixup::k2of2)
				...
				{
					assert(methods[i].methodName && (methods[i].implAtom || isProtocolList) && (methods[i].selectorRefAtom
					        || methods[i].selectorStringAtom));
					callback(methods[i]);
				}
			}
			...
			template <typename A>
			MethodListAtom<A>::MethodListAtom(ld::Internal &state, const ld::Atom *baseMethodList,
			                                  MethodListAtom<A>::ListFormat kind, MethodListAtom<A>::ListUse use,
				...
				: ld::Atom((kind == threeDeltas) ? _s_section_rel : _s_section_ptrs,
				           ...
				           symbolTableIn, false, false, false, ld::Atom::Alignment(3)), _file(NULL), _methodCount(0), _listFormat(kind),
				  _listUse(use)
			{
				static const bool log = false;
				...
				if (log)
				{
					fprintf(stderr, "total method count in merged list %u\n\n", _methodCount);
				}
			...
			template <typename A>
			void
			MethodListAtom<A>::appendMethod(uint32_t methodIndex, const MethodEntryInfo &method, ld::Internal &state,
			                                NameToAtom &selectorNameToSlot)
			{
				int32_t entryOffset;
				...
				switch (_listFormat)
				{
					case threeDeltas:
						...
						_fixups.push_back(ld::Fixup(entryOffset + 0,  ld::Fixup::k1of4, ld::Fixup::kindSetTargetAddress,
						                            ld::Fixup::bindingByContentBound, selectorRefAtom));
						_fixups.push_back(ld::Fixup(entryOffset + 0,  ld::Fixup::k2of4, ld::Fixup::kindAddAddend,             -entryOffset));
						_fixups.push_back(ld::Fixup(entryOffset + 0,  ld::Fixup::k3of4, ld::Fixup::kindSubtractTargetAddress,
						                            ld::Fixup::bindingDirectlyBound, this));
						...
						{
							_fixups.push_back(ld::Fixup(entryOffset + 4,  ld::Fixup::k1of4, ld::Fixup::kindSetTargetAddress,
							                            ld::Fixup::bindingByContentBound, method.typeAtom));
							_fixups.push_back(ld::Fixup(entryOffset + 4,  ld::Fixup::k2of4, ld::Fixup::kindAddAddend,
							                            -(entryOffset + 4)));
							_fixups.push_back(ld::Fixup(entryOffset + 4,  ld::Fixup::k3of4, ld::Fixup::kindSubtractTargetAddress,
							                            ld::Fixup::bindingDirectlyBound, this));
							_fixups.push_back(ld::Fixup(entryOffset + 4,  ld::Fixup::k4of4, ld::Fixup::kindStoreLittleEndian32));
						}
						// 32-delta to impl
						_fixups.push_back(ld::Fixup(entryOffset + 8,  ld::Fixup::k1of4, ld::Fixup::kindSetTargetAddress,
						                            ld::Fixup::bindingDirectlyBound, method.implAtom));
						_fixups.push_back(ld::Fixup(entryOffset + 8,  ld::Fixup::k2of4, ld::Fixup::kindAddAddend,
						                            -(entryOffset + 8)));
						_fixups.push_back(ld::Fixup(entryOffset + 8,  ld::Fixup::k3of4, ld::Fixup::kindSubtractTargetAddress,
						                            ld::Fixup::bindingDirectlyBound, this));
					...
					case threePointers:
						...
						_fixups.push_back(ld::Fixup(entryOffset + 0, 				  ld::Fixup::k1of1, pointerFixupKind<typename A::P>(),
						                            ld::Fixup::bindingByContentBound, method.selectorStringAtom));
						// method types are optional
						if (method.typeAtom != nullptr)
						{
							_fixups.push_back(ld::Fixup(entryOffset + sizeof(pint_t),   ld::Fixup::k1of1, pointerFixupKind<typename A::P>(),
							                            ld::Fixup::bindingByContentBound, method.typeAtom));
						}
						// protocol method lists have no impl pointer
						if (method.implAtom != nullptr)
						{
							_fixups.push_back(ld::Fixup(entryOffset + sizeof(pint_t) * 2, ld::Fixup::k1of1, pointerFixupKind<typename A::P>(),
							                            ld::Fixup::bindingDirectlyBound,  method.implAtom));
						}
						break;
					case threePointersAuthImpl:
						...
						_fixups.push_back(ld::Fixup(entryOffset + sizeof(pint_t),   ld::Fixup::k1of1, pointerFixupKind<typename A::P>(),
						                            ld::Fixup::bindingByContentBound, method.typeAtom));
						_fixups.push_back(ld::Fixup(entryOffset + sizeof(pint_t) * 2, ld::Fixup::k1of2, ld::Fixup::kindSetAuthData,
						                            methodImplAuthData));
						_fixups.push_back(ld::Fixup(entryOffset + sizeof(pint_t) * 2, ld::Fixup::k2of2,
						                            ld::Fixup::kindStoreTargetAddressLittleEndianAuth64,  method.implAtom));
#endif
						break;
					case twoPointers:
			...
			template <typename A>
			void
			MethodListAtom<A>::copyRawContent(uint8_t buffer[]) const
			{
				bzero(buffer, size());
				// set count
				           ...
				           symbolTableIn, false, false, false, ld::Atom::Alignment(3)), _file(NULL), _protocolCount(0)
			{
				...
				for (const ld::Atom *aCategoryAtom : *categories)
				{
					...
					if (categoryProtocolListAtom != NULL)
					{
						...
						for (ld::Fixup::iterator fit = categoryProtocolListAtom->fixupsBegin(); fit != categoryProtocolListAtom->fixupsEnd();
						        ++fit)
						{
							ld::Fixup fixup = *fit;
				           ...
				           symbolTableNotIn, false, false, false, ld::Atom::Alignment(3)), _file(NULL), _propertyCount(0)
			{
				...
				for (const ld::Atom *aCategoryAtom : *categories)
				{
					const ld::Atom *categoryPropertyListAtom = kind == PropertyKind::ClassProperties ? Category<A>::getClassProperties(
					            state, aCategoryAtom) : Category<A>::getInstanceProperties(state, aCategoryAtom);
				...
				for (const ld::Atom *aCategoryAtom : *categories)
				{
					...
					if (categoryPropertyListAtom != NULL)
					{
						for (ld::Fixup::iterator fit = categoryPropertyListAtom->fixupsBegin(); fit != categoryPropertyListAtom->fixupsEnd();
						        ++fit)
						{
							ld::Fixup fixup = *fit;
							fixup.offsetInAtom += slide;
			...
			template <typename A>
			bool
			scanCategories(ld::Internal &state)
			{
				bool warned = false;
			...
			void
			doPass(const Options &opts, ld::Internal &state)
			{
