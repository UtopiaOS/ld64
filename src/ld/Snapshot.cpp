...
static const char *frameworkStubsString     =
    "framework_stubs";    // directory containing framework stub info (text files)
static const char *dataFilesString          =
...
static const char *linkCommandString        =
...
Snapshot::Snapshot(const Options *opts) : fOptions(opts), fRecordArgs(false), fRecordObjects(false),
	fRecordDylibSymbols(false), fRecordArchiveFiles(false), fRecordUmbrellaFiles(false), fRecordDataFiles(false),
	fFrameworkArgAdded(false), fRecordKext(false), fSnapshotLocation(NULL), fSnapshotName(NULL), fRootDir(NULL),
...
void
Snapshot::setSnapshotPath(const char *path)
{
	if (fRootDir == NULL)
	{
...
void
Snapshot::setSnapshotMode(SnapshotMode mode)
{
	if (fRootDir == NULL)
	{
		// free stuff
	...
	if (fRootDir == NULL)
	{
		...
		switch (mode)
		{
			case SNAPSHOT_DISABLED:
				break;
			case SNAPSHOT_DEBUG:
				fRecordArgs = fRecordObjects = fRecordDylibSymbols = fRecordArchiveFiles = fRecordUmbrellaFiles = fRecordDataFiles =
				                                   true;
				break;
			case SNAPSHOT_KEXT:
				fRecordKext = fRecordArgs = fRecordObjects = fRecordDylibSymbols = fRecordArchiveFiles = fRecordUmbrellaFiles =
				                                fRecordDataFiles = true;
				break;
			default:
				break;
		}
...
void
Snapshot::setOutputPath(const char *path)
{
	fOutputPath = strdup(path);
}

void
Snapshot::setSnapshotName()
{
	if (fRootDir == NULL)
	{
		if (fOutputPath == NULL)
		...
		else
		{
			...
			else
			{
				...
				localtime_r(&now, &t);
				char buf[PATH_MAX];
				snprintf(buf, sizeof(buf) - 1, "%s-%4.4d-%2.2d-%2.2d-%2.2d%2.2d%2.2d.ld-snapshot", base, t.tm_year + 1900, t.tm_mon,
...
// file - the file name
void
Snapshot::buildPath(char *buf, const char *subdir, const char *file)
{
	if (fRootDir == NULL)
	{
	...
	if (subdir)
	{
		...
		mode_t mode = fRecordKext ? (S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH) :
...
// file - the file name
void
Snapshot::buildUniquePath(char *buf, const char *subdir, const char *file)
{
...
const char *
Snapshot::subdir(const char *subdir)
{
	if (fRecordKext)
	{
		return filesString;
...
void
Snapshot::copyFileToSnapshot(const char *sourcePath, const char *subdir, char *path)
{
...
void
Snapshot::writeCommandLine(bool rawArgs)
{
...
void
Snapshot::recordRawArgs(int argc, const char *argv[])
{
...
void
Snapshot::addSnapshotLinkArg(int argIndex, int argCount, int fileArg)
{
	if (fRootDir == NULL)
	{
...
void
Snapshot::recordArch(const char *arch)
{
...
void
Snapshot::recordObjectFile(const char *path)
{
	if (fRootDir == NULL)
	{
...
void
Snapshot::addFrameworkArg(const char *framework)
{
	bool found = false;
...
void
Snapshot::addDylibArg(const char *dylib)
{
	bool found = false;
	for (unsigned i = 0; i < fArgs.size() - 1; i++)
	{
...
void
Snapshot::recordDylibSymbol(ld::dylib::File *dylibFile, const char *name)
{
	if (fRootDir == NULL)
	{
...
void
Snapshot::recordArchive(const char *archiveFile)
{
	if (fRootDir == NULL)
	{
...
void
Snapshot::recordSubUmbrella(const char *frameworkPath)
{
	if (fRootDir == NULL)
	{
...
void
Snapshot::recordSubLibrary(const char *dylibPath)
{
	if (fRootDir == NULL)
	{
...
void
Snapshot::recordAssertionMessage(const char *fmt, ...)
{
	char *msg;
	va_list args;
	va_start(args, fmt);
