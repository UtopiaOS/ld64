...
namespace ld
{
	namespace tool
	{
		...
		void
		OutputFile::dumpAtomsBySection(ld::Internal &state, bool printAtoms)
		{
		...
		void
		OutputFile::write(ld::Internal &state)
		{
		...
		bool
		OutputFile::findSegment(ld::Internal &state, uint64_t addr, uint64_t *start, uint64_t *end, uint32_t *index)
		{
			uint32_t segIndex = 0;
		...
		void
		OutputFile::assignAtomAddresses(ld::Internal &state)
		{
			const bool log = false;
			if (log)
			{
				fprintf(stderr, "assignAtomAddresses()\n");
			}
			for (std::vector<ld::Internal::FinalSection *>::iterator sit = state.sections.begin(); sit != state.sections.end();
			        ++sit)
			{
				...
				for (std::vector<const ld::Atom *>::iterator ait = sect->atoms.begin(); ait != sect->atoms.end(); ++ait)
				{
					...
					switch (sect-> type())
					{
						...
						default:
							...
							if (log)
							{
								fprintf(stderr, "    atom=%p, addr=0x%08llX, name=%s\n", atom, atom->finalAddress(), atom->name());
							}
							break;
					}
				}
		...
		void
		OutputFile::updateLINKEDITAddresses(ld::Internal &state)
		{
		...
		void
		OutputFile::setLoadCommandsPadding(ld::Internal &state)
		{
			...
			switch (_options.outputKind())
			{
				...
				default:
					...
					for (std::vector<ld::Internal::FinalSection *>::reverse_iterator it = state.sections.rbegin();
					        it != state.sections.rend(); ++it)
					{
						ld::Internal::FinalSection *sect = *it;
		...
		uint64_t
		OutputFile::pageAlign(uint64_t addr)
		{
		...
		uint64_t
		OutputFile::pageAlign(uint64_t addr, uint64_t pageSize)
		{
			return ((addr + pageSize - 1) & (-pageSize));
		}

		static const char *
		makeName(const ld::Atom &atom)
		{
		...
		static const char *
		referenceTargetAtomName(ld::Internal &state, const ld::Fixup *ref)
		{
			switch (ref->binding)
			{
		...
		bool
		OutputFile::targetIsThumb(ld::Internal &state, const ld::Fixup *fixup)
		{
		...
		uint64_t
		OutputFile::addressOf(const ld::Internal &state, const ld::Fixup *fixup, const ld::Atom **target)
		{
		...
		uint64_t
		OutputFile::addressAndTarget(const ld::Internal &state, const ld::Fixup *fixup, const ld::Atom **target)
		{
			switch (fixup->binding)
			{
				case ld::Fixup::bindingNone:
		...
		uint64_t
		OutputFile::sectionOffsetOf(const ld::Internal &state, const ld::Fixup *fixup)
		{
			...
			for (std::vector<ld::Internal::FinalSection *>::const_iterator it = state.sections.begin(); it != state.sections.end();
			        ++it)
			{
		...
		uint64_t
		OutputFile::tlvTemplateOffsetOf(const ld::Internal &state, const ld::Fixup *fixup)
		{
			const ld::Atom *target = NULL;
			switch (fixup->binding)
			{
		...
		void
		OutputFile::printSectionLayout(ld::Internal &state)
		{
		...
		void
		OutputFile::rangeCheck8(int64_t displacement, ld::Internal &state, const ld::Atom *atom, const ld::Fixup *fixup)
		{
		...
		void
		OutputFile::rangeCheck16(int64_t displacement, ld::Internal &state, const ld::Atom *atom, const ld::Fixup *fixup)
		{
		...
		void
		OutputFile::rangeCheckBranch32(int64_t displacement, ld::Internal &state, const ld::Atom *atom, const ld::Fixup *fixup)
		{
		...
		void
		OutputFile::rangeCheckAbsolute32(int64_t displacement, ld::Internal &state, const ld::Atom *atom,
		...
		void
		OutputFile::rangeCheckRIP32(int64_t displacement, ld::Internal &state, const ld::Atom *atom, const ld::Fixup *fixup)
		{
			const int64_t twoGigLimit  = 0x7FFFFFFF;
			if ((displacement > twoGigLimit) || (displacement < (-twoGigLimit)))
			{
		...
		void
		OutputFile::rangeCheckARM12(int64_t displacement, ld::Internal &state, const ld::Atom *atom, const ld::Fixup *fixup)
		{
		...
		bool
		OutputFile::checkArmBranch24Displacement(int64_t displacement)
		{
			return ((displacement < 33554428LL) && (displacement > (-33554432LL)));
		}

		void
		OutputFile::rangeCheckARMBranch24(int64_t displacement, ld::Internal &state, const ld::Atom *atom,
		                                  const ld::Fixup *fixup)
		{
		...
		bool
		OutputFile::checkThumbBranch22Displacement(int64_t displacement)
		{
		...
		void
		OutputFile::rangeCheckThumbBranch22(int64_t displacement, ld::Internal &state, const ld::Atom *atom,
		                                    const ld::Fixup *fixup)
		{
		...
		void
		OutputFile::rangeCheckARM64Branch26(int64_t displacement, ld::Internal &state, const ld::Atom *atom,
		                                    const ld::Fixup *fixup)
		{
		...
		void
		OutputFile::rangeCheckARM64Page21(int64_t displacement, ld::Internal &state, const ld::Atom *atom,
		                                  const ld::Fixup *fixup)
		{
		...
		uint16_t
		OutputFile::get16LE(uint8_t *loc)
		{
			return LittleEndian::get16(*(uint16_t *)loc);
		}
		void
		OutputFile::set16LE(uint8_t *loc, uint16_t value)
		{
			LittleEndian::set16(*(uint16_t *)loc, value);
		}

		uint32_t
		OutputFile::get32LE(uint8_t *loc)
		{
			return LittleEndian::get32(*(uint32_t *)loc);
		}
		void
		OutputFile::set32LE(uint8_t *loc, uint32_t value)
		{
			LittleEndian::set32(*(uint32_t *)loc, value);
		}

		uint64_t
		OutputFile::get64LE(uint8_t *loc)
		{
			return LittleEndian::get64(*(uint64_t *)loc);
		}
		void
		OutputFile::set64LE(uint8_t *loc, uint64_t value)
		{
			LittleEndian::set64(*(uint64_t *)loc, value);
		}

		uint16_t
		OutputFile::get16BE(uint8_t *loc)
		{
			return BigEndian::get16(*(uint16_t *)loc);
		}
		void
		OutputFile::set16BE(uint8_t *loc, uint16_t value)
		{
			BigEndian::set16(*(uint16_t *)loc, value);
		}

		uint32_t
		OutputFile::get32BE(uint8_t *loc)
		{
			return BigEndian::get32(*(uint32_t *)loc);
		}
		void
		OutputFile::set32BE(uint8_t *loc, uint32_t value)
		{
			BigEndian::set32(*(uint32_t *)loc, value);
		}

		uint64_t
		OutputFile::get64BE(uint8_t *loc)
		{
			return BigEndian::get64(*(uint64_t *)loc);
		}
		void
		OutputFile::set64BE(uint8_t *loc, uint64_t value)
		{
			BigEndian::set64(*(uint64_t *)loc, value);
		}
		...
		static uint32_t
		makeLDR_literal(const LoadStoreInfo &info, uint64_t targetAddress, uint64_t instructionAddress)
		{
		...
		static uint32_t
		makeADR(uint32_t destReg, uint64_t targetAddress, uint64_t instructionAddress)
		{
		...
		static uint32_t
		makeLoadOrStore(const LoadStoreInfo &info)
		{
		...
		static bool
		parseLoadOrStore(uint32_t instruction, LoadStoreInfo &info)
		{
		...
		static bool
		parseADRP(uint32_t instruction, AdrpInfo &info)
		{
		...
		static bool
		parseADD(uint32_t instruction, AddInfo &info)
		{
...
#if 0
		static uint32_t
		makeLDR_scaledOffset(const LoadStoreInfo &info)
		{
			assert((info.reg & 0xFFFFFFE0) == 0);
			...
			return (0x39000000 | (sizeBits << 30) | (vBit << 26) | (opcBits << 22) | (scaledOffset << 10) |
			        (info.baseReg << 5) | info.reg);
		}

		static uint32_t
		makeLDR_literal(uint32_t destReg, uint32_t loadSize, bool isFloat, uint64_t targetAddress, uint64_t instructionAddress)
		{
			int64_t delta = targetAddress - instructionAddress;
			assert(delta < 1024 * 1024);
			assert(delta > -1024 * 1024);
		...
		static bool
		ldrInfo(uint32_t instruction, uint8_t *size, uint8_t *destReg, bool *v, uint32_t *scaledOffset)
		{
		...
		static bool
		withinOneMeg(uint64_t addr1, uint64_t addr2)
		{
		...
		void
		OutputFile::setInfo(ld::Internal &state, const ld::Atom *atom, uint8_t *buffer,
		                    const std::map<uint32_t, const Fixup *> &usedByHints,
...
#if SUPPORT_ARCH_arm64
		static bool
		isPageKind(const ld::Fixup *fixup, bool mustBeGOT = false)
		{
			if (fixup == NULL)
			{
				return false;
		...
		static bool
		isPageOffsetKind(const ld::Fixup *fixup, bool mustBeGOT = false)
		{
			if (fixup == NULL)
			{
				return false;
...
#define LOH_ASSERT(cond) \
	if ( !(cond) ) { \
		...
		void
		OutputFile::applyFixUps(ld::Internal &state, uint64_t mhAddress, const ld::Atom *atom, uint8_t *buffer)
		{
				...
				switch ((ld::Fixup::Kind)(fit->kind))
				{
					...
					case ld::Fixup::kindStoreLittleEndian32:
						...
						if (_options.makeChainedFixups() && !state.cantUseChainedFixups && !fit->contentAddendOnly
						        && (atom->contentType() != ld::Atom::ContentType::typeCFI)
					...
					case ld::Fixup::kindStoreTargetAddressLittleEndian64:
						...
						else if ((atom->contentType() == ld::Atom::typeInitializerPointers) && _options.dyldLoadsOutput()
						         && ((accumulator - mhAddress) > 0x100000000ULL))
						{
					...
					case ld::Fixup::kindStoreTargetAddressARMBranch24:
						...
						if (toTarget->contentType() == ld::Atom::typeBranchIsland)
						{
							...
							for (ld::Fixup::iterator islandfit = toTarget->fixupsBegin(), end = toTarget->fixupsEnd(); islandfit != end;
							        ++islandfit)
							{
				...
				for (ld::Fixup::iterator fit = atom->fixupsBegin(), end = atom->fixupsEnd(); fit != end; ++fit)
				{
					...
					switch (alt.info.kind)
					{
						...
						case LOH_ARM64_ADRP_LDR:
							...
							if (literalableSize && usableSegment && targetFourByteAligned
							        && withinOneMeg(infoB.instructionAddress, infoA.targetAddress))
							{
							...
							else
							{
								if (_options.verboseOptimizationHints())
									fprintf(stderr,
									        ...
									        infoB.instructionAddress, isLDR, literalableSize, withinOneMeg(infoB.instructionAddress, infoA.targetAddress),
									        usableSegment, ldrInfoB.offset);
							}
							break;
						case LOH_ARM64_ADRP_ADD_LDR:
							...
							if (literalableSize && usableSegment && targetFourByteAligned
							        && withinOneMeg(infoC.instructionAddress, infoA.targetAddress + ldrInfoC.offset))
							{
								// can do T1 transformation to LDR literal
								set32LE(infoA.instructionContent, makeNOP());
								set32LE(infoB.instructionContent, makeNOP());
								set32LE(infoC.instructionContent, makeLDR_literal(ldrInfoC, infoA.targetAddress + ldrInfoC.offset,
							...
							else if (usableSegment && withinOneMeg(infoA.instructionAddress, infoA.targetAddress + ldrInfoC.offset))
							{
								...
								set32LE(infoA.instructionContent, makeADR(ldrInfoC.baseReg, infoA.targetAddress + ldrInfoC.offset,
							...
							else
							{
								if (_options.verboseOptimizationHints())
									fprintf(stderr,
									        ...
									        infoC.instructionAddress, ldrInfoC.size, literalableSize, withinOneMeg(infoC.instructionAddress,
									                infoA.targetAddress + ldrInfoC.offset), usableSegment, targetFourByteAligned, ldrInfoC.offset);
							}
							break;
						...
						case LOH_ARM64_ADRP_LDR_GOT_LDR:
							...
							if (isLDR)
							{
								...
								if (usableSegment && targetFourByteAligned
								        && withinOneMeg(infoB.instructionAddress, infoA.targetAddress + ldrInfoC.offset))
								{
									// can do T5 transform
									set32LE(infoA.instructionContent, makeNOP());
									set32LE(infoB.instructionContent, makeLDR_literal(ldrInfoB, infoA.targetAddress, infoB.instructionAddress));
							...
							else if (isADD)
							{
								...
								targetFourByteAligned = (((infoA.targetAddress) & 0x3) == 0);
								literalableSize  = ((ldrInfoC.size != 1) && (ldrInfoC.size != 2));
								if (usableSegment && literalableSize && targetFourByteAligned
								        && withinOneMeg(infoC.instructionAddress, infoA.targetAddress + ldrInfoC.offset))
								{
									// can do T1 transform
									set32LE(infoA.instructionContent, makeNOP());
							...
							else
							{
								if (_options.verboseOptimizationHints())
									fprintf(stderr,
									        ...
									        infoC.instructionAddress, ldrInfoC.size, withinOneMeg(infoC.instructionAddress, infoA.targetAddress + ldrInfoC.offset),
									        usableSegment, ldrInfoC.offset);
							}
							break;
		...
		static bool
		chainedFixupBindAddendFitsInline(uint64_t accumulator, uint16_t chainedPointerFormat)
		{
		...
		bool
		OutputFile::needsBind(const ld::Atom *toTarget, bool authPtr, uint64_t *accumulator, uint64_t *inlineAddend,
			...
			switch (toTarget->definition())
			{
				...
				case ld::Atom::definitionTentative:
					...
					if ((toTarget->combine() == ld::Atom::combineByName) && (toTarget->definition() == ld::Atom::definitionRegular)
					        && _options.dyldOrKernelLoadsOutput())
					{
						isWeakBind = true;
					}
		...
		void
		OutputFile::setFixup64(uint8_t *fixUpLocation, uint64_t accumulator, const ld::Atom *toTarget)
		{
...
#if SUPPORT_ARCH_arm64e
		void
		OutputFile::setFixup64e(uint8_t *fixUpLocation, uint64_t accumulator, Fixup::AuthData authData,
		...
		void
		OutputFile::setFixup32(uint8_t *fixUpLocation, uint64_t accumulator, const ld::Atom *toTarget)
		{
			uint32_t bindOrdinal = 0;
			uint64_t inlineAddend = 0;
			bool isBind = needsBind(toTarget, false, &accumulator, &inlineAddend, &bindOrdinal);
		...
		void
		OutputFile::copyNoOps(uint8_t *from, uint8_t *to, bool thumb)
		{
		...
		bool
		OutputFile::takesNoDiskSpace(const ld::Section *sect)
		{
			switch (sect->type())
			{
		...
		bool
		OutputFile::hasZeroForFileOffset(const ld::Section *sect)
		{
			switch (sect->type())
			{
				case ld::Section::typeZeroFill:
		...
		void
		OutputFile::writeAtoms(ld::Internal &state, uint8_t *wholeBuffer)
		{
			...
			if (_options.useLinkedListBinding() && !_threadedRebaseBindIndices.empty())
			{
				...
				if (_options.makeThreadedStartsSection())
				{
					...
					uint64_t threadStartsFileOffset = threadStartsAtom->finalAddress() - threadStartsSection->address +
					                                  threadStartsSection->fileOffset;
					// Skip the header
					if (logThreadedFixups)
					{
						fprintf(stderr, "thread start[0x%llX]: header=0x%X\n", threadStartsFileOffset,
						        get32LE(&wholeBuffer[threadStartsFileOffset]));
					}
					...
					for (ChainedFixupSegInfo &segInfo : _chainedFixupSegments)
					{
						...
						for (ChainedFixupPageInfo &pageInfo : segInfo.pages)
						{
							...
							if (!pageInfo.chainOverflows.empty())
							{
								...
								dyld_chained_starts_in_segment *segChains = (dyld_chained_starts_in_segment *)((uint8_t *)chains +
								        chains->seg_info_offset[segIndex]);
								uint32_t						maxOverFlowCount = (segChains->size - offsetof(dyld_chained_starts_in_segment,
								        page_start[segChains->page_count])) / sizeof(uint16_t);
		...
		OutputFile::farthestChainableLocation(dyld_chained_ptr_32_rebase *start)
		{
		...
		void
		OutputFile::chain32bitPointers(dyld_chained_ptr_32_rebase *prevLoc, dyld_chained_ptr_32_rebase *finalLoc,
		...
		void
		OutputFile::computeContentUUID(ld::Internal &state, uint8_t *wholeBuffer)
		{
			const bool log = false;
			if ((_options.outputKind() != Options::kObjectFile) || state.someObjectFileHasDwarf)
			{
				        ...
				        bitcodeSectOffset, bitcodePaddingEnd))
				{
					...
					// Exclude section cmd
					if (log)
						fprintf(stderr, "bundle cmd start=0x%08llX, bundle cmd end=0x%08llX\n",
					...
					if (log)
						fprintf(stderr, "bundle start=0x%08llX, bundle end=0x%08llX\n",
						        bitcodeSectOffset, bitcodePaddingEnd);
				...
				if (_symbolTableAtom->hasStabs(stabsStringsOffsetStart, tabsStringsOffsetEnd, stabsOffsetStart, stabsOffsetEnd))
				{
					...
					if (log)
					{
						fprintf(stderr, "stabNlist offset=0x%08llX, size=0x%08llX\n", firstStabNlistFileOffset,
						        lastStabNlistFileOffset - firstStabNlistFileOffset);
					}
					if (log)
					{
						fprintf(stderr, "stabString offset=0x%08llX, size=0x%08llX\n", firstStabStringFileOffset,
						        lastStabStringFileOffset - firstStabStringFileOffset);
					}
					...
					{
						excludeRegions.emplace_back(std::pair<uint64_t, uint64_t>(firstStabStringFileOffset,
						                            stringPoolFileOffset + stringPoolFileSize));
					}
					else
					{
					...
					excludeRegions.emplace_back(std::pair<uint64_t, uint64_t>(linkeditSegCmdOffset,
					                            linkeditSegCmdOffset + linkeditSegCmdSize));
					if (log)
					{
						fprintf(stderr, "linkedit SegCmdOffset=0x%08llX, size=0x%08llX\n", linkeditSegCmdOffset, linkeditSegCmdSize);
					}
					...
					excludeRegions.emplace_back(std::pair<uint64_t, uint64_t>(symbolTableCmdOffset,
				...
				if (!excludeRegions.empty())
				{
					...
					for (auto &region : excludeRegions)
					{
						...
						if (log)
						{
							fprintf(stderr, "checksum 0x%08llX -> 0x%08llX\n", checksumStart, regionStart);
						}
					...
					if (log)
						fprintf(stderr, "uuid=%02X, %02X, %02X, %02X, %02X, %02X, %02X, %02X\n", digest[0], digest[1], digest[2],
		...
		void
		OutputFile::writeOutputFile(ld::Internal &state)
		{
		...
		{
			bool
			operator()(const ld::Atom *left, const ld::Atom *right) const
			{
				return (strcmp(left->name(), right->name()) < 0);
			}

			bool
			operator()(const ld::Atom *left, const char *right) const
			{
				return (strcmp(left->name(), right) < 0);
			}

			bool
			operator()(const char *left, const ld::Atom *right) const
			{
		...
		class NotInSet
		{
			public:
				...
				bool
				operator()(const ld::Atom *atom) const
				{
		...
		void
		OutputFile::buildSymbolTable(ld::Internal &state)
		{
						...
						case ld::Atom::scopeLinkageUnit:
							...
							else
							{
								...
								// this works by making __mh_execute_header be a local symbol which takes symbol index 0
								         ...
								         && !_options.makeThreadedStartsSection())
								{
									_localAtoms.push_back(atom);
								}
								else
		...
		void
		OutputFile::addPreloadLinkEdit(ld::Internal &state)
		{
			switch (_options.architecture())
			{
		...
		void
		OutputFile::addLinkEdit(ld::Internal &state)
		{
		...
		void
		OutputFile::addLoadCommands(ld::Internal &state)
		{
			switch (_options.architecture())
			{
#if SUPPORT_ARCH_x86_64
		...
		bool
		OutputFile::hasOrdinalForInstallPath(const char *path, int *ordinal)
		{
			for (std::map<const ld::dylib::File *, int>::const_iterator it = _dylibToOrdinal.begin(); it != _dylibToOrdinal.end();
			        ++it)
			{
		...
		uint32_t
		OutputFile::dylibToOrdinal(const ld::dylib::File *dylib) const
		{
		...
		void
		OutputFile::buildDylibOrdinalMapping(ld::Internal &state)
		{
			...
			for (auto &dylib : state.dylibs)
			{
				...
				if (potentiallyDeadDylibs.count(file) != 0)
				{
					continue;
				}
			...
			for (std::vector<ld::dylib::File *>::iterator it = dylibs.begin(); it != dylibs.end(); ++it)
			{
				...
				if (aDylib->explicitlyLinked() && aDylib->willBeReExported())
				{
					...
					aDylib->forEachExportedSymbol(^(const char *symbolName, bool weakDef)
					{
						...
						{
							warning("symbol '%s' re-exported from %s and %s", _options.demangleSymbol(symbolName),
							        allReExports[symbolName]->leafName(), aDylib->leafName());
						}
						else
						{
		...
		uint32_t
		OutputFile::lazyBindingInfoOffsetForLazyPointerAddress(uint64_t lpAddress)
		{
			return _lazyPointerAddressToInfoOffset[lpAddress];
		}

		void
		OutputFile::setLazyBindingInfoOffset(uint64_t lpAddress, uint32_t lpInfoOffset)
		{
			_lazyPointerAddressToInfoOffset[lpAddress] = lpInfoOffset;
		}

		int
		OutputFile::compressedOrdinalForAtom(const ld::Atom *target) const
		{
		...
		bool
		OutputFile::isPcRelStore(const ld::Fixup *fixup)
		{
			return fixup->isPcRelStore(_options.outputKind() == Options::kKextBundle);
		}

		bool
		OutputFile::isStore(ld::Fixup::Kind kind)
		{
			switch (kind)
			{
				case ld::Fixup::kindNone:
		...
		bool
		OutputFile::setsTarget(const ld::Fixup &fixup)
		{
			return fixup.setsTarget(_options.outputKind() == Options::kObjectFile);
		}

		bool
		OutputFile::isPointerToTarget(ld::Fixup::Kind kind)
		{
			switch (kind)
			{
				case ld::Fixup::kindSetTargetAddress:
		...
		bool
		OutputFile::isPointerFromTarget(ld::Fixup::Kind kind)
		{
			switch (kind)
			{
				case ld::Fixup::kindSubtractTargetAddress:
		...
		uint64_t
		OutputFile::lookBackAddend(ld::Fixup::iterator fit)
		{
			uint64_t addend = 0;
		...
		void
		OutputFile::generateLinkEditInfo(ld::Internal &state)
		{
			for (std::vector<ld::Internal::FinalSection *>::iterator sit = state.sections.begin(); sit != state.sections.end();
			        ++sit)
			{
				...
				if (_options.makeEncryptable() && (strcmp(sect->segmentName(), "__TEXT") == 0)
				        && (strcmp(sect->sectionName(), "__oslogstring") != 0))
				{
		...
		bool
		OutputFile::isFixupForChain(ld::Fixup::iterator fit)
		{
			switch (fit->kind)
			{
				case ld::Fixup::kindStoreLittleEndian64:
		...
		uint16_t
		OutputFile::chainedPointerFormat() const
		{
#if SUPPORT_ARCH_arm64e
		...
		void
		OutputFile::buildChainedFixupInfo(ld::Internal &state)
		{
			if (!_options.makeChainedFixups() && !state.cantUseChainedFixups)
			{
				return;
							...
							switch (fit->binding)
							{
								...
								case ld::Fixup::bindingDirectlyBound:
									...
									if (target->isAlias() && (target->definition() == ld::Atom::definitionProxy))
									{
										...
										{
											if (tfit->firstInCluster() && (tfit->kind == ld::Fixup::kindNoneFollowOn)
											        && (tfit->binding == ld::Fixup::bindingDirectlyBound))
											{
		...
		void
		OutputFile::noteTextReloc(const ld::Atom *atom, const ld::Atom *target)
		{
				...
				if (! this->pieDisabled)
				{
					switch (_options.architecture())
					{
...
#if SUPPORT_ARCH_arm64
						{
							...
							throwf("Absolute addressing not allowed in arm64 code but used in '%s' referencing '%s'", demangledName,
							       _options.demangleSymbol(target->name()));
						}
#endif
						default:
			...
			{
				throwf("illegal text-relocoation (direct reference) to (global,weak) %s in %s from %s in %s", target->name(),
				       target->safeFilePath(), atom->name(), atom->safeFilePath());
			}
			else
			{
				if ((target->file() != NULL) && (atom->file() != NULL))
				{
					throwf("illegal text-relocation to '%s' in %s from '%s' in %s", target->name(), target->safeFilePath(), atom->name(),
					       atom->safeFilePath());
				}
				else
				{
		...
		void
		OutputFile::addDyldInfo(ld::Internal &state,  ld::Internal::FinalSection *sect, const ld::Atom *atom,
		                        ...
		                        uint64_t targetAddend, uint64_t minusTargetAddend)
		{
			...
			{
				warning("addend too large. '%s' contains a pointer to %s+%llu. Dylibs in dyld shared cache can have max addend of 31",
				        atom->name(), target->name(), addend);
			}
			if (needsBinding)
			{
				if (inReadOnlySeg)
		...
		void
		OutputFile::addChainedFixupLocation(ld::Internal &state, ld::Internal::FinalSection *sect,
		...
		void
		OutputFile::addThreadedRebaseInfo(ld::Internal &state,  ld::Internal::FinalSection *sect, const ld::Atom *atom,
		                                  ld::Fixup *fixupWithTarget, ld::Fixup *fixupWithMinusTarget, ld::Fixup *fixupWithStore,
		                                  const ld::Atom *target, const ld::Atom *minusTarget,
		                                  uint64_t targetAddend, uint64_t minusTargetAddend)
		{
		...
		void
		OutputFile::addClassicRelocs(ld::Internal &state, ld::Internal::FinalSection *sect, const ld::Atom *atom,
		                             ld::Fixup *fixupWithTarget, ld::Fixup *fixupWithMinusTarget, ld::Fixup *fixupWithStore,
		                             const ld::Atom *target, const ld::Atom *minusTarget,
		                             uint64_t targetAddend, uint64_t minusTargetAddend)
		{
			...
			assert(_localRelocsAtom != NULL);
			uint64_t relocAddress =  atom->finalAddress() + fixupWithTarget->offsetInAtom - _localRelocsAtom->relocBaseAddress(
			                             state);
			bool inReadOnlySeg = (strcmp(sect->segmentName(), "__TEXT") == 0);
			...
			switch (fixupWithStore->kind)
			{
				...
				case ld::Fixup::kindStoreLittleEndianAuth64:
					...
					{
						throwf("authenticated pointer in atom %s from %s to %s is not supported", atom->name(), atom->safeFilePath(),
						       target->name());
					}
					break;
#endif
		...
		bool
		OutputFile::useExternalSectionReloc(const ld::Atom *atom, const ld::Atom *target, ld::Fixup *fixupWithTarget)
		{
			if ((_options.architecture() == CPU_TYPE_X86_64)
			        || (_options.architecture() == CPU_TYPE_ARM64)
#if SUPPORT_ARCH_arm64_32
		...
		bool
		OutputFile::useSectionRelocAddend(ld::Fixup *fixupWithTarget)
		{
#if SUPPORT_ARCH_arm64
		...
		void
		OutputFile::addSectionRelocs(ld::Internal &state, ld::Internal::FinalSection *sect, const ld::Atom *atom,
		                             ...
		                             uint64_t targetAddend, uint64_t minusTargetAddend)
		{
			...
			bool minusTargetUsesExternalReloc = (minusTarget != NULL)
			                                    && this->useExternalSectionReloc(atom, minusTarget, fixupWithMinusTarget);
		...
		void
		OutputFile::makeSplitSegInfo(ld::Internal &state)
		{
			if (!_options.sharedRegionEligible())
			{
				return;
			        ...
			        ++sit)
			{
				...
				if ((_options.outputKind() == Options::kDynamicLibrary) && (strcmp(sect->sectionName(), "__interpose") == 0)
				        && (strncmp(sect->segmentName(), "__DATA", 6) == 0))
				{
		...
		void
		OutputFile::makeSplitSegInfoV2(ld::Internal &state)
		{
			        ...
			        ++sit)
			{
				...
				if (log)
				{
					fprintf(stderr, "sect: %s, address=0x%llX\n", sect->sectionName(), sect->address);
				}
				for (std::vector<const ld::Atom *>::iterator ait = sect->atoms.begin(); ait != sect->atoms.end(); ++ait)
				{
					const ld::Atom *atom = *ait;
					...
					for (ld::Fixup::iterator fit = atom->fixupsBegin(), end = atom->fixupsEnd(); fit != end; ++fit)
					{
						...
						if (fit->lastInCluster())
						{
							if ((kind != 0) && (target != NULL) && (target->definition() != ld::Atom::definitionProxy))
							{
								...
								if (log)
									fprintf(stderr, "from (%d.%s + 0x%llX) to (%d.%s + 0x%llX), kind=%d, atomAddr=0x%llX, sectAddr=0x%llx\n",
		...
		void
		OutputFile::writeMapFile(ld::Internal &state)
		{
			if (_options.generatedMapPath() != NULL)
			{
				...
				if (mapFile != NULL)
				{
					...
					for (std::map<ld::File::Ordinal, const ld::File *>::iterator it = ordinalToReader.begin(); it != ordinalToReader.end();
					        ++it)
					{
		...
		void
		OutputFile::writeJSONEntry(ld::Internal &state)
		{
		...
		const char *
		OutputFile::canonicalOSOPath(const char *path)
		{
		...
		const char *
		OutputFile::assureFullPath(const char *path)
		{
			if (path[0] == '/')
			{
				return path;
		...
		void
		OutputFile::synthesizeDebugNotes(ld::Internal &state)
		{
			...
			for (std::vector<const ld::Atom *>::iterator it = atomsNeedingDebugNotes.begin(); it != atomsNeedingDebugNotes.end();
			        it++)
			{
			...
			std::sort(orderedFilesSeen.begin(), orderedFilesSeen.end(), [](const ld::relocatable::File * lhs,
			          const ld::relocatable::File * rhs)
			{
		...
		void
		OutputFile::ChainedFixupBinds::ensureTarget(const ld::Atom *atom, bool authPtr, uint64_t addend)
		{
			if (addend == 0)
			{
		...
		uint32_t
		OutputFile::ChainedFixupBinds::count() const
		{
			return (uint32_t)_bindsTargets.size();
		}

		bool
		OutputFile::ChainedFixupBinds::hasLargeAddends() const
		{
			return _hasLargeAddends;
		}

		bool
		OutputFile::ChainedFixupBinds::hasHugeAddends() const
		{
			return _hasHugeAddends;
		}

		bool
		OutputFile::ChainedFixupBinds::hasHugeSymbolStrings() const
		{
		...
		void
		OutputFile::ChainedFixupBinds::forEachBind(void (^callback)(unsigned bindOrdinal, const ld::Atom *importAtom,
		        uint64_t addend))
		{
			unsigned index = 0;
			for (const AtomAndAddend &entry : _bindsTargets)
		...
		uint32_t
		OutputFile::ChainedFixupBinds::ordinal(const ld::Atom *atom, uint64_t addend) const
		{
			if (addend == 0)
			{
