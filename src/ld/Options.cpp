...
bool
Options::FileInfo::checkFileExists(const Options &options, const char *p)
{
	if (isInlined)
	{
		modTime = 0;
...
Options::Options(int argc, const char *argv[])
	: fOutputFile("a.out"), fArchitecture(0), fSubArchitecture(0),
	  ...
	  fDyldInstallPath("/usr/lib/dyld"), fLtoCachePath(NULL), fTempLtoObjectPath(NULL), fOverridePathlibLTO(NULL),
	  fLtoCpu(NULL),
	  ...
	  fCommonsMode(kCommonsIgnoreDylibs),  fUUIDMode(kUUIDContent), fLocalSymbolHandling(kLocalSymbolsAll),
	  fWarnCommons(false),
	  ...
	  fDataInCodeInfoLoadCommand(false), fDataInCodeInfoLoadCommandForcedOn(false),
	  ...
	  fUseLinkedListBinding(false), fMakeChainedFixups(false), fMakeChainedFixupsSection(false), fNoLazyBinding(false),
	  fDebugVariant(false),
...
bool
Options::errorBecauseOfWarnings() const
{
...
bool
Options::interposable(const char *name) const
{
...
bool
Options::printWhyLive(const char *symbolName) const
{
...
bool
Options::hasWildCardExportRestrictList() const
{
...
bool
Options::hasWeakBitTweaks() const
{
	// has -exported_symbols_list which contains some wildcards
...
bool
Options::allGlobalsAreDeadStripRoots() const
{
...
bool
Options::dyldOrKernelLoadsOutput() const
{
	switch (fOutputKind)
	{
		case Options::kStaticExecutable:
...
uint32_t
Options::initialSegProtection(const char *segName) const
{
	for (std::vector<Options::SegmentProtect>::const_iterator it = fCustomSegmentProtections.begin();
	        it != fCustomSegmentProtections.end(); ++it)
	{
...
bool
Options::readOnlyDataSegment(const char *name) const
{
	// set SG_READ_ONLY on segment
	// but not for shared cache dylibs (because __objc_const is not read-only)
	return (fUseDataConstSegment && (strcmp(name, "__DATA_CONST") == 0) && (!fSharedRegionEligible
	        || (fOutputKind == Options::kDyld)));
}

uint32_t
Options::maxSegProtection(const char *segName) const
{
...
uint64_t
Options::segPageSize(const char *segName) const
{
...
uint64_t
Options::customSegmentAddress(const char *segName) const
{
	for (std::vector<SegmentStart>::const_iterator it = fCustomSegmentAddresses.begin();
	        it != fCustomSegmentAddresses.end(); ++it)
	{
		if (strcmp(it->name, segName) == 0)
		{
			return it->address;
...
bool
Options::hasCustomSegmentAddress(const char *segName) const
{
	for (std::vector<SegmentStart>::const_iterator it = fCustomSegmentAddresses.begin();
	        it != fCustomSegmentAddresses.end(); ++it)
	{
...
bool
Options::hasCustomSectionAlignment(const char *segName, const char *sectName) const
{
	for (std::vector<SectionAlignment>::const_iterator it =	fSectionAlignments.begin(); it != fSectionAlignments.end();
	        ++it)
	{
...
uint8_t
Options::customSectionAlignment(const char *segName, const char *sectName) const
{
	for (std::vector<SectionAlignment>::const_iterator it =	fSectionAlignments.begin(); it != fSectionAlignments.end();
	        ++it)
	{
...
bool
Options::segmentOrderAfterFixedAddressSegment(const char *segName) const
{
...
bool
Options::hasExportedSymbolOrder()
{
	return (fExportSymbolsOrder.size() > 0);
}

bool
Options::exportedSymbolOrder(const char *sym, unsigned int *order) const
{
...
void
Options::loadSymbolOrderFile(const char *fileOfExports, NameToOrder &orderMapping)
{
	// read in whole file
...
void
Options::loadImplictZipperFile(const char *path, std::vector<const char *> &paths)
{
	// read in whole file
	int fd = ::open(path, O_RDONLY, 0);
	if (fd == -1)
	{
		return;    // Early exist if the file is not present
	}
	struct stat stat_buf;
	::fstat(fd, &stat_buf);
	char *p = (char *)malloc(stat_buf.st_size);
...
bool
Options::forceWeak(const char *symbolName) const
{
	return fForceWeakSymbols.contains(symbolName);
}

bool
Options::forceNotWeak(const char *symbolName) const
{
	return fForceNotWeakSymbols.contains(symbolName);
}

bool
Options::forceWeakNonWildCard(const char *symbolName) const
{
	return fForceWeakSymbols.containsNonWildcard(symbolName);
}

bool
Options::forceNotWeakNonWildcard(const char *symbolName) const
{
	return fForceNotWeakSymbols.containsNonWildcard(symbolName);
}

bool
Options::forceCoalesce(const char *symbolName) const
{
...
bool
Options::shouldExport(const char *symbolName) const
{
	switch (fExportMode)
	{
		case kExportSome:
...
bool
Options::shouldReExport(const char *symbolName) const
{
	return fReExportSymbols.contains(symbolName);
}

bool
Options::keepLocalSymbol(const char *symbolName) const
{
...
Options::sectionOrder(const char *segName) const
{
...
void
Options::setInferredArch(cpu_type_t type, cpu_subtype_t subtype)
{
...
bool
Options::armUsesZeroCostExceptions() const
{
	return ((fArchitecture == CPU_TYPE_ARM) && (fSubArchitecture == CPU_SUBTYPE_ARM_V7K));
}

void
Options::selectFallbackArch(const char *arch)
{
...
void
Options::parseArch(const char *arch)
{
	if (arch == NULL)
	{
...
bool
Options::checkForFile(const char *format, const char *dir, const char *rootName, FileInfo &result) const
{
...
Options::FileInfo
Options::findLibrary(const char *rootName, bool dylibsOnly) const
{
	FileInfo result;
...
Options::FileInfo
Options::findFramework(const char *frameworkName) const
{
...
Options::FileInfo
Options::findFramework(const char *rootName, const char *suffix) const
{
...
void
Options::addTAPIInterface(tapi::LinkerInterfaceFile *interface, const char *path) const
{
...
bool
Options::findFile(const std::string &path, const std::vector<std::string> &tbdExtensions, FileInfo &result) const
{
	FileInfo tbdInfo;
	...
	else if (!tbdInfo.missing() && !dylibInfo.missing())
	{
		...
		else
		{
			...
			{
				warning("text-based stub file %s and library file %s are out of sync. Falling back to library file for linking.",
				        tbdInfo.path, dylibInfo.path);
			}
			result = dylibInfo;
		}
	}
...
Options::FileInfo
Options::findFile(const std::string &path, const ld::dylib::File *fromDylib) const
{
	FileInfo result;
...
bool
Options::hasInlinedTAPIFile(const std::string &path) const
{
...
Options::findTAPIFile(const std::string &path) const
{
#if ((TAPI_API_VERSION_MAJOR == 1 &&  TAPI_API_VERSION_MINOR >= 3) || (TAPI_API_VERSION_MAJOR > 1))
...
Options::FileInfo
Options::findIndirectDylib(const std::string &installName, const ld::dylib::File *fromDylib) const
{
	FileInfo result;
...
void
Options::loadFileList(const char *fileOfPaths, ld::File::Ordinal baseOrdinal)
{
	FILE *file;
...
bool
Options::SetWithWildcards::hasWildCards(const char *symbol)
{
...
void
Options::SetWithWildcards::insert(const char *symbol, SymbolMatchingMode match_mode)
{
...
bool
Options::SetWithWildcards::contains(const char *symbol, bool *matchBecauseOfWildcard) const
{
...
bool
Options::SetWithWildcards::containsWithPrefix(const char *symbol, const char *file, bool &wildCardMatch) const
{
	wildCardMatch = false;
...
bool
Options::SetWithWildcards::containsNonWildcard(const char *symbol) const
{
...
std::vector<const char *>
Options::SetWithWildcards::data() const
{
...
bool
Options::SetWithWildcards::inCharRange(const char *&p, unsigned char c) const
{
	++p; // find end
	const char *b = p;
	while (*p != '\0')
...
bool
Options::SetWithWildcards::wildCardMatch(const char *pattern, const char *symbol) const
{
	const char *s = symbol;
...
void
Options::loadExportFile(const char *fileOfExports, const char *option, SetWithWildcards &set,
                        SymbolMatchingMode match_mode)
{
...
void
Options::parseAliasFile(const char *fileOfAliases)
{
	// read in whole file
...
void
Options::setUndefinedTreatment(const char *treatment)
{
	if (treatment == NULL)
	{
...
void
Options::setWeakReferenceMismatchTreatment(const char *treatment)
{
	if (treatment == NULL)
	{
...
void
Options::addDylibOverride(const char *paths)
{
	if (paths == NULL)
	{
...
uint64_t
Options::parseAddress(const char *addr)
{
	char *endptr;
...
uint32_t
Options::parseProtection(const char *prot)
{
	uint32_t result = 0;
...
//
uint64_t
Options::parseVersionNumber64(const char *versionString)
{
	uint64_t a = 0;
	uint64_t b = 0;
	uint64_t c = 0;
...
uint32_t
Options::currentVersion32() const
{
...
//
uint32_t
Options::parseVersionNumber32(const char *versionString)
{
	uint32_t x = 0;
	uint32_t y = 0;
	uint32_t z = 0;
...
static const char *
cstringSymbolName(const char *orderFileString)
{
	char *result;
...
void
Options::parseOrderFile(const char *path, bool cstring)
{
...
void
Options::parseSectionOrderFile(const char *segment, const char *section, const char *path)
{
...
void
Options::addSection(const char *segment, const char *section, const char *path)
{
...
void
Options::addSectionRename(const char *srcSegment, const char *srcSection, const char *dstSegment,
...
void
Options::addSegmentRename(const char *srcSegment, const char *dstSegment)
{
	if (strlen(srcSegment) > 16)
	{
...
void
Options::addSymbolMove(const char *dstSegment, const char *symbolList,
...
bool
Options::moveRwSymbol(std::string_view symName, const char *filePath, const char *&seg, bool &wildCardMatch) const
{
...
bool
Options::moveAXMethodList(const char *className) const
{
...
bool
Options::moveRoSymbol(std::string_view symName, const char *filePath, const char *&seg, bool &wildCardMatch) const
{
	auto nameStr = std::string(symName);
	auto *name = nameStr.c_str();
...
void
Options::addSectionAlignment(const char *segment, const char *section, const char *alignmentStr)
{
	if (strlen(segment) > 16)
	{
...
void
Options::addLibrary(const FileInfo &info)
{
...
void
Options::warnObsolete(const char *arg)
{
...
void
Options::cannotBeUsedWithBitcode(const char *arg)
{
	if (fBundleBitcode)
	{
...
std::string
Options::getVersionString32(uint32_t ver) const
{
...
std::string
Options::getVersionString64(uint64_t ver) const
{
...
bool
Options::parsePackedVersion32(const std::string &versionStr, uint32_t &result)
{
	result = 0;
	if (versionStr.empty())
	{
...
Options::writeBitcodeLinkOptions() const
{
	...
	for (const SectionAlignment &sectalign : fSectionAlignments)
		for (const char *arg :
		        {
		            "-sectalign", sectalign.segmentName, sectalign.sectionName
		        })
...
const char *
Options::checkForNullArgument(const char *argument_name, const char *arg, bool allowDashArg) const
{
	if (arg == NULL)
	{
...
const char *
Options::checkForNullVersionArgument(const char *argument_name, const char *arg) const
{
	if (arg == NULL)
	{
...
static const ld::PlatformInfo *
isPlatformOption(const char *arg)
{
...
static const char *
removeDoubleSlash(const char *path)
{
			...
			else if (const ld::PlatformInfo *info = isPlatformOption(arg))
			{
				...
				if (value < info->minimumOsVersion)
				{
					if (info->platform == ld::Platform::iOSMac)
					{
						...
						warning("changing %s minOS version from %s to %s", info->printName, versStr,
						        getVersionString32(info->minimumOsVersion).c_str());
					}
					else
					{
...
bool
Options::shouldUseBuildVersion(ld::Platform plat, uint32_t minOSvers) const
{
...
void
Options::buildSearchPaths(int argc, const char *argv[])
{
...
void
Options::parsePreCommandLineEnvironmentSettings()
{
...
void
Options::parsePostCommandLineEnvironmentSettings()
{
...
bool
Options::sharedCacheEligiblePath(const char *path) const
{
	...
	{
		if ((strncmp(path, "/System/iOSSupport/System/Library/", 34) == 0)
...
Options::reconfigureDefaults()
{
	...
	if ((fFinalName != NULL) && ((strcmp(fFinalName, "/usr/libexec/locationd") == 0)
	                             || (strcmp(fFinalName, "/usr/libexec/terminusd") == 0)))
	{
		fUseDataConstSegment = false;
	}
	...
	if (!platforms().contains(ld::Platform::iOS) && !platforms().contains(ld::Platform::tvOS)
	        && !platforms().contains(ld::Platform::watchOS))
	{
		fEncryptable = false;
	}
	...
	switch (fOutputKind)
	{
		case Options::kDynamicExecutable:
			...
			if (platforms().minOS(ld::supportsChainedFixups) && (fArchitecture != CPU_TYPE_I386) && !targetIOSSimulator()
	...
	if ((fArchitecture == CPU_TYPE_X86_64) && (fOutputKind == kDynamicExecutable) && (platforms().minOS(ld::mac10_6)
	...
	if ((fOutputKind == Options::kDynamicLibrary) && (fDylibInstallName != NULL) && (fFinalName != NULL)
	        && sharedCacheEligiblePath(fFinalName) && !fDebugVariant)
	{
...
void
Options::checkIllegalOptionCombinations()
{
	...
	if (fCustomSegmentAddresses.size() != 0)
	{
		...
		{
			for (std::vector<SegmentStart>::iterator it = fCustomSegmentAddresses.begin(); it != fCustomSegmentAddresses.end();
			        ++it)
			{
		...
		for (std::vector<SegmentStart>::iterator it = fCustomSegmentAddresses.begin(); it != fCustomSegmentAddresses.end();
		        ++it)
		{
			for (std::vector<SegmentStart>::iterator it2 = fCustomSegmentAddresses.begin(); it2 != fCustomSegmentAddresses.end();
			        ++it2)
			{
	...
	if ((fOutputKind == Options::kDynamicLibrary) && (fDylibInstallName != NULL))
	{
		if (platforms().contains(ld::Platform::iOS) && !platforms().minOS(ld::iOS_8_0) && (fDylibInstallName[0] == '@')
		        && !fEncryptableForceOff)
		{
...
void
Options::expandResponseFiles(int &argc, const char ** &argv)
{
	int newargc = argc;
...
void
Options::checkForClassic(int argc, const char *argv[])
{
	// scan options
...
void
Options::gotoClassicLinker(int argc, const char *argv[])
{
	argv[0] = "ld_classic";
...
// to use it long term.
const char *
Options::demangleSymbol(const char *sym) const
{
...
void
Options::writeDependencyInfo() const
{
...
void
Options::addDependency(uint8_t opcode, const char *path) const
{
...
void
Options::writeToTraceFile(const char *buffer, size_t len) const
{
...
uint64_t
Options::machHeaderVmAddr() const
{
...
bool
Options::fromSDK(const char *path) const
{
	for (const char *sdkPath : fSDKPaths)
	{
