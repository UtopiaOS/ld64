...
namespace ld
{
	namespace tool
	{
		...
		SectionBoundaryAtom *
		SectionBoundaryAtom::makeSectionBoundaryAtom(const char *name, bool start, const char *segSectName, const Options	&opts)
		{
		...
		SectionBoundaryAtom *
		SectionBoundaryAtom::makeOldSectionBoundaryAtom(const char *name, bool start)
		{
			...
			warning("grandfathering in old symbol '%s' as alias for 'section$%s$%s$%s'", name, start ? "start" : "end", segName,
			        sectName);
		...
		SegmentBoundaryAtom *
		SegmentBoundaryAtom::makeSegmentBoundaryAtom(const char *name, bool start, const char *segName)
		{
			if (*segName == '\0')
			{
		...
		SegmentBoundaryAtom *
		SegmentBoundaryAtom::makeOldSegmentBoundaryAtom(const char *name, bool start)
		{
			// e.g. __DATA__begin
			char temp[18];
			strlcpy(temp, name, 7);
		...
		void
		Resolver::doLinkerOption(const std::vector<const char *> &linkerOption, const char *fileName)
		{
		...
		void
		Resolver::doFile(const ld::File &file)
		{
			...
			if ((atom.section().type() == ld::Section::typeInitializerPointers) && !_havellvmProfiling)
			{
				switch (_options.initializersTreatment())
				{
					...
					case Options::kWarning:
						warning("static initializer found in '%s'. Use -no_inits to make this an error.  Use -no_warn_inits to suppress warning",
						        atom.safeFilePath());
						break;
					default:
						break;
		...
		bool
		Resolver::isDtraceProbe(ld::Fixup::Kind kind)
		{
			switch (kind)
			{
		...
		void
		Resolver::convertReferencesToIndirect(const ld::Atom &atom)
		{
			// convert references by-name or by-content to by-slot
		...
		void
		Resolver::markLive(const ld::Atom &atom, WhyLiveBackChain *previous)
		{
		...
		void
		Resolver::deadStripOptimize(bool force)
		{
			...
			{
				for (std::vector<const ld::Atom *>::iterator it = _dontDeadStripIfReferencesLive.begin();
				        it != _dontDeadStripIfReferencesLive.end(); ++it)
				{
		...
		void
		Resolver::remainingUndefines(std::vector<const char *> &undefs)
		{
			StringSet  undefSet;
		...
		void
		Resolver::liveUndefines(std::vector<const char *> &undefs)
		{
			StringSet  undefSet;
		...
		bool
		Resolver::printReferencedBy(const char *name, SymbolTable::IndirectBindingSlot slot)
		{
		...
		void
		Resolver::checkUndefines(bool force)
		{
			...
			{
				this->remainingUndefines(
				    unresolvableUndefines);    // <rdar://problem/10052396> LTO may have eliminated need for some undefines
			}
			else
			{
			...
			if (_options.hasExportMaskList())
			{
				unresolvableUndefines.erase(std::remove_if(unresolvableUndefines.begin(), unresolvableUndefines.end(),
				                            ExportedObjcClass(_options)), unresolvableUndefines.end());
			}
			// hack to temporarily make missing aliases a warning
			if (_options.haveCmdLineAliases())
			{
				unresolvableUndefines.erase(std::remove_if(unresolvableUndefines.begin(), unresolvableUndefines.end(),
				                            UndefinedAlias(_options)), unresolvableUndefines.end());
			}
		...
		void
		Resolver::checkDylibSymbolCollisions()
		{
		...
		void
		Resolver::fillInHelpersInInternalState()
		{
		...
		void
		Resolver::fillInEntryPoint()
		{
			_internal.entryPoint = this->entryPoint(true);
		}

		void
		Resolver::syncAliases()
		{
		...
		void
		Resolver::removeCoalescedAwayAtoms()
		{
			const bool log = false;
			if (log)
			{
		...
		Resolver::linkTimeOptimize()
		{
			...
			for (std::vector<const AliasAtom *>::const_iterator it = _aliasesFromCmdLine.begin(); it != _aliasesFromCmdLine.end();
			        ++it)
			{
		...
		Resolver::tweakWeakness()
		{
			...
			{
				for (std::vector<ld::Internal::FinalSection *>::iterator sit = _internal.sections.begin();
				        sit != _internal.sections.end(); ++sit)
				{
		...
		void
		Resolver::checkChainedFixupsBounds()
		{
