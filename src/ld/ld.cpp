...
class InternalState : public ld::Internal
{
	public:
		...
		~InternalState() {}
	private:
		bool									inMoveRWChain(const ld::Atom &atom, const char *filePath, bool followedBackBranch, const char *&dstSeg,
		        bool &wildCardMatch);
...
ld::Section	InternalState::FinalSection::_s_DATA_nl_symbol_ptr("__DATA", "__nl_symbol_ptr",
...
size_t
InternalState::SectionHash::operator()(const ld::Section *sect) const
{
	size_t hash = 0;
	ld::CStringHash temp;
...
bool
InternalState::SectionEquals::operator()(const ld::Section *left, const ld::Section *right) const
{
	return (*left == *right);
}
...
const ld::Section &
InternalState::FinalSection::outputSection(const ld::Section &sect, bool mergeZeroFill)
{
...
const ld::Section &
InternalState::FinalSection::objectOutputSection(const ld::Section &sect, const Options &options)
{
...
uint32_t
InternalState::FinalSection::segmentOrder(const ld::Section &sect, const Options &options)
{
	...
	{
		bool armCloseStubs = (options.architecture() == CPU_TYPE_ARM) && !options.sharedRegionEligible()
		                     && !options.makeEncryptable() && !options.makeChainedFixups();
		if (strcmp(sect.segmentName(), "__PAGEZERO") == 0)
		{
			return 0;
		}
...
uint32_t
InternalState::FinalSection::sectionOrder(const ld::Section &sect, uint32_t sectionsSeen, const Options &options)
{
	...
	if (((options.outputKind() == Options::kPreload) || (options.outputKind() == Options::kDyld) || options.isKernel())
	        && (sectionList != NULL))
	{
		uint32_t count = 10;
...
#ifndef NDEBUG
static void
validateFixups(const ld::Atom &atom)
{
...
bool
InternalState::hasReferenceToWeakExternal(const ld::Atom &atom)
{
...
bool
InternalState::inMoveRWChain(const ld::Atom &atom, const char *filePath, bool followedBackBranch, const char *&dstSeg,
                             bool &wildCardMatch)
{
	...
	for (ld::Fixup::iterator fit = atom.fixupsBegin(); fit != atom.fixupsEnd(); ++fit)
	{
		...
		else if (!followedBackBranch && (fit->kind == ld::Fixup::kindNoneGroupSubordinate)
...
bool
InternalState::inMoveAuthChain(const ld::Atom &atom, bool followedBackBranch, const char *&dstSeg)
{
...
InternalState::addAtom(const ld::Atom &atom)
{
	...
	if (atom.symbolTableInclusion() == ld::Atom::symbolTableIn)
	{
		...
		if (inMoveRWChain(atom, path, false, dstSeg, wildCardMatch))
		{
			        ...
			        && (sectType != ld::Section::typeDyldInfo))
			{
				if (!wildCardMatch)
				{
					warning("cannot move symbol '%s' from file %s to segment '%s' because symbol is not data (is %d)", atom.name(), path,
					        dstSeg, sectType);
				}
			}
			else
		...
		if ((fs == NULL) && inMoveROChain(atom, path, dstSeg, wildCardMatch))
		{
			        ...
			        && (sectType != ld::Section::typeUnclassified))
			{
				if (!wildCardMatch)
				{
					warning("cannot move symbol '%s' from file %s to segment '%s' because symbol is not code (is %d)", atom.name(), path,
					        dstSeg, sectType);
				}
			}
			else
	...
	for (const Options::SectionRename &rename : _options.sectionRenames())
	{
		...
		{
			if (_options.useDataConstSegment() && _options.sharedRegionEligible() && (strcmp(curSectName, "__const") == 0)
			        && (strcmp(curSegName, "__DATA") == 0) && hasReferenceToWeakExternal(atom))
			{
				// if __DATA,__const atom has pointer to weak external symbol, don't move to __DATA_CONST
			...
			else if (_options.useDataConstSegment() && _options.sharedRegionEligible()
			         && (sectType == ld::Section::typeNonLazyPointer) && hasReferenceToWeakExternal(atom))
			{
...
ld::Internal::FinalSection *
InternalState::getFinalSection(const char *seg, const char *sect, ld::Section::Type type)
{
...
ld::Internal::FinalSection *
InternalState::getFinalSection(const ld::Section &inputSection)
{
...
void
InternalState::sortSections()
{
	//fprintf(stderr, "UNSORTED final sections:\n");
	//for (std::vector<ld::Internal::FinalSection*>::iterator it = sections.begin(); it != sections.end(); ++it) {
	//	fprintf(stderr, "final section %p %s/%s\n", (*it), (*it)->segmentName(), (*it)->sectionName());
	//}
	std::stable_sort(sections.begin(), sections.end(), [](const ld::Internal::FinalSection * l,
	                 const ld::Internal::FinalSection * r)
	{
	...
	assert((sections[0]->type() == ld::Section::typeMachHeader)
	       ...
	       || ((sections[0]->type() == ld::Section::typePageZero) && (sections[1]->type() == ld::Section::typeFirstSection)
	           && (sections[2]->type() == ld::Section::typeMachHeader)));
}


bool
InternalState::hasZeroForFileOffset(const ld::Section *sect)
{
	switch (sect->type())
	{
		case ld::Section::typeZeroFill:
...
uint64_t
InternalState::pageAlign(uint64_t addr)
{
...
uint64_t
InternalState::pageAlign(uint64_t addr, uint64_t pageSize)
{
	return ((addr + pageSize - 1) & (-pageSize));
}

void
InternalState::setSectionSizesAndAlignments()
{
...
uint64_t
InternalState::assignFileOffsets()
{
	...
	if (log)
	{
		fprintf(stderr, "Fixed address segments:\n");
	}
	for (std::vector<ld::Internal::FinalSection *>::iterator it = sections.begin(); it != sections.end(); ++it)
	{
		ld::Internal::FinalSection *sect = *it;
		...
		if (log)
			fprintf(stderr, "  address=0x%08llX, hidden=%d, alignment=%02d, section=%s,%s\n",
	...
	if (haveFixedSegments && !_options.segmentOrder().empty())
	{
		if (log)
		{
			fprintf(stderr, "After Fixed address segments:\n");
		}
		lastSegName = "";
	...
	if (log)
	{
		fprintf(stderr, "Regular layout segments:\n");
	}
	for (std::vector<ld::Internal::FinalSection *>::iterator it = sections.begin(); it != sections.end(); ++it)
	{
		ld::Internal::FinalSection *sect = *it;
		...
		if ((prevSect != NULL) && (prevSect->type() == ld::Section::typeFirstSection)
		        && (strcmp(prevSect->segmentName(), sect->segmentName()) == 0))
		{
		...
		if (log)
			fprintf(stderr, "  address=0x%08llX, size=0x%08llX, hidden=%d, alignment=%02d, padBytes=%d, section=%s,%s\n",
	...
	if (log)
	{
		fprintf(stderr, "All segments with file offsets:\n");
	}
	for (std::vector<ld::Internal::FinalSection *>::iterator it = sections.begin(); it != sections.end(); ++it)
	{
		ld::Internal::FinalSection *sect = *it;
		...
		if (log)
			fprintf(stderr, "  fileoffset=0x%08llX, address=0x%08llX, hidden=%d, size=%lld, alignment=%02d, section=%s,%s\n",
...
static char *
commatize(uint64_t in, char *out)
{
	char *result = out;
	char rawNum[30];
...
static void
printTime(const char *msg, uint64_t partTime, uint64_t totalTime)
{
...
printTime(const char *msg, uint64_t partTime, uint64_t totalTime)
{
	...
	if (partTime < sUnitsPerSecond)
	{
		...
		fprintf(stderr, "%24s: % 4d.%d milliseconds (% 4d.%d%%)\n", msg, milliSeconds, milliSecondsTimeTen - milliSeconds * 10,
		        percent, percentTimesTen - percent * 10);
	}
	else
	{
		...
		uint32_t percentTimesTen = (partTime * 1000) / totalTime;
		uint32_t percent = percentTimesTen / 10;
		fprintf(stderr, "%24s: % 4d.%d seconds (% 4d.%d%%)\n", msg, seconds, secondsTimeTen - seconds * 10, percent,
		        percentTimesTen - percent * 10);
	}
}


static void
getVMInfo(vm_statistics_data_t &info)
{
...
main(int argc, const char *argv[])
{
	...
	try
	{
		...
		if (options.printStatistics())
		{
			...
			char temp[40];
			fprintf(stderr, "processed %3u object files,  totaling %15s bytes\n", inputFiles._totalObjectLoaded,
			        commatize(inputFiles._totalObjectSize, temp));
			fprintf(stderr, "processed %3u archive files, totaling %15s bytes\n", inputFiles._totalArchivesLoaded,
			        commatize(inputFiles._totalArchiveSize, temp));
