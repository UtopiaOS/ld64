...
printFlags(uint32_t flags)
{
	...
	{
		uint32_t flag = 1 << i;
		if (flag & objc_image_info::SwiftVersionMask)
		{
			continue;
		}
		if (flags & flag)
...
main(int argc, const char *argv[])
{
	...
	for (; i < argc; i++)
	{
		if (!processFile(argv[i], set, clear))
		{
			return 1;
		}
	}
...
static bool
segnameEquals(const char *lhs, const char *rhs)
{
	return 0 == strncmp(lhs, rhs, 16);
}

static bool
segnameStartsWith(const char *segname, const char *prefix)
{
	return 0 == strncmp(segname, prefix, strlen(prefix));
}

static bool
sectnameEquals(const char *lhs, const char *rhs)
{
...
template <typename P>
static void
dosect(const char *filename, uint8_t *start, macho_section<P> *sect,
...
template<typename P>
static bool
parse_macho(const char *filename, uint8_t *buffer,
            uint32_t set, uint32_t clear)
{
...
static bool
parse_fat(const char *filename, uint8_t *buffer, size_t size,
          uint32_t set, uint32_t clear)
{
	uint32_t magic;
...
static bool
processFile(const char *filename, uint32_t set, uint32_t clear)
{
	if (debug)
	{
		printf("file %s\n", filename);
