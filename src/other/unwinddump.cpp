...
class UnwindPrinter
{
	public:
		...
		static UnwindPrinter<A>						*
		make(const uint8_t *fileContent, uint32_t fileLength,
...
template <>	 const char	*
UnwindPrinter<x86>::archName()
{
	return "i386";
}
template <>	 const char	*
UnwindPrinter<x86_64>::archName()
{
	return "x86_64";
}
template <>	 const char	*
UnwindPrinter<arm>::archName()
{
	return "arm";
}
#if SUPPORT_ARCH_arm64
template <>	 const char	*
UnwindPrinter<arm64>::archName()
{
	return "arm64";
}
#endif
...
template <>	 const char	*
UnwindPrinter<arm64_32>::archName()
{
	return "arm64_32";
}
#endif
...
template <>
bool
UnwindPrinter<x86>::validFile(const uint8_t *fileContent)
{
...
template <>
bool
UnwindPrinter<x86_64>::validFile(const uint8_t *fileContent)
{
	const macho_header<P> *header = (const macho_header<P> *)fileContent;
...
template <>
bool
UnwindPrinter<arm64>::validFile(const uint8_t *fileContent)
{
	const macho_header<P> *header = (const macho_header<P> *)fileContent;
	if (header->magic() != MH_MAGIC_64)
	{
...
template <>
bool
UnwindPrinter<arm64_32>::validFile(const uint8_t *fileContent)
{
	const macho_header<P> *header = (const macho_header<P> *)fileContent;
	if (header->magic() != MH_MAGIC)
	{
...
template <>
bool
UnwindPrinter<arm>::validFile(const uint8_t *fileContent)
{
	const macho_header<P> *header = (const macho_header<P> *)fileContent;
	if (header->magic() != MH_MAGIC)
	{
...
template <typename A>
void
UnwindPrinter<A>::getSymbolTableInfo()
{
...
template <typename A>
const char *
UnwindPrinter<A>::functionName(pint_t addr, uint32_t *offset)
{
...
template <typename A>
bool
UnwindPrinter<A>::findUnwindSection()
{
...
template <>
void
UnwindPrinter<x86_64>::decode(uint32_t encoding, const uint8_t *funcStart, char *str)
{
	*str = '\0';
...
template <>
void
UnwindPrinter<x86>::decode(uint32_t encoding, const uint8_t *funcStart, char *str)
{
	*str = '\0';
...
template <>
void
UnwindPrinter<arm64>::decode(uint32_t encoding, const uint8_t *funcStart, char *str)
{
	uint32_t stackSize;
...
template <>
void
UnwindPrinter<arm64_32>::decode(uint32_t encoding, const uint8_t *funcStart, char *str)
{
	uint32_t stackSize;
	switch (encoding & UNWIND_ARM64_MODE_MASK)
	{
...
template <>
void
UnwindPrinter<arm>::decode(uint32_t encoding, const uint8_t *funcStart, char *str)
{
	*str = '\0';
...
template <>
const char *
UnwindPrinter<x86_64>::personalityName(const macho_relocation_info<x86_64::P> *reloc)
{
...
template <>
const char *
UnwindPrinter<x86>::personalityName(const macho_relocation_info<x86::P> *reloc)
{
	//assert(reloc->r_extern() && "reloc not extern on personality column in __compact_unwind section");
...
template <>
const char *
UnwindPrinter<arm64>::personalityName(const macho_relocation_info<arm64::P> *reloc)
{
	//assert(reloc->r_extern() && "reloc not extern on personality column in __compact_unwind section");
...
template <>
const char *
UnwindPrinter<arm64_32>::personalityName(const macho_relocation_info<arm64_32::P> *reloc)
{
	//assert(reloc->r_extern() && "reloc not extern on personality column in __compact_unwind section");
	//assert((reloc->r_type() == ARM64_RELOC_UNSIGNED) && "wrong reloc type on personality column in __compact_unwind section");
	const macho_nlist<P> &sym = fSymbols[reloc->r_symbolnum()];
...
template <>
const char *
UnwindPrinter<arm>::personalityName(const macho_relocation_info<arm::P> *reloc)
{
	//assert(reloc->r_extern() && "reloc not extern on personality column in __compact_unwind section");
	//assert((reloc->r_type() == GENERIC_RELOC_VANILLA) && "wrong reloc type on personality column in __compact_unwind section");
	const macho_nlist<P> &sym = fSymbols[reloc->r_symbolnum()];
...
template <typename A>
bool
UnwindPrinter<A>::hasExernReloc(uint64_t sectionOffset, const char **personalityStr, pint_t *addr)
{
...
template <typename A>
void
UnwindPrinter<A>::printObjectUnwindSection(bool showFunctionNames)
{
	...
	const macho_compact_unwind_entry<P> *const entriesStart = (macho_compact_unwind_entry<P> *)((
	            uint8_t *)fHeader + fUnwindSection->offset());
	const macho_compact_unwind_entry<P> *const entriesEnd = (macho_compact_unwind_entry<P> *)((
	...
	for (const macho_compact_unwind_entry<P> *entry = entriesStart; entry < entriesEnd; ++entry)
	{
		...
		if (hasExernReloc(((char *)entry - (char *)entriesStart) + macho_compact_unwind_entry<P>::codeStartFieldOffset(),
		                  &functionNameStr, &funcAddress))
		{
		...
		{
			printf("  start:        0x%08llX   %s+0x%X\n", (uint64_t)funcAddress + offsetInFunction, functionNameStr,
			       offsetInFunction);
		}
		printf("  end:          0x%08llX   (len=0x%08X)\n", (uint64_t)(funcAddress + offsetInFunction + entry->codeLen()),
		       entry->codeLen());
		...
		if (hasExernReloc(((char *)entry - (char *)entriesStart) + macho_compact_unwind_entry<P>::personalityFieldOffset(),
		                  &personalityNameStr))
		{
...
template <typename A>
void
UnwindPrinter<A>::printUnwindSection(bool showFunctionNames)
{
	...
	macho_unwind_info_section_header_index_entry<P> *indexes = (macho_unwind_info_section_header_index_entry<P> *)
	        &sectionContent[sectionHeader->indexSectionOffset()];
	...
	uint32_t lsdaIndexArrayCount = (lsdaIndexArrayEndSectionOffset - lsdaIndexArraySectionOffset) / sizeof(
	                                   macho_unwind_info_section_header_lsda_index_entry<P>);
	printf("\tLSDA table: (section offset 0x%08X, count=%u)\n", lsdaIndexArraySectionOffset, lsdaIndexArrayCount);
	macho_unwind_info_section_header_lsda_index_entry<P> *lindex = (macho_unwind_info_section_header_lsda_index_entry<P> *)
	        &sectionContent[lsdaIndexArraySectionOffset];
	...
	for (uint32_t i = 0; i < sectionHeader->indexCount() - 1; ++i)
	{
		printf("\tsecond level index[%u] sectionOffset=0x%08X, count=%u, fileOffset=0x%08X\n", i,
		       indexes[i].secondLevelPagesSectionOffset(),
		...
		macho_unwind_info_regular_second_level_page_header<P> *page = (macho_unwind_info_regular_second_level_page_header<P> *)
		        &sectionContent[indexes[i].secondLevelPagesSectionOffset()];
		if (page->kind() == UNWIND_SECOND_LEVEL_REGULAR)
		{
			...
			const macho_unwind_info_regular_second_level_entry<P> *entry = (macho_unwind_info_regular_second_level_entry<P> *)((
			            char *)page + page->entryPageOffset());
		...
		{
			macho_unwind_info_compressed_second_level_page_header<P> *cp = (macho_unwind_info_compressed_second_level_page_header<P>
			        *)page;
...
static void
dump(const char *path, const std::set<cpu_type_t> &onlyArchs, bool showFunctionNames)
{
