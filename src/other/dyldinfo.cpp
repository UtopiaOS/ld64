...
class DyldInfoPrinter
{
	public:
		...
		static DyldInfoPrinter<A>					*
		make(const uint8_t *fileContent, uint32_t fileLength, const char *path, bool printArch)
		{
...
template <>
bool
DyldInfoPrinter<ppc>::validFile(const uint8_t *fileContent)
{
...
template <>
bool
DyldInfoPrinter<ppc64>::validFile(const uint8_t *fileContent)
{
	const macho_header<P> *header = (const macho_header<P> *)fileContent;
...
template <>
bool
DyldInfoPrinter<x86>::validFile(const uint8_t *fileContent)
{
	const macho_header<P> *header = (const macho_header<P> *)fileContent;
	if (header->magic() != MH_MAGIC)
	{
...
template <>
bool
DyldInfoPrinter<x86_64>::validFile(const uint8_t *fileContent)
{
	const macho_header<P> *header = (const macho_header<P> *)fileContent;
	if (header->magic() != MH_MAGIC_64)
	{
...
template <>
bool
DyldInfoPrinter<arm>::validFile(const uint8_t *fileContent)
{
	const macho_header<P> *header = (const macho_header<P> *)fileContent;
	if (header->magic() != MH_MAGIC)
	{
...
template <>
bool
DyldInfoPrinter<arm64>::validFile(const uint8_t *fileContent)
{
	const macho_header<P> *header = (const macho_header<P> *)fileContent;
	if (header->magic() != MH_MAGIC_64)
	{
...
template <>
bool
DyldInfoPrinter<arm64_32>::validFile(const uint8_t *fileContent)
{
	const macho_header<P> *header = (const macho_header<P> *)fileContent;
	if (header->magic() != MH_MAGIC)
	{
...
template <typename A>
const char *
DyldInfoPrinter<A>::rebaseTypeName(uint8_t type)
{
	switch (type)
	{
...
template <typename A>
const char *
DyldInfoPrinter<A>::bindTypeName(uint8_t type)
{
	switch (type)
	{
...
template <typename A>
typename A::P::uint_t
DyldInfoPrinter<A>::segStartAddress(uint8_t segIndex)
{
...
template <typename A>
const char *
DyldInfoPrinter<A>::segmentName(uint8_t segIndex)
{
	if (segIndex > fSegments.size())
	{
		throw "segment index out of range";
...
template <typename A>
const char *
DyldInfoPrinter<A>::sectionName(uint8_t segIndex, pint_t address)
{
	if (segIndex > fSegments.size())
	{
		throw "segment index out of range";
...
template <typename A>
const char *
DyldInfoPrinter<A>::getSegAndSectName(uint8_t segIndex, pint_t address)
{
...
template <typename A>
uint8_t
DyldInfoPrinter<A>::segmentIndexForAddress(pint_t address)
{
...
template <typename A>
typename A::P::uint_t	*
DyldInfoPrinter<A>::mappedAddressForVMAddress(pint_t vmaddress)
{
	for (unsigned int i = 0; i < fSegments.size(); ++i)
	{
...
template <typename A>
const char *
DyldInfoPrinter<A>::ordinalName(int libraryOrdinal)
{
...
template <typename A>
const char *
DyldInfoPrinter<A>::classicOrdinalName(int libraryOrdinal)
{
...
template <typename A>
void
DyldInfoPrinter<A>::printRebaseInfo()
{
								...
								do
								{
									...
									if (isRebase)
									{
										...
										if (isAuthenticated)
										{
											...
											printf("%-7s %-16s 0x%08llX  %s  0x%08llX (JOP: diversity %d, address %s, %s)\n", segName, sectionName(segIndex,
											        segStartAddr + segOffset), segStartAddr + segOffset, typeName, targetValue, diversity,
...
#endif
										{
											...
											printf("%-7s %-16s 0x%08llX  %s  0x%08llX\n", segName, sectionName(segIndex, segStartAddr + segOffset),
											       segStartAddr + segOffset, typeName, targetValue);
										}
									}
			...
			switch (opcode)
			{
				...
				case REBASE_OPCODE_DO_REBASE_IMM_TIMES:
					for (int i = 0; i < immediate; ++i)
					{
						...
						printf("%-7s %-16s 0x%08llX  %s  0x%08llX\n", segName, sectionName(segIndex, segStartAddr + segOffset),
						       segStartAddr + segOffset, typeName, value);
...
template <typename A>
void
DyldInfoPrinter<A>::printRebaseInfoOpcodes()
{
	if ((fInfo == NULL) || (fInfo->rebase_off() == 0))
	{
		printf("no compressed rebase info\n");
...
template <typename A>
void
DyldInfoPrinter<A>::printBindingInfoOpcodes(bool weakbinding)
{
	if (fInfo == NULL)
	{
...
struct ThreadedBindData
{
	...
	std::tuple<const char *, int64_t, int, uint8_t, uint8_t>
	pack() const
	{
...
template <typename A>
void
DyldInfoPrinter<A>::printBindingInfo()
{
				...
				case BIND_OPCODE_DO_BIND:
					if (!useThreadedRebaseBind)
					{
						printf("%-7s %-16s 0x%08llX %10s  %5lld %-16s %s%s\n", segName, sectionName(segIndex, segStartAddr + segOffset),
						       segStartAddr + segOffset, typeName, addend, fromDylib, symbolName, weak_import);
						segOffset += sizeof(pint_t);
					}
					else
					{
								...
								else
								{
									...
									if (isAuthenticated)
									{
										...
										printf("%-7s %-16s 0x%08llX %10s  %5lld %-16s %s%s with value 0x%016llX (JOP: diversity %d, address %s, %s)\n", segName,
										       sectionName(segIndex, segStartAddr + segOffset), segStartAddr + segOffset, typeName, addend, fromDylib, symbolName,
										       weak_import, value, diversity, hasAddressDiversity ? "true" : "false", keyNames[key]);
									}
									else
#endif
									{
										printf("%-7s %-16s 0x%08llX %10s  %5lld %-16s %s%s with value 0x%016llX\n", segName, sectionName(segIndex,
										        segStartAddr + segOffset), segStartAddr + segOffset, typeName, addend, fromDylib, symbolName, weak_import, value);
									}
								}
								// The delta is bits [51..61]
								// And bit 62 is to tell us if we are a rebase (0) or bind (1)
...
template <typename A>
void
DyldInfoPrinter<A>::printWeakBindingInfo()
{
			...
			switch (opcode)
			{
				...
				case BIND_OPCODE_DO_BIND:
					printf("%-7s %-16s 0x%08llX %10s   %5lld %s\n", segName, sectionName(segIndex, segStartAddr + segOffset),
					       segStartAddr + segOffset, typeName, addend, symbolName);
					segOffset += sizeof(pint_t);
					break;
				case BIND_OPCODE_DO_BIND_ADD_ADDR_ULEB:
					printf("%-7s %-16s 0x%08llX %10s   %5lld %s\n", segName, sectionName(segIndex, segStartAddr + segOffset),
...
template <typename A>
void
DyldInfoPrinter<A>::printLazyBindingInfo()
{
	if (fInfo == NULL)
	{
			...
			switch (opcode)
			{
				...
				case BIND_OPCODE_DO_BIND:
					printf("%-7s %-16s 0x%08llX 0x%04X %-16s %s%s\n", segName, sectionName(segIndex, segStartAddr + segOffset),
					       segStartAddr + segOffset, lazy_offset, fromDylib, symbolName, weak_import);
					segOffset += sizeof(pint_t);
					break;
				default:
...
template <typename A>
void
DyldInfoPrinter<A>::printLazyBindingOpcodes()
{
	if (fInfo == NULL)
	{
		printf("no compressed dyld info\n");
...
{
	bool
	operator()(const mach_o::trie::Entry &left, const mach_o::trie::Entry &right)
	{
...
template <typename A>
void
DyldInfoPrinter<A>::printExportInfo()
{
...
template <typename A>
void
DyldInfoPrinter<A>::processExportGraphNode(const uint8_t *const start, const uint8_t *const end,
        const uint8_t *parent, const uint8_t *p,
        char *cummulativeString, int curStrOffset)
{
...
template <typename A>
void
DyldInfoPrinter<A>::printExportInfoGraph()
{
	if ((fInfo == NULL) || (fInfo->export_off() == 0))
	{
		printf("no compressed export info\n");
...
template <typename A>
void
DyldInfoPrinter<A>::gatherNodeStarts(const uint8_t *const start, const uint8_t *const end,
                                     const uint8_t *parent, const uint8_t *p,
                                     std::vector<uint32_t> &nodeStarts)
{
...
template <typename A>
void
DyldInfoPrinter<A>::printExportInfoNodes()
{
	if ((fInfo == NULL) || (fInfo->export_off() == 0))
	{
		printf("no compressed export info\n");
...
template <typename A>
const uint8_t *
DyldInfoPrinter<A>::printSharedRegionV1InfoForEachULEB128Address(const uint8_t *p, const uint8_t *end, uint8_t kind)
{
	const char *kindStr =  "??";
	switch (kind)
	{
...
template <typename A>
void
DyldInfoPrinter<A>::printSharedRegionInfo()
{
	...
	else
	{
		...
		if (*infoStart == DYLD_CACHE_ADJ_V2_FORMAT)
		{
			...
			for (uint64_t i = 0; i < sectionCount; ++i)
			{
				...
				printf("from sect=%s/%s, to sect=%s/%s, count=%lld:\n", fromSection->segname(), fromSectionName, toSection->segname(),
				       toSection->sectname(), toOffsetCount);
				...
				for (uint64_t j = 0; j < toOffsetCount; ++j)
				{
					...
					for (uint64_t k = 0; k < fromOffsetCount; ++k)
					{
						...
						for (uint64_t l = 0; l < fromSectDeltaCount; ++l)
						{
							...
							printf("       from addr=0x%0llX %s to addr=0x%0llX", fromSection->addr() + fromSectionOffset,
							       sharedRegionKindName(kind), toSection->addr() + toSectionOffset);
							if (toSymbol != NULL)
							{
								if (symbolOffset == 0)
								{
...
template <typename A>
const char *
DyldInfoPrinter<A>::sharedRegionKindName(uint8_t kind)
{
	switch (kind)
	{
		default:
...
template <>
void
DyldInfoPrinter<arm>::printFunctionStartLine(uint64_t addr)
{
	if (addr & 1)
	{
...
template <typename A>
void
DyldInfoPrinter<A>::printFunctionStartLine(uint64_t addr)
{
	printf("0x%0llX   %s\n", addr, symbolNameForAddress(addr));
}
...
template <typename A>
void
DyldInfoPrinter<A>::printFunctionStartsInfo()
{
...
template <typename A>
void
DyldInfoPrinter<A>::printDylibsInfo()
{
	printf("attributes     dependent dylibs\n");
	for (typename std::vector<const macho_dylib_command<P>*>::iterator it = fDylibLoadCommands.begin();
	        it != fDylibLoadCommands.end(); ++it)
	{
...
template <typename A>
void
DyldInfoPrinter<A>::printDataInCode()
{
	if (fDataInCode == NULL)
	{
	...
	else
	{
		...
		const macho_data_in_code_entry<P> *end = (macho_data_in_code_entry<P> *)((uint8_t *)fHeader + fDataInCode->dataoff() +
		        fDataInCode->datasize());
...
template <>
x86_64::P::uint_t
DyldInfoPrinter<x86_64>::localRelocBase()
{
...
template <typename A>
typename A::P::uint_t
DyldInfoPrinter<A>::localRelocBase()
{
	return fFirstSegment->vmaddr();
}
...
template <>
x86_64::P::uint_t
DyldInfoPrinter<x86_64>::externalRelocBase()
{
	if (fHeader->filetype() == MH_KEXT_BUNDLE)
	{
		// for kext bundles the reloc base address starts at __TEXT segment
...
template <typename A>
typename A::P::uint_t
DyldInfoPrinter<A>::externalRelocBase()
{
	return 0;
}
...
template <>
const char	*
DyldInfoPrinter<ppc>::relocTypeName(uint8_t r_type)
{
...
template <>
const char	*
DyldInfoPrinter<ppc64>::relocTypeName(uint8_t r_type)
{
	if (r_type == GENERIC_RELOC_VANILLA)
	{
		return "pointer";
...
template <>
const char	*
DyldInfoPrinter<x86>::relocTypeName(uint8_t r_type)
{
	if (r_type == GENERIC_RELOC_VANILLA)
	{
		return "pointer";
...
template <>
const char	*
DyldInfoPrinter<x86_64>::relocTypeName(uint8_t r_type)
{
...
template <>
const char	*
DyldInfoPrinter<arm>::relocTypeName(uint8_t r_type)
{
...
template <>
const char	*
DyldInfoPrinter<arm64>::relocTypeName(uint8_t r_type)
{
...
template <>
const char	*
DyldInfoPrinter<arm64_32>::relocTypeName(uint8_t r_type)
{
	if (r_type == ARM64_RELOC_UNSIGNED)
	{
		return "pointer";
...
template <typename A>
void
DyldInfoPrinter<A>::printRelocRebaseInfo()
{
	// First check if we can find a magic section for threaded rebase
	{
		auto rebaseChain = [this](uintptr_t chainStartMappedAddress, uintptr_t chainStartVMAddress, uint64_t stepMultiplier,
	...
	else
	{
		...
		const macho_relocation_info<P> *const relocsStart = (macho_relocation_info<P> *)(((uint8_t *)fHeader) +
		        fDynamicSymbolTable->locreloff());
		...
		uint8_t segIndex = 0;
		for (typename std::vector<const macho_segment_command<P>*>::iterator segit = fSegments.begin();
		        segit != fSegments.end(); ++segit, ++segIndex)
		{
...
template <typename A>
void
DyldInfoPrinter<A>::printSymbolTableExportInfo()
{
	if (fDynamicSymbolTable == NULL)
	{
		printf("no classic dynamic symbol table");
...
template <typename A>
const char *
DyldInfoPrinter<A>::closestSymbolNameForAddress(uint64_t addr, uint64_t *offset, uint8_t sectIndex)
{
...
template <typename A>
const char *
DyldInfoPrinter<A>::symbolNameForAddress(uint64_t addr)
{
	uint64_t offset;
...
template <typename A>
void
DyldInfoPrinter<A>::printClassicBindingInfo()
{
	if (fDynamicSymbolTable == NULL)
	{
		printf("no classic dynamic symbol table");
...
template <typename A>
void
DyldInfoPrinter<A>::printClassicLazyBindingInfo()
{
	if (fDynamicSymbolTable == NULL)
	{
		printf("no classic dynamic symbol table");
