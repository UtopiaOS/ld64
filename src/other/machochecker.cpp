...
class MachOChecker
{
	public:
		...
		static MachOChecker<A>						*
		make(const uint8_t *fileContent, uint32_t fileLength, const char *path,
...
template <>
bool
MachOChecker<x86>::validFile(const uint8_t *fileContent)
{
...
template <>
bool
MachOChecker<x86_64>::validFile(const uint8_t *fileContent)
{
	const macho_header<P> *header = (const macho_header<P> *)fileContent;
...
template <>
bool
MachOChecker<arm>::validFile(const uint8_t *fileContent)
{
	const macho_header<P> *header = (const macho_header<P> *)fileContent;
	if (header->magic() != MH_MAGIC)
	{
...
template <>
bool
MachOChecker<arm64>::validFile(const uint8_t *fileContent)
{
	const macho_header<P> *header = (const macho_header<P> *)fileContent;
	if (header->magic() != MH_MAGIC_64)
	{
...
template <>
bool
MachOChecker<arm64_32>::validFile(const uint8_t *fileContent)
{
	const macho_header<P> *header = (const macho_header<P> *)fileContent;
	if (header->magic() != MH_MAGIC)
	{
...
template <> uint8_t
MachOChecker<ppc>::loadCommandSizeMask()
{
	return 0x03;
}
template <> uint8_t
MachOChecker<ppc64>::loadCommandSizeMask()
{
	return 0x07;
}
template <> uint8_t
MachOChecker<x86>::loadCommandSizeMask()
{
	return 0x03;
}
template <> uint8_t
MachOChecker<x86_64>::loadCommandSizeMask()
{
	return 0x07;
}
template <> uint8_t
MachOChecker<arm>::loadCommandSizeMask()
{
	return 0x03;
}
template <> uint8_t
MachOChecker<arm64>::loadCommandSizeMask()
{
	return 0x07;
}
#if SUPPORT_ARCH_arm64_32
template <> uint8_t
MachOChecker<arm64_32>::loadCommandSizeMask()
{
	return 0x03;
}
#endif
...
template <>
x86::P::uint_t
MachOChecker<x86>::getInitialStackPointer(const macho_thread_command<x86::P> *threadInfo)
{
...
template <>
x86_64::P::uint_t
MachOChecker<x86_64>::getInitialStackPointer(const macho_thread_command<x86_64::P> *threadInfo)
{
	return threadInfo->thread_register(7);
}
...
template <>
arm::P::uint_t
MachOChecker<arm>::getInitialStackPointer(const macho_thread_command<arm::P> *threadInfo)
{
...
template <>
arm64::P::uint_t
MachOChecker<arm64>::getInitialStackPointer(const macho_thread_command<arm64::P> *threadInfo)
{
...
template <>
arm64_32::P::uint_t
MachOChecker<arm64_32>::getInitialStackPointer(const macho_thread_command<arm64_32::P> *threadInfo)
{
...
template <>
ppc::P::uint_t
MachOChecker<ppc>::getEntryPoint(const macho_thread_command<ppc::P> *threadInfo)
{
...
template <>
x86::P::uint_t
MachOChecker<x86>::getEntryPoint(const macho_thread_command<x86::P> *threadInfo)
{
...
template <>
x86_64::P::uint_t
MachOChecker<x86_64>::getEntryPoint(const macho_thread_command<x86_64::P> *threadInfo)
{
...
template <>
arm::P::uint_t
MachOChecker<arm>::getEntryPoint(const macho_thread_command<arm::P> *threadInfo)
{
...
template <>
arm64::P::uint_t
MachOChecker<arm64>::getEntryPoint(const macho_thread_command<arm64::P> *threadInfo)
{
	throw "LC_UNIXTHREAD not supported for arm64";
}
...
template <>
arm64_32::P::uint_t
MachOChecker<arm64_32>::getEntryPoint(const macho_thread_command<arm64_32::P> *threadInfo)
{
	throw "LC_UNIXTHREAD not supported for arm64_32";
}
#endif
...
MachOChecker<A>::MachOChecker(const uint8_t *fileContent, uint32_t fileLength, const char *path,
                              const char *verifierDstRoot, const std::vector<const char *> &mergeRootPaths)
	: fHeader(NULL), fLength(fileLength), fInstallName(NULL), fStrings(NULL), fSymbols(NULL), fSymbolCount(0),
	  fDynamicSymbolTable(NULL), fIndirectTableCount(0),
	  fLocalRelocations(NULL),  fLocalRelocationsCount(0),  fExternalRelocations(NULL),  fExternalRelocationsCount(0),
	  fWriteableSegmentWithAddrOver4G(false), fSlidableImage(false), fHasLC_RPATH(false), fIsDebugVariant(false),
	  fFirstSegment(NULL), fFirstWritableSegment(NULL),
...
template <typename A>
void
MachOChecker<A>::checkMachHeader()
{
...
template <typename A>
void
MachOChecker<A>::checkLoadCommands()
{
	...
	for (uint32_t i = 0; i < cmd_count; ++i)
	{
		...
		switch (cmd->cmd())
		{
			...
			case LC_ID_DYLIB:
				...
				{
					throwf("malformed mach-o: LC_ID_DYLIB load command has offset (%u) outside its size (%u)", dylibID->name_offset(),
					       size);
				}
	...
	for (uint32_t i = 0; i < cmd_count; ++i)
	{
		if (cmd->cmd() == macho_segment_command<P>::CMD)
		{
			...
			for (typename std::vector<std::pair<pint_t, pint_t> >::iterator it = segmentAddressRanges.begin();
			        it != segmentAddressRanges.end(); ++it)
			{
	...
	else if (entryPoint != NULL)
	{
		...
		if ((initialOffset < fTEXTSegment->fileoff())
		        || (initialOffset >= (fTEXTSegment->fileoff() + fTEXTSegment->filesize())))
		{
		...
		switch (cmd->cmd())
		{
			...
			case LC_DYSYMTAB:
			{
				...
				if (fIndirectTableCount != 0)
				{
					...
					if ((fDynamicSymbolTable->indirectsymoff() + fIndirectTableCount * 4) > (linkEditSegment->fileoff() +
				...
				if (fLocalRelocationsCount != 0)
				{
					...
					if ((fDynamicSymbolTable->locreloff() + fLocalRelocationsCount * sizeof(macho_relocation_info<P>)) >
				...
				if (fExternalRelocationsCount != 0)
				{
					...
					if ((fDynamicSymbolTable->extreloff() + fExternalRelocationsCount * sizeof(macho_relocation_info<P>)) >
					        (linkEditSegment->fileoff() + linkEditSegment->filesize()))
					{
						throw "external relocations not in __LINKEDIT";
					}
...
template <typename A>
void
MachOChecker<A>::checkSection(const macho_segment_command<P> *segCmd, const macho_section<P> *sect)
{
...
static bool
sharedCacheEligiblePath(const char *path)
{
...
template <typename A>
void
MachOChecker<A>::verifyInstallName()
{
...
template <typename A>
void
MachOChecker<A>::verifyNoFlatLookups()
{
...
template <typename A>
void
MachOChecker<A>::verifyNoDylibMain()
{
	if (fHeader->filetype() != MH_DYLIB)
	{
		return;
...
template <typename A>
void
MachOChecker<A>::checkIndirectSymbolTable()
{
		...
		if (cmd->cmd() == macho_segment_command<P>::CMD)
		{
			...
			for (const macho_section<P> *sect = sectionsStart; sect < sectionsEnd; ++sect)
			{
				...
				if (elementSize != 0)
				{
					...
					{
						throwf("%s section references beyond end of indirect symbol table (%d > %d)", sect->sectname(), start + count,
						       fIndirectTableCount);
					}
				}
			}
	...
	{
		throwf("start of local symbols out of range (%d > %d) in indirect symbol table", fDynamicSymbolTable->ilocalsym(),
		       fSymbolCount);
	}
	...
	{
		throwf("start of extern symbols out of range (%d > %d) in indirect symbol table", fDynamicSymbolTable->iextdefsym(),
		       fSymbolCount);
	}
	...
	{
		throwf("start of undefined symbols out of range (%d > %d) in indirect symbol table", fDynamicSymbolTable->iundefsym(),
		       fSymbolCount);
	}
...
template <typename A>
void
MachOChecker<A>::checkSymbolTable()
{
...
template <typename A>
void
MachOChecker<A>::checkThreadedRebaseBind()
{
	// look bind info
	if (fDyldInfo != NULL)
	{
...
template <>
arm64_32::P::uint_t
MachOChecker<arm64_32>::relocBase()
{
	return fFirstWritableSegment->vmaddr();
}
#endif
...
template <typename A>
bool
MachOChecker<A>::addressInWritableSegment(pint_t address)
{
	const macho_load_command<P> *const cmds = (macho_load_command<P> *)((uint8_t *)fHeader + sizeof(macho_header<P>));
	const uint32_t cmd_count = fHeader->ncmds();
	const macho_load_command<P> *cmd = cmds;
...
template <>
void
MachOChecker<x86>::checkExternalReloation(const macho_relocation_info<P> *reloc)
{
	if (reloc->r_length() != 2)
	{
...
template <>
void
MachOChecker<x86_64>::checkExternalReloation(const macho_relocation_info<P> *reloc)
{
	if (reloc->r_length() != 3)
	{
		throw "bad external relocation length";
...
template <>
void
MachOChecker<arm>::checkExternalReloation(const macho_relocation_info<P> *reloc)
{
	if (reloc->r_length() != 2)
	{
		throw "bad external relocation length";
...
template <>
void
MachOChecker<arm64>::checkExternalReloation(const macho_relocation_info<P> *reloc)
{
...
template <>
void
MachOChecker<arm64_32>::checkExternalReloation(const macho_relocation_info<P> *reloc)
{
...
template <>
void
MachOChecker<x86>::checkLocalReloation(const macho_relocation_info<P> *reloc)
{
	// FIX
}
...
template <>
void
MachOChecker<x86_64>::checkLocalReloation(const macho_relocation_info<P> *reloc)
{
	if (reloc->r_length() != 3)
	{
...
template <>
void
MachOChecker<arm>::checkLocalReloation(const macho_relocation_info<P> *reloc)
{
...
template <>
void
MachOChecker<arm64>::checkLocalReloation(const macho_relocation_info<P> *reloc)
{
...
template <>
void
MachOChecker<arm64_32>::checkLocalReloation(const macho_relocation_info<P> *reloc)
{
...
template <typename A>
void
MachOChecker<A>::checkRelocations()
{
...
template <typename A>
typename A::P::uint_t
MachOChecker<A>::segStartAddress(uint8_t segIndex)
{
...
template <typename A>
bool
MachOChecker<A>::hasTextRelocInRange(pint_t rangeStart, pint_t rangeEnd)
{
...
template <typename A>
bool
MachOChecker<A>::addressIsRebaseSite(pint_t targetAddr, pint_t &pointeeAddr)
{
	// look at local relocs
	const macho_relocation_info<P> *const localRelocsEnd = &fLocalRelocations[fLocalRelocationsCount];
	for (const macho_relocation_info<P> *reloc = fLocalRelocations; reloc < localRelocsEnd; ++reloc)
...
template <typename A>
bool
MachOChecker<A>::addressIsBindingSite(pint_t targetAddr)
{
...
static void
check(const char *path, const char *verifierDstRoot, const std::vector<const char *> &mergeRootPaths)
{
