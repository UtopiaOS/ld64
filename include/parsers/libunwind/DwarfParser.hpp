...
namespace libunwind
{
	...
	class CFI_Parser
	{
		public:
			...
			static bool findFDE(A &addressSpace, pint_t pc, pint_t ehSectionStart, uint32_t sectionLength, pint_t fdeHint,
			                    FDE_Info *fdeInfo, CIE_Info *cieInfo);
			static const char *decodeFDE(A &addressSpace, pint_t fdeStart, FDE_Info *fdeInfo, CIE_Info *cieInfo);
			static bool parseFDEInstructions(A &addressSpace, const FDE_Info &fdeInfo, const CIE_Info &cieInfo, pint_t upToPC,
			                                 PrologInfo *results);
	...
	template <typename A>
	const char *
	CFI_Parser<A>::decodeFDE(A &addressSpace, pint_t fdeStart, FDE_Info *fdeInfo, CIE_Info *cieInfo)
	{
		pint_t p = fdeStart;
	...
	template <typename A>
	bool
	CFI_Parser<A>::findFDE(A &addressSpace, pint_t pc, pint_t ehSectionStart, uint32_t sectionLength, pint_t fdeHint,
	                       FDE_Info *fdeInfo, CIE_Info *cieInfo)
	{
	...
	template <typename A>
	const char *
	CFI_Parser<A>::parseCIE(A &addressSpace, pint_t cie, CIE_Info *cieInfo)
	{
	...
	template <typename A>
	uint32_t
	CFI_Parser<A>::getCFICount(A &addressSpace, pint_t ehSectionStart, uint32_t sectionLength)
	{
		uint32_t count = 0;
	...
	template <typename A>
	const char *
	CFI_Parser<A>::getCFIs(A &addressSpace, pint_t ehSectionStart, uint32_t sectionLength,
	                       std::vector<FDE_Atom_Info> &fdes, std::vector<CIE_Atom_Info> &cies)
	{
		const pint_t ehSectionEnd = ehSectionStart + sectionLength;
		for (pint_t p = ehSectionStart; p < ehSectionEnd;)
	...
	template <typename A>
	bool
	CFI_Parser<A>::parseFDEInstructions(A &addressSpace, const FDE_Info &fdeInfo, const CIE_Info &cieInfo, pint_t upToPC,
	                                    PrologInfo *results)
	{
		// clear results
	...
	template <typename A>
	bool
	CFI_Parser<A>::parseInstructions(A &addressSpace, pint_t instructions, pint_t instructionsEnd, const CIE_Info &cieInfo,
	                                 pint_t pcoffset, PrologInfoStackEntry *&rememberStack, PrologInfo *results)
	{
		...
		while ((p < instructionsEnd) && (codeOffset < pcoffset))
		{
			...
			switch (opcode)
			{
				...
				case DW_CFA_advance_loc1:
					...
					if (logDwarf)
					{
						fprintf(stderr, "DW_CFA_advance_loc1: new offset=%u\n", codeOffset);
					}
					break;
				case DW_CFA_advance_loc2:
					...
					if (logDwarf)
					{
						fprintf(stderr, "DW_CFA_advance_loc2: new offset=%u\n", codeOffset);
					}
					break;
				case DW_CFA_advance_loc4:
					...
					if (logDwarf)
					{
						fprintf(stderr, "DW_CFA_advance_loc4: new offset=%u\n", codeOffset);
					}
					break;
				...
				case DW_CFA_def_cfa_expression:
					...
					p += length;
					if (logDwarf)
						fprintf(stderr, "DW_CFA_def_cfa_expression(expression=0x%llX, length=%llu)\n",
				...
				case DW_CFA_expression:
					...
					p += length;
					if (logDwarf)
						fprintf(stderr, "DW_CFA_expression(reg=%lld, expression=0x%llX, length=%llu)\n",
				...
				case DW_CFA_val_expression:
					...
					p += length;
					if (logDwarf)
						fprintf(stderr, "DW_CFA_val_expression(reg=%lld, expression=0x%llX, length=%lld)\n",
						        reg, results->savedRegisters[reg].value, length);
					break;
				...
				default:
					...
					switch (opcode & 0xC0)
					{
						...
						case DW_CFA_advance_loc:
							...
							if (logDwarf)
							{
								fprintf(stderr, "DW_CFA_advance_loc: new offset=%u\n", codeOffset);
							}
							break;
						case DW_CFA_restore:
