...
static volatile struct __DATA__dyld  myDyldSection
__attribute__((section("__DATA,__dyld"))) = { 0, NULL };


static int
my_dyld_func_lookup(const char *dyld_func_name, void **address)
{
...
bool
_dyld_find_unwind_sections(void *addr, dyld_unwind_sections *info)
{
...
namespace libunwind
{
	...
	inline uintptr_t
	LocalAddressSpace::getP(pint_t addr)
	{
#if __LP64__
		return get64(addr);
#else
	...
	inline bool
	LocalAddressSpace::findUnwindSections(pint_t addr, pint_t &mh, pint_t &dwarfStart, pint_t &dwarfLen,
	                                      pint_t &compactStart)
	{
	...
	inline bool
	LocalAddressSpace::findFunctionName(pint_t addr, char *buf, size_t bufLen, unw_word_t *offset)
	{
		dl_info dyldInfo;
	...
	template <typename P>
	uint8_t
	OtherAddressSpace<P>::get8(pint_t addr)
	{
	...
	template <typename P>
	uint16_t
	OtherAddressSpace<P>::get16(pint_t addr)
	{
	...
	template <typename P>
	uint32_t
	OtherAddressSpace<P>::get32(pint_t addr)
	{
	...
	template <typename P>
	uint64_t
	OtherAddressSpace<P>::get64(pint_t addr)
	{
	...
	template <typename P>
	typename P::uint_t
	OtherAddressSpace<P>::getP(pint_t addr)
	{
	...
	template <typename P>
	uint64_t
	OtherAddressSpace<P>::getULEB128(pint_t &addr, pint_t end)
	{
	...
	template <typename P>
	int64_t
	OtherAddressSpace<P>::getSLEB128(pint_t &addr, pint_t end)
	{
		uintptr_t size = (end - addr);
		LocalAddressSpace::pint_t laddr = (LocalAddressSpace::pint_t)localCopy(addr);
		LocalAddressSpace::pint_t sladdr = laddr;
	...
	template <typename P>
	void *
	OtherAddressSpace<P>::localCopy(pint_t addr)
	{
		// FIX ME
	}
	...
	template <typename P>
	bool
	OtherAddressSpace<P>::findFunctionName(pint_t addr, char *buf, size_t bufLen, unw_word_t *offset)
	{
		// FIX ME
	}
