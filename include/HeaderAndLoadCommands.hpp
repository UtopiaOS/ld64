	...
	namespace tool
	{
		...
		class HeaderAndLoadCommandsAtom : public HeaderAndLoadCommandsAbtract
		{
			public:
				...
				name() const
				{
					return "mach-o header and load commands";
				}
				...
				virtual void
				setUUID(const uint8_t digest[16])
				{
					memcpy(_uuid, digest, 16);
				}
			...
			private:
				...
				uint8_t					*copyBuildVersionLoadCommand(uint8_t *p, ld::Platform platform, uint32_t minVersion,
		...
		template <typename A>
		ld::Section HeaderAndLoadCommandsAtom<A>::_s_preload_section("__HEADER", "__mach_header", ld::Section::typeMachHeader,
		        true);
		...
		HeaderAndLoadCommandsAtom<A>::HeaderAndLoadCommandsAtom(const Options &opts, ld::Internal &state, OutputFile &writer)
			: HeaderAndLoadCommandsAbtract((opts.outputKind() == Options::kPreload) ? _s_preload_section : _s_section,
			  ...
			  _options(opts), _state(state), _writer(writer), _address(0), _uuidCmdInOutputBuffer(NULL), _linkeditCmdOffset(0),
			  _symboltableCmdOffset(0),
			  _toolsVersions(state.toolsVersions)
		{
			...
			_hasChainedFixupsLoadCommand = opts.makeChainedFixups() && !state.cantUseChainedFixups
			                               && opts.dyldOrKernelLoadsOutput();
			switch (opts.outputKind())
			{
				...
				case Options::kObjectFile:
					...
					for (std::vector<ld::Internal::FinalSection *>::iterator it = _state.sections.begin(); it != _state.sections.end();
					        ++it)
					{
		...
		template <typename A>
		uint32_t
		HeaderAndLoadCommandsAtom<A>::alignedSize(uint32_t size)
		{
			if (sizeof(pint_t) == 4)
			{
		...
		template <typename A>
		unsigned int
		HeaderAndLoadCommandsAtom<A>::nonHiddenSectionCount() const
		{
		...
		template <typename A>
		unsigned int
		HeaderAndLoadCommandsAtom<A>::segmentCount() const
		{
		...
		template <typename A>
		bool
		HeaderAndLoadCommandsAtom<A>::bitcodeBundleCommand(uint64_t &cmdOffset, uint64_t &cmdEnd,
		        uint64_t &sectOffset, uint64_t &sectEnd) const
		{
			if (_options.outputKind() == Options::kObjectFile)
			{
		...
		template <typename A>
		void
		HeaderAndLoadCommandsAtom<A>::linkeditCmdInfo(uint64_t &offset, uint64_t &size) const
		{
		...
		template <typename A>
		void
		HeaderAndLoadCommandsAtom<A>::symbolTableCmdInfo(uint64_t &offset, uint64_t &size) const
		{
		...
		template <typename A>
		uint64_t
		HeaderAndLoadCommandsAtom<A>::size() const
		{
			...
			{
				for (ld::relocatable::File::LinkerOptionsList::const_iterator it = _linkerOptions.begin(); it != _linkerOptions.end();
				        ++it)
				{
		...
		template <typename A>
		uint32_t
		HeaderAndLoadCommandsAtom<A>::commandsCount() const
		{
		...
		template <typename A>
		uint32_t
		HeaderAndLoadCommandsAtom<A>::fileType() const
		{
		...
		template <typename A>
		uint32_t
		HeaderAndLoadCommandsAtom<A>::flags() const
		{
			uint32_t bits = 0;
			if (_options.outputKind() == Options::kObjectFile)
			{
		...
		template <> uint32_t
		HeaderAndLoadCommandsAtom<x86>::magic() const
		{
			return MH_MAGIC;
		}
		template <> uint32_t
		HeaderAndLoadCommandsAtom<x86_64>::magic() const
		{
			return MH_MAGIC_64;
		}
		template <> uint32_t
		HeaderAndLoadCommandsAtom<arm>::magic() const
		{
			return MH_MAGIC;
		}
		template <> uint32_t
		HeaderAndLoadCommandsAtom<arm64>::magic() const
		{
			return MH_MAGIC_64;
		}
#if SUPPORT_ARCH_arm64_32
		template <> uint32_t
		HeaderAndLoadCommandsAtom<arm64_32>::magic() const
		{
			return MH_MAGIC;
		}
#endif
		...
		template <> uint32_t
		HeaderAndLoadCommandsAtom<x86>::cpuType() const
		{
			return CPU_TYPE_I386;
		}
		template <> uint32_t
		HeaderAndLoadCommandsAtom<x86_64>::cpuType() const
		{
			return CPU_TYPE_X86_64;
		}
		template <> uint32_t
		HeaderAndLoadCommandsAtom<arm>::cpuType() const
		{
			return CPU_TYPE_ARM;
		}
		template <> uint32_t
		HeaderAndLoadCommandsAtom<arm64>::cpuType() const
		{
			return CPU_TYPE_ARM64;
		}
#if SUPPORT_ARCH_arm64_32
		template <> uint32_t
		HeaderAndLoadCommandsAtom<arm64_32>::cpuType() const
		{
		...
		template <>
		uint32_t
		HeaderAndLoadCommandsAtom<x86>::cpuSubType() const
		{
		...
		template <>
		uint32_t
		HeaderAndLoadCommandsAtom<x86_64>::cpuSubType() const
		{
			if ((_options.outputKind() == Options::kDynamicExecutable) && (_state.cpuSubType == CPU_SUBTYPE_X86_64_ALL)
			        && _options.platforms().minOS(ld::mac10_5))
			{
		...
		template <>
		uint32_t
		HeaderAndLoadCommandsAtom<arm>::cpuSubType() const
		{
			return _state.cpuSubType;
		}
		...
		template <>
		uint32_t
		HeaderAndLoadCommandsAtom<arm64>::cpuSubType() const
		{
			return _state.cpuSubType;
		}
		...
		template <>
		uint32_t
		HeaderAndLoadCommandsAtom<arm64_32>::cpuSubType() const
		{
		...
		template <typename A>
		uint8_t *
		HeaderAndLoadCommandsAtom<A>::copySingleSegmentLoadCommand(uint8_t *p) const
		{
			...
			for (std::vector<ld::Internal::FinalSection *>::iterator sit = _state.sections.begin(); sit != _state.sections.end();
			        ++sit)
			{
				...
				msect->set_reloff((fsect->relocCount == 0) ? 0 : _writer.sectionRelocationsSection->fileOffset + fsect->relocStart *
				                  sizeof(macho_relocation_info<P>));
		...
		SegInfo::SegInfo(const char *n, const Options &opts)
			: segName(n), nonHiddenSectionCount(0), nonSectCreateSections(0), maxProt(opts.maxSegProtection(n)),
			  initProt(opts.initialSegProtection(n)), flags(0)
		{
		...
		template <typename A>
		uint32_t
		HeaderAndLoadCommandsAtom<A>::sectionFlags(ld::Internal::FinalSection *sect) const
		{
			uint32_t bits;
			switch (sect->type())
			{
				case ld::Section::typeUnclassified:
					...
					else if ((_options.outputKind() == Options::kObjectFile) && !sect->atoms.empty()
					         && sect->atoms.front()->dontDeadStripIfReferencesLive())
					{
		...
		template <typename A>
		bool
		HeaderAndLoadCommandsAtom<A>::sectionTakesNoDiskSpace(ld::Internal::FinalSection *sect) const
		{
			switch (sect->type())
			{
				case ld::Section::typeZeroFill:
		...
		template <typename A>
		uint8_t *
		HeaderAndLoadCommandsAtom<A>::copySegmentLoadCommands(uint8_t *p, uint8_t *base) const
		{
		...
		template <typename A>
		uint8_t *
		HeaderAndLoadCommandsAtom<A>::copySymbolTableLoadCommand(uint8_t *p, uint8_t *base) const
		{
		...
		template <typename A>
		uint8_t *
		HeaderAndLoadCommandsAtom<A>::copyDynamicSymbolTableLoadCommand(uint8_t *p) const
		{
			// build LC_SYMTAB command
			...
			//}
			bool hasIndirectSymbols = ((_writer.indirectSymbolTableSection != NULL)
			                           && (_writer.indirectSymbolTableSection->size != 0));
			dynamicSymbolTableCmd->set_indirectsymoff(hasIndirectSymbols ? _writer.indirectSymbolTableSection->fileOffset : 0);
			dynamicSymbolTableCmd->set_nindirectsyms(hasIndirectSymbols ? _writer.indirectSymbolTableSection->size / sizeof(
			            uint32_t) : 0);
			// FIX ME: support for classic relocations
			if (_options.outputKind() != Options::kObjectFile)
			{
				bool hasExternalRelocs = ((_writer.externalRelocationsSection != NULL)
				                          && (_writer.externalRelocationsSection->size != 0));
		...
		template <typename A>
		uint8_t *
		HeaderAndLoadCommandsAtom<A>::copyDyldInfoLoadCommand(uint8_t *p) const
		{
		...
		template <typename A>
		uint8_t *
		HeaderAndLoadCommandsAtom<A>::copyExportsTrieLoadCommand(uint8_t *p) const
		{
		...
		template <typename A>
		uint8_t *
		HeaderAndLoadCommandsAtom<A>::copyChainedFixupsLoadCommand(uint8_t *p) const
		{
		...
		template <typename A>
		uint8_t *
		HeaderAndLoadCommandsAtom<A>::copyDyldLoadCommand(uint8_t *p) const
		{
		...
		template <typename A>
		uint8_t *
		HeaderAndLoadCommandsAtom<A>::copyDylibIDLoadCommand(uint8_t *p) const
		{
		...
		template <typename A>
		uint8_t *
		HeaderAndLoadCommandsAtom<A>::copyRoutinesLoadCommand(uint8_t *p) const
		{
		...
		template <typename A>
		void
		HeaderAndLoadCommandsAtom<A>::recopyUUIDCommand()
		{
		...
		template <typename A>
		uint8_t *
		HeaderAndLoadCommandsAtom<A>::copyUUIDLoadCommand(uint8_t *p) const
		{
		...
		template <typename A>
		uint8_t *
		HeaderAndLoadCommandsAtom<A>::copyVersionLoadCommand(uint8_t *p, ld::Platform platform, uint32_t minVersion,
		        uint32_t sdkVersion) const
		{
		...
		template <typename A>
		uint8_t *
		HeaderAndLoadCommandsAtom<A>::copyBuildVersionLoadCommand(uint8_t *p, ld::Platform platform, uint32_t minVersion,
		        uint32_t sdkVersion) const
		{
			...
			cmd->set_cmdsize(alignedSize(sizeof(macho_build_version_command<P>) + sizeof(macho_build_tool_version<P>)
			                             *_toolsVersions.size()));
		...
		template <typename A>
		uint8_t *
		HeaderAndLoadCommandsAtom<A>::copySourceVersionLoadCommand(uint8_t *p) const
		{
		...
		template <>
		uint32_t
		HeaderAndLoadCommandsAtom<x86>::threadLoadCommandSize() const
		{
		...
		template <>
		uint8_t *
		HeaderAndLoadCommandsAtom<x86>::copyThreadsLoadCommand(uint8_t *p) const
		{
		...
		template <>
		uint32_t
		HeaderAndLoadCommandsAtom<x86_64>::threadLoadCommandSize() const
		{
		...
		template <>
		uint8_t *
		HeaderAndLoadCommandsAtom<x86_64>::copyThreadsLoadCommand(uint8_t *p) const
		{
			assert(_state.entryPoint != NULL);
			pint_t start = _state.entryPoint->finalAddress();
			macho_thread_command<P> *cmd = (macho_thread_command<P> *)p;
		...
		template <>
		uint32_t
		HeaderAndLoadCommandsAtom<arm>::threadLoadCommandSize() const
		{
		...
		template <>
		uint8_t *
		HeaderAndLoadCommandsAtom<arm>::copyThreadsLoadCommand(uint8_t *p) const
		{
			assert(_state.entryPoint != NULL);
			pint_t start = _state.entryPoint->finalAddress();
			if (_state.entryPoint->isThumb())
		...
		template <>
		uint32_t
		HeaderAndLoadCommandsAtom<arm64>::threadLoadCommandSize() const
		{
		...
		template <>
		uint8_t *
		HeaderAndLoadCommandsAtom<arm64>::copyThreadsLoadCommand(uint8_t *p) const
		{
			assert(_state.entryPoint != NULL);
			pint_t start = _state.entryPoint->finalAddress();
			macho_thread_command<P> *cmd = (macho_thread_command<P> *)p;
		...
		template <>
		uint32_t
		HeaderAndLoadCommandsAtom<arm64_32>::threadLoadCommandSize() const
		{
			return this->alignedSize(16 + 34 * 8); // base size + ARM_EXCEPTION_STATE64_COUNT * 4
		}
		...
		template <>
		uint8_t *
		HeaderAndLoadCommandsAtom<arm64_32>::copyThreadsLoadCommand(uint8_t *p) const
		{
			assert(_state.entryPoint != NULL);
			pint_t start = _state.entryPoint->finalAddress();
			macho_thread_command<P> *cmd = (macho_thread_command<P> *)p;
		...
		template <typename A>
		uint8_t *
		HeaderAndLoadCommandsAtom<A>::copyEntryPointLoadCommand(uint8_t *p) const
		{
		...
		template <typename A>
		uint8_t *
		HeaderAndLoadCommandsAtom<A>::copyEncryptionLoadCommand(uint8_t *p) const
		{
		...
		template <typename A>
		uint8_t *
		HeaderAndLoadCommandsAtom<A>::copySplitSegInfoLoadCommand(uint8_t *p) const
		{
		...
		template <typename A>
		uint8_t *
		HeaderAndLoadCommandsAtom<A>::copyDylibLoadCommand(uint8_t *p, const ld::dylib::File *dylib) const
		{
		...
		template <typename A>
		uint8_t *
		HeaderAndLoadCommandsAtom<A>::copyRPathLoadCommand(uint8_t *p, const char *path) const
		{
		...
		template <typename A>
		uint8_t *
		HeaderAndLoadCommandsAtom<A>::copySubFrameworkLoadCommand(uint8_t *p) const
		{
		...
		template <typename A>
		uint8_t *
		HeaderAndLoadCommandsAtom<A>::copyAllowableClientLoadCommand(uint8_t *p, const char *client) const
		{
		...
		template <typename A>
		uint8_t *
		HeaderAndLoadCommandsAtom<A>::copyDyldEnvLoadCommand(uint8_t *p, const char *env) const
		{
		...
		template <typename A>
		uint8_t *
		HeaderAndLoadCommandsAtom<A>::copySubUmbrellaLoadCommand(uint8_t *p, const char *nm) const
		{
		...
		template <typename A>
		uint8_t *
		HeaderAndLoadCommandsAtom<A>::copySubLibraryLoadCommand(uint8_t *p, const char *nm) const
		{
		...
		template <typename A>
		uint8_t *
		HeaderAndLoadCommandsAtom<A>::copyFunctionStartsLoadCommand(uint8_t *p) const
		{
			macho_linkedit_data_command<P> *cmd = (macho_linkedit_data_command<P> *)p;
		...
		template <typename A>
		uint8_t *
		HeaderAndLoadCommandsAtom<A>::copyDataInCodeLoadCommand(uint8_t *p) const
		{
			macho_linkedit_data_command<P> *cmd = (macho_linkedit_data_command<P> *)p;
		...
		template <typename A>
		uint8_t *
		HeaderAndLoadCommandsAtom<A>::copyLinkerOptionsLoadCommand(uint8_t *p, const std::vector<const char *> &options) const
		{
		...
		template <typename A>
		uint8_t *
		HeaderAndLoadCommandsAtom<A>::copyOptimizationHintsLoadCommand(uint8_t *p) const
		{
			macho_linkedit_data_command<P> *cmd = (macho_linkedit_data_command<P> *)p;
		...
		template <typename A>
		uint8_t *
		HeaderAndLoadCommandsAtom<A>::copyCodeSignatureLoadCommand(uint8_t *p) const
		{
			macho_linkedit_data_command<P> *cmd = (macho_linkedit_data_command<P> *)p;
		...
		template <typename A>
		void
		HeaderAndLoadCommandsAtom<A>::copyRawContent(uint8_t buffer[]) const
		{
