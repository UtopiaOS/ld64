	...
	namespace tool
	{
		...
		class ClassicLinkEditAtom : public ld::Atom
		{
			public:
				...
				virtual bool
				hasStabs(uint32_t &ssos, uint32_t &ssoe, uint32_t &sos, uint32_t &soe)
				{
					return false;
				}
		...
		void
		StringPoolAtom::copyRawContent(uint8_t buffer[]) const
		{
			uint64_t offset = 0;
		...
		int32_t
		StringPoolAtom::add(const char *str)
		{
		...
		int32_t
		StringPoolAtom::addUnique(const char *str)
		{
		...
		const char *
		StringPoolAtom::stringForIndex(int32_t index) const
		{
		...
		static bool
		chainLeadsTo(const ld::Atom *startAtom, const ld::Atom *targetAtom)
		{
		...
		template <typename A>
		bool
		SymbolTableAtom<A>::isAltEntry(const ld::Atom *atom)
		{
		...
		template <typename A>
		bool
		SymbolTableAtom<A>::addLocal(const ld::Atom *atom, StringPoolAtom *pool)
		{
			macho_nlist<P> entry;
		...
		template <typename A>
		void
		SymbolTableAtom<A>::addGlobal(const ld::Atom *atom, StringPoolAtom *pool)
		{
			macho_nlist<P> entry;
			// set n_strx
			...
			if ((atom->definition() == ld::Atom::definitionRegular) && (atom->combine() == ld::Atom::combineByName))
			{
				...
				if ((atom->scope() == ld::Atom::scopeGlobal) && atom->autoHide()
				        && (this->_options.outputKind() == Options::kObjectFile))
				{
					desc |= N_WEAK_REF;
				}
		...
		template <typename A>
		uint8_t
		SymbolTableAtom<A>::classicOrdinalForProxy(const ld::Atom *atom)
		{
		...
		template <typename A>
		void
		SymbolTableAtom<A>::addImport(const ld::Atom *atom, StringPoolAtom *pool)
		{
			macho_nlist<P> entry;
			// set n_strx
		...
		template <typename A>
		uint8_t
		SymbolTableAtom<A>::sectionIndexForStab(const ld::relocatable::File::Stab &stab)
		{
		...
		template <typename A>
		uint64_t
		SymbolTableAtom<A>::valueForStab(const ld::relocatable::File::Stab &stab)
		{
			switch (stab.type)
			{
				case N_FUN:
		...
		template <typename A>
		uint32_t
		SymbolTableAtom<A>::stringOffsetForStab(const ld::relocatable::File::Stab &stab, StringPoolAtom *pool)
		{
			switch (stab.type)
			{
				case N_SO:
		...
		template <typename A>
		bool
		SymbolTableAtom<A>::hasStabs(uint32_t &ssos, uint32_t &ssoe, uint32_t &sos, uint32_t &soe)
		{
		...
		template <typename A>
		void
		SymbolTableAtom<A>::copyRawContent(uint8_t buffer[]) const
		{
		...
		uint32_t
		RelocationsAtomAbstract::symbolIndex(const ld::Atom *atom) const
		{
			...
			for (std::map<const ld::Atom *, uint32_t>::iterator it = this->_writer._atomToSymbolIndex.begin();
			        it != this->_writer._atomToSymbolIndex.end(); ++it)
			{
		...
		class LocalRelocationsAtom : public RelocationsAtomAbstract
		{
			public:
				...
				virtual void
				addExternalPointerReloc(uint64_t addr, const ld::Atom *) {}
				virtual void
				addExternalCallSiteReloc(uint64_t addr, const ld::Atom *) {}
				...
				virtual	void
				addSectionReloc(ld::Internal::FinalSection	*sect, ld::Fixup::Kind,
				                const ld::Atom *inAtom, uint32_t offsetInAtom,
				                bool toTargetUsesExternalReloc, bool fromTargetExternalReloc,
#if SUPPORT_ARCH_arm64e
				                ld::Fixup *fixupWithAuthData,
		...
		template <>
		uint64_t
		LocalRelocationsAtom<x86_64>::relocBaseAddress(ld::Internal &state)
		{
		...
		template <typename A>
		uint64_t
		LocalRelocationsAtom<A>::relocBaseAddress(ld::Internal &state)
		{
		...
		template <typename A>
		void
		LocalRelocationsAtom<A>::addPointerReloc(uint64_t addr, uint32_t symNum, uint32_t length)
		{
		...
		template <typename A>
		void
		LocalRelocationsAtom<A>::addTextReloc(uint64_t addr, ld::Fixup::Kind kind, uint64_t targetAddr, uint32_t symNum)
		{
		}
		...
		template <typename A>
		uint64_t
		LocalRelocationsAtom<A>::size() const
		{
		...
		template <typename A>
		void
		LocalRelocationsAtom<A>::copyRawContent(uint8_t buffer[]) const
		{
		...
		class ExternalRelocationsAtom : public RelocationsAtomAbstract
		{
			public:
				...
				// overrides of RelocationsAtomAbstract
				virtual void
				addPointerReloc(uint64_t addr, uint32_t symNum, uint32_t length) {}
				virtual void
				addTextReloc(uint64_t addr, ld::Fixup::Kind k, uint64_t targetAddr, uint32_t symNum) {}
			...
			private:
				...
				struct LocAndAtom
				{
					...
					bool
					operator<(const LocAndAtom &rhs) const
					{
		...
		template <>
		uint64_t
		ExternalRelocationsAtom<x86_64>::relocBaseAddress(ld::Internal &state)
		{
		...
		template <typename A>
		uint64_t
		ExternalRelocationsAtom<A>::relocBaseAddress(ld::Internal &state)
		{
			return 0;
		}
		...
		template <typename A>
		void
		ExternalRelocationsAtom<A>::addExternalPointerReloc(uint64_t addr, const ld::Atom *target)
		{
		...
		template <typename A>
		void
		ExternalRelocationsAtom<A>::addExternalCallSiteReloc(uint64_t addr, const ld::Atom *target)
		{
		...
		template <typename A>
		uint64_t
		ExternalRelocationsAtom<A>::size() const
		{
		...
		template <> uint32_t
		ExternalRelocationsAtom<arm64>::pointerReloc()
		{
		...
		template <> uint32_t
		ExternalRelocationsAtom<arm64_32>::pointerReloc()
		{
			return ARM64_RELOC_UNSIGNED;
		}
#endif
		...
		template <> uint32_t
		ExternalRelocationsAtom<arm>::pointerReloc()
		{
			return ARM_RELOC_VANILLA;
		}
#endif
		template <> uint32_t
		ExternalRelocationsAtom<x86>::pointerReloc()
		{
			return GENERIC_RELOC_VANILLA;
		}
		template <> uint32_t
		ExternalRelocationsAtom<x86_64>::pointerReloc()
		{
		...
		template <> uint32_t
		ExternalRelocationsAtom<x86_64>::callReloc()
		{
			return X86_64_RELOC_BRANCH;
		}
		template <> uint32_t
		ExternalRelocationsAtom<x86>::callReloc()
		{
			return GENERIC_RELOC_VANILLA;
		}
#if SUPPORT_ARCH_arm64
		template <> uint32_t
		ExternalRelocationsAtom<arm64>::callReloc()
		{
...
#if SUPPORT_ARCH_arm64_32
		template <> uint32_t
		ExternalRelocationsAtom<arm64_32>::callReloc()
		{
			return ARM64_RELOC_BRANCH26;
		}
#endif
		...
		template <typename A>
		uint32_t
		ExternalRelocationsAtom<A>::callReloc()
		{
		...
		template <typename A>
		void
		ExternalRelocationsAtom<A>::copyRawContent(uint8_t buffer[]) const
		{
			...
			for (typename std::vector<LocAndAtom>::const_iterator it = _pointerLocations.begin(); it != _pointerLocations.end();
			        ++it, ++r)
			{
			...
			for (typename std::vector<LocAndAtom>::const_iterator it = _callSiteLocations.begin(); it != _callSiteLocations.end();
			        ++it, ++r)
			{
				r->set_r_address(it->loc);
				r->set_r_symbolnum(it->symbolIndex);
		...
		class SectionRelocationsAtom : public RelocationsAtomAbstract
		{
			public:
				...
				addExternalCallSiteReloc(uint64_t addr, const ld::Atom *) {}
				virtual uint64_t
				relocBaseAddress(ld::Internal &state)
				{
					return 0;
				}
				virtual	void								addSectionReloc(ld::Internal::FinalSection	*sect, ld::Fixup::Kind,
		...
		template <typename A>
		uint64_t
		SectionRelocationsAtom<A>::size() const
		{
			uint32_t count = 0;
			for (typename std::vector<SectionAndEntries>::const_iterator it = _entriesBySection.begin();
			        it != _entriesBySection.end(); ++it)
			{
		...
		template <typename A>
		void
		SectionRelocationsAtom<A>::copyRawContent(uint8_t buffer[]) const
		{
			uint32_t offset = 0;
			for (typename std::vector<SectionAndEntries>::const_iterator it = _entriesBySection.begin();
			        it != _entriesBySection.end(); ++it)
		...
		template <>
		void
		SectionRelocationsAtom<x86_64>::encodeSectionReloc(ld::Internal::FinalSection *sect,
		        const Entry &entry, std::vector<macho_relocation_info<P> > &relocs)
		{
		...
		template <typename A>
		uint32_t
		SectionRelocationsAtom<A>::sectSymNum(bool external, const ld::Atom *target)
		{
		...
		template <>
		void
		SectionRelocationsAtom<x86>::encodeSectionReloc(ld::Internal::FinalSection *sect,
		        const Entry &entry, std::vector<macho_relocation_info<P> > &relocs)
		{
			macho_relocation_info<P> reloc1;
			macho_relocation_info<P> reloc2;
		...
		template <>
		void
		SectionRelocationsAtom<arm>::encodeSectionReloc(ld::Internal::FinalSection *sect,
		        const Entry &entry, std::vector<macho_relocation_info<P> > &relocs)
		{
			macho_relocation_info<P> reloc1;
			macho_relocation_info<P> reloc2;
		...
		template <>
		void
		SectionRelocationsAtom<arm64>::encodeSectionReloc(ld::Internal::FinalSection *sect,
		        const Entry &entry, std::vector<macho_relocation_info<P> > &relocs)
		{
			macho_relocation_info<P> reloc1;
			macho_relocation_info<P> reloc2;
		...
		template <>
		void
		SectionRelocationsAtom<arm64_32>::encodeSectionReloc(ld::Internal::FinalSection *sect,
		        const Entry &entry, std::vector<macho_relocation_info<P> > &relocs)
		{
			macho_relocation_info<P> reloc1;
			macho_relocation_info<P> reloc2;
		...
		template <typename A>
		void
		SectionRelocationsAtom<A>::addSectionReloc(ld::Internal::FinalSection	*sect, ld::Fixup::Kind kind,
		        const ld::Atom *inAtom, uint32_t offsetInAtom,
		        bool toTargetUsesExternalReloc, bool fromTargetExternalReloc,
#if SUPPORT_ARCH_arm64e
		        ld::Fixup *fixupWithAuthData,
		        ...
		        const ld::Atom *fromTarget, uint64_t fromAddend)
		{
			...
			if (sect != lastSection)
			{
				for (typename std::vector<SectionAndEntries>::iterator it = _entriesBySection.begin(); it != _entriesBySection.end();
				        ++it)
				{
					if (sect == it->sect)
					{
		...
		template <typename A>
		void
		SectionRelocationsAtom<A>::encode()
		{
		...
		template <typename A>
		uint32_t
		IndirectSymbolTableAtom<A>::symbolIndex(const ld::Atom *atom)
		{
			std::map<const ld::Atom *, uint32_t>::iterator pos = this->_writer._atomToSymbolIndex.find(atom);
			if (pos != this->_writer._atomToSymbolIndex.end())
			{
		...
		template <typename A>
		uint32_t
		IndirectSymbolTableAtom<A>::symIndexOfStubAtom(const ld::Atom *stubAtom)
		{
		...
		template <typename A>
		uint32_t
		IndirectSymbolTableAtom<A>::symIndexOfLazyPointerAtom(const ld::Atom *lpAtom)
		{
			...
			else if (lpAtom->contentType() == ld::Atom::typeNonLazyPointer)
			{
				for (ld::Fixup::iterator fit = lpAtom->fixupsBegin(); fit != lpAtom->fixupsEnd(); ++fit)
				{
					if ((fit->kind == ld::Fixup::kindStoreTargetAddressLittleEndian32)
					        || (fit->kind == ld::Fixup::kindStoreTargetAddressLittleEndian64))
					{
						assert(fit->binding == ld::Fixup::bindingDirectlyBound);
						return symbolIndex(fit->u.target);
					}
		...
		template <typename A>
		uint32_t
		IndirectSymbolTableAtom<A>::symIndexOfNonLazyPointerAtom(const ld::Atom *nlpAtom)
		{
		...
		template <typename A>
		void
		IndirectSymbolTableAtom<A>::encodeStubSection(ld::Internal::FinalSection *sect)
		{
		...
		template <typename A>
		void
		IndirectSymbolTableAtom<A>::encodeLazyPointerSection(ld::Internal::FinalSection *sect)
		{
			sect->indirectSymTabStartIndex = _entries.size();
			for (std::vector<const ld::Atom *>::iterator ait = sect->atoms.begin(); ait != sect->atoms.end(); ++ait)
			{
		...
		template <typename A>
		void
		IndirectSymbolTableAtom<A>::encodeNonLazyPointerSection(ld::Internal::FinalSection *sect)
		{
			sect->indirectSymTabStartIndex = _entries.size();
			for (std::vector<const ld::Atom *>::iterator ait = sect->atoms.begin(); ait != sect->atoms.end(); ++ait)
			{
		...
		template <typename A>
		void
		IndirectSymbolTableAtom<A>::encodeTLVPointerSection(ld::Internal::FinalSection *sect)
		{
			sect->indirectSymTabStartIndex = _entries.size();
			for (std::vector<const ld::Atom *>::iterator ait = sect->atoms.begin(); ait != sect->atoms.end(); ++ait)
			{
		...
		template <typename A>
		void
		IndirectSymbolTableAtom<A>::encode()
		{
			...
			for (std::vector<ld::Internal::FinalSection *>::iterator sit = this->_state.sections.begin();
			        sit != this->_state.sections.end(); ++sit)
			{
				ld::Internal::FinalSection *sect = *sit;
				switch (sect->type())
				{
		...
		template <typename A>
		uint64_t
		IndirectSymbolTableAtom<A>::size() const
		{
		...
		template <typename A>
		void
		IndirectSymbolTableAtom<A>::copyRawContent(uint8_t buffer[]) const
		{
