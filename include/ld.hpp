...
namespace ld
{
	...
	struct PlatformVersion
	{
		...
		bool
		operator==(const PlatformVersion &other) const
		{
	...
	struct VersionSet
	{
		...
		public:
			...
			void
			updateMinVersion(const Platform &platform, uint32_t minVersion)
			{
				auto i = std::find_if(_versions.begin(), _versions.end(), [&platform](const PlatformVersion & version)
				{
					return platform == version.platform;
			...
			void
			updateSDKVersion(const Platform &platform, uint32_t sdkVersion)
			{
				auto i = std::find_if(_versions.begin(), _versions.end(), [&platform](const PlatformVersion & version)
				{
					return platform == version.platform;
			...
			void
			forEach(void (^callback)(ld::Platform platform, uint32_t minVersion, uint32_t sdkVersion, bool &stop)) const
			{
				bool stop = false;
			...
			bool
			contains(ld::Platform platform) const
			{
				auto i = std::find_if(_versions.begin(), _versions.end(), [&platform](const PlatformVersion & version)
				{
					return platform == version.platform;
			...
			bool
			contains(const ld::PlatformSet &platforms) const
			{
			...
			uint32_t
			minOS(ld::Platform platform) const
			{
				for (const auto &version : _versions)
				{
			...
			bool
			minOS(const PlatformVersion &version) const
			{
				return minOS(version.platform) >= version.minVersion;
			}

			bool
			minOS(const ld::VersionSet &requiredMinVersions) const
			{
				__block bool retval = true;
				forEach(^(ld::Platform platform, uint32_t minVersion, uint32_t sdkVersion, bool & stop)
				{
			...
			bool
			operator==(const VersionSet &other) const
			{
	...
	static const PlatformSet
	simulatorPlatforms({Platform::iOS_simulator, Platform::tvOS_simulator, Platform::watchOS_simulator});

	//FIXME do we need to add simulatots to these?
	//FIXME Are the dates correct?
	static const VersionSet
	version2007({mac10_4, iOS_2_0});
	static const VersionSet
	version2008({mac10_5, iOS_2_0});
	static const VersionSet
	version2008Fall({mac10_5, iOS_3_1});
	static const VersionSet
	version2009({mac10_6, iOS_3_1});
	static const VersionSet
	version2010({mac10_7, iOS_4_2});
	static const VersionSet
	version2010Fall({mac10_7, iOS_4_3});

	static const VersionSet
	version2012({mac10_8, iOS_6_0});
	static const VersionSet
	version2013({mac10_9, iOS_7_0});
	static const VersionSet
	version2019Fall({mac10_15, iOS_13_0, watchOS_6_0, tvOS_13_0, bridgeOS_4_0});
	static const VersionSet
	version2020Fall({mac10_16, iOS_14_0, watchOS_7_0, tvOS_14_0, bridgeOS_5_0});

	static const VersionSet
	supportsSplitSegV2({mac10_12, iOS_9_0, watchOS_2_0, tvOS_9_0});
	// FIXME: Use the comment out line instead.
	static const VersionSet
	supportsLCBuildVersion({mac10_14, iOS_12_0, watchOS_5_0, tvOS_12_0, bridgeOS_1_0});
	static const VersionSet
	supportsPIE({mac10_5, iOS_4_2});
	static const VersionSet
	supportsTLV({mac10_7, iOS_9_0});
	static const VersionSet
	supportsChainedFixups({mac10_16, iOS_14_0, watchOS_7_0, tvOS_14_0, bridgeOS_Future});
		...
		public:
			...
			class Ordinal
			{
				private:
					...
					{
						_ordinal = ((uint64_t)partition << 48) | ((uint64_t)majorIndex << 32) | ((uint64_t)minorIndex << 16) | ((
						               uint64_t)counter << 0);
					}
					...
					partition() const
					{
						return (_ordinal >> 48) & 0xffff;
					}
					const uint16_t
					majorIndex() const
					{
						return (_ordinal >> 32) & 0xffff;
					}
					const uint16_t
					minorIndex() const
					{
						return (_ordinal >> 16) & 0xffff;
					}
					const uint16_t
					counter() const
					{
						return (_ordinal >> 00) & 0xffff;
					}

					const Ordinal
					nextMajorIndex()		const
					{
						assert(majorIndex() < 0xffff);
						return Ordinal(_ordinal + ((uint64_t)1 << 32));
					}
					const Ordinal
					nextMinorIndex()		const
					{
						assert(minorIndex() < 0xffff);
						return Ordinal(_ordinal + ((uint64_t)1 << 16));
					}
					const Ordinal
					nextCounter()		const
					{
						assert(counter() < 0xffff);
						return Ordinal(_ordinal + ((uint64_t)1 << 0));
					}
				...
				public:
					...
					bool
					operator ==(const Ordinal &rhs) const
					{
					...
					static const Ordinal
					makeArgOrdinal(uint16_t argIndex)
					{
						return Ordinal(kArgListPartition, argIndex, 0, 0);
					};
					const Ordinal
					nextFileListOrdinal() const
					{
						return nextMinorIndex();
					}
					const Ordinal
					archiveOrdinalWithMemberIndex(uint16_t memberIndex) const
					{
						return Ordinal(partition(), majorIndex(), minorIndex(), memberIndex);
					}

					// For indirect libraries the partition is IndirectDylibPartition and the counter is used or order the libraries.
					static const ld::File::Ordinal
					indirectDylibBase()
					{
						return Ordinal(kIndirectDylibPartition, 0, 0, 0);
					}
					const Ordinal
					nextIndirectDylibOrdinal() const
					{
						return nextCounter();
					}
					...
					LTOOrdinal()
					{
						return Ordinal(kLTOPartition, 0, 0, 0);
					}
					...
					linkeOptionBase()
					{
						return Ordinal(kIndirectDylibPartition, 1, 0, 0);
					}
					const Ordinal
			...
			swiftVersion() const
			{
				return 0;    // ABI version, now fixed
			}
			virtual uint16_t
			swiftLanguageVersion() const
			{
				return 0;    // language version in 4.4 format
			}
			virtual uint32_t
			cpuSubType() const
			{
	...
	namespace relocatable
	{
		...
		class File : public ld::File
		{
			public:
				...
				virtual time_t
				debugInfoModificationTime() const
				{
				...
				virtual void
				forEachLtoSymbol(void (^handler)(const char *)) const { }
		};
	} // namespace relocatable
	...
	namespace dylib
	{
		...
		class File : public ld::File
		{
			public:
				...
				timestamp() const
				{
					return _dylibTimeStamp;
				}
				uint32_t
				currentVersion() const
				{
					return _dylibCurrentVersion;
				}
				uint32_t
				compatibilityVersion() const
				{
					return _dylibCompatibilityVersion;
				}
				void
				setExplicitlyLinked()
				{
					_explicitlyLinked = true;
				}
				bool
				explicitlyLinked() const
				{
					return _explicitlyLinked;
				}
				void
				setImplicitlyLinked()
				{
					_implicitlyLinked = true;
				}
				bool
				implicitlyLinked() const
				{
					return _implicitlyLinked;
				}
				void
				setSpeculativelyLoaded()
				{
					_speculativelyLoaded = true;
				}
				bool
				speculativelyLoaded() const
				{
					return _speculativelyLoaded;
				}
				...
				setForcedWeakLinked()
				{
					_forcedWeakLinked = true;
				}
				bool
				forcedWeakLinked() const
				{
					return _forcedWeakLinked;
				}
				void
				setNeededDylib()
				{
				...
				bool
				willBeUpwardDylib() const
				{
					return _upward;
				}
				void
				setWillBeRemoved(bool value)
				{
					_dead = value;
				}
				bool
				...
				virtual bool
				installPathVersionSpecific() const
				{
					return false;
				}
				...
				virtual bool
				hasReExportedDependentsThatProvidedExportAtom() const
				{
					return false;
				}
				virtual bool
				isUnzipperedTwin() const
				{
					return false;
				}
	...
	class Section
	{
		public:
			...
			bool
			operator==(const Section &rhs) const
			{
				return ((_hidden == rhs._hidden) &&
	...
	struct Fixup
	{
		...
		bool
		setsTarget(bool isObjectFile) const
		{
			switch (kind)
			{
				case ld::Fixup::kindSetTargetAddress:
		...
		bool
		isPcRelStore(bool isKextBundle) const
		{
			switch (kind)
			{
	...
	class Atom
	{
		public:
			...
			struct Alignment
			{
				...
				trailingZeros() const
				{
					return (modulus == 0) ? powerOf2 : __builtin_ctz(modulus);
				}
				uint16_t	powerOf2;
				uint16_t	modulus;
			};
			...
			SymbolTableInclusion
			symbolTableInclusion() const
			{
				return _symbolTableInclusion;
			}
			bool
			dontDeadStrip() const
			{
				return _dontDeadStrip;
			}
			bool
			dontDeadStripIfReferencesLive() const
			{
				return _dontDeadStripIfRefLive;
			}
			bool
			isThumb() const
			{
			...
			bool
			overridesDylibsWeakDef() const
			{
				return _overridesADylibsWeakDef;
			}
			bool
			coalescedAway() const
			{
				return _coalescedAway;
			}
			bool
			weakImported() const
			{
				return _weakImportState == weakImportTrue;
			}
			WeakImportState
			weakImportState() const
			{
				return _weakImportState;
			}
			bool
			autoHide() const
			{
			...
			void
			setSymbolTableInclusion(SymbolTableInclusion i)
			{
				_symbolTableInclusion = i;
			}
			void
			setCombine(Combine c)
			{
				_combine = c;
			}
			void
			...
			void
			setWeakImportState(bool w)
			{
				assert(_definition == definitionProxy);
				_weakImportState = (w ? weakImportTrue : weakImportFalse);
			}
			void
			setAutoHide()
			{
			...
			void
			setDontDeadStripIfReferencesLive()
			{
				_dontDeadStripIfRefLive = true;
			}
			void
			setLive()
			{
			...
			void
			setSectionStartAddress(uint64_t a)
			{
				assert(_mode == modeSectionOffset);
				_address += a;
			...
			bool
			finalAddressMode() const
			{
				return (_mode == modeFinalAddress);
			}
			...
			contentHash(const class IndirectBindingTable &) const
			{
				return 0;
			}
			virtual bool
			canCoalesceWith(const Atom &rhs, const class IndirectBindingTable &) const
			{
				return false;
			}
			virtual Fixup::iterator
			fixupsBegin() const
			{
				return NULL;
			}
			...
			bool
			hasFixupsOfKind(Fixup::Kind kind) const
			{
			...
			void
			setAttributesFromAtom(const Atom &a)
			{
				_section = a._section;
