...
namespace mach_o
{
	namespace trie
	{
		...
		struct Node
		{
				...
				void
				addSymbol(const char *fullStr, uint64_t address, uint64_t flags, uint64_t other, const char *importName)
				{
				...
				void
				addOrderedNodes(const char *name, std::vector<Node *> &orderedNodes)
				{
					if (!fOrdered)
					{
				...
				void
				appendToStream(std::vector<uint8_t> &out)
				{
					if (fHaveExportInfo)
					{
						if (fFlags & EXPORT_SYMBOL_FLAGS_REEXPORT)
			...
			private:
				static void
				append_uleb128(uint64_t value, std::vector<uint8_t> &out)
				{
					uint8_t byte;
					do
					{
				...
				static void
				append_string(const char *str, std::vector<uint8_t> &out)
				{
		...
		inline void
		makeTrie(const std::vector<Entry> &entries, std::vector<uint8_t> &output)
		{
			Node start(strdup(""));
		...
		struct EntryWithOffset
		{
			...
			bool
			operator<(const EntryWithOffset &other) const
			{
		...
		static inline void
		processExportNode(const uint8_t *const start, const uint8_t *p, const uint8_t *const end,
		...
		inline void
		parseTrie(const uint8_t *start, const uint8_t *end, std::vector<Entry> &output)
		{
