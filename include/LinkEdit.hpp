	...
	namespace tool
	{
		...
		class ByteStream
		{
			...
			public:
				...
				void
				append_delta_encoded_uleb128_run(uint64_t start, const std::vector<uint64_t> &locations)
				{
				...
				void
				append_byte(uint8_t byte)
				{
					_data.push_back(byte);
				}

				void
				append_mem(const void *mem, size_t len)
				{
				...
				void
				pad_to_size(unsigned int alignment)
				{
		...
		class LinkEditAtom : public ld::Atom
		{
			public:
				...
				rawContent() const
				{
					return this->_encodedData.start();
				}
		...
		void
		LinkEditAtom::copyRawContent(uint8_t buffer[]) const
		{
			assert(_encoded);
		...
		template <typename A>
		void
		RebaseInfoAtom<A>::encodeV1() const
		{
			std::vector<OutputFile::RebaseInfo> &info = this->_writer._rebaseInfo;
			...
			for (typename std::vector<rebase_tmp>::iterator it = mid.begin(); !done && it != mid.end() ; ++it)
			{
				switch (it->opcode)
				{
					case REBASE_OPCODE_DONE:
						if (log)
						{
							fprintf(stderr, "REBASE_OPCODE_DONE()\n");
						}
						done = true;
						break;
					case REBASE_OPCODE_SET_TYPE_IMM:
						if (log)
						{
							fprintf(stderr, "REBASE_OPCODE_SET_TYPE_IMM(%lld)\n", it->operand1);
						}
					...
					case REBASE_OPCODE_SET_SEGMENT_AND_OFFSET_ULEB:
						if (log)
						{
							fprintf(stderr, "REBASE_OPCODE_SET_SEGMENT_AND_OFFSET_ULEB(%lld, 0x%llX)\n", it->operand1, it->operand2);
						}
					...
					case REBASE_OPCODE_ADD_ADDR_ULEB:
						if (log)
						{
							fprintf(stderr, "REBASE_OPCODE_ADD_ADDR_ULEB(0x%llX)\n", it->operand1);
						}
					...
					case REBASE_OPCODE_ADD_ADDR_IMM_SCALED:
						if (log)
						{
							fprintf(stderr, "REBASE_OPCODE_ADD_ADDR_IMM_SCALED(%lld=0x%llX)\n", it->operand1, it->operand1 * sizeof(pint_t));
						}
					...
					case REBASE_OPCODE_DO_REBASE_IMM_TIMES:
						if (log)
						{
							fprintf(stderr, "REBASE_OPCODE_DO_REBASE_IMM_TIMES(%lld)\n", it->operand1);
						}
					...
					case REBASE_OPCODE_DO_REBASE_ULEB_TIMES:
						if (log)
						{
							fprintf(stderr, "REBASE_OPCODE_DO_REBASE_ULEB_TIMES(%lld)\n", it->operand1);
						}
					...
					case REBASE_OPCODE_DO_REBASE_ADD_ADDR_ULEB:
						if (log)
						{
							fprintf(stderr, "REBASE_OPCODE_DO_REBASE_ADD_ADDR_ULEB(0x%llX)\n", it->operand1);
						}
					...
					case REBASE_OPCODE_DO_REBASE_ULEB_TIMES_SKIPPING_ULEB:
						if (log)
						{
							fprintf(stderr, "REBASE_OPCODE_DO_REBASE_ULEB_TIMES_SKIPPING_ULEB(%lld, %lld)\n", it->operand1, it->operand2);
						}
			...
			if (log)
			{
				fprintf(stderr, "total rebase info size = %ld\n", this->_encodedData.size());
			}
		}
		...
		template <typename A>
		void
		BindingInfoAtom<A>::encode() const
		{
			// use encoding based on target minOS
			if (_options.useLinkedListBinding() && !this->_writer._hasUnalignedFixup)
			{
		...
		template <typename A>
		void
		BindingInfoAtom<A>::encodeV1() const
		{
			...
			for (typename std::vector<binding_tmp>::iterator it = mid.begin(); !done && it != mid.end() ; ++it)
			{
				switch (it->opcode)
				{
					...
					case BIND_OPCODE_SET_DYLIB_ORDINAL_IMM:
						if (log)
						{
							fprintf(stderr, "BIND_OPCODE_SET_DYLIB_ORDINAL_IMM(%lld)\n", it->operand1);
						}
					...
					case BIND_OPCODE_SET_DYLIB_ORDINAL_ULEB:
						if (log)
						{
							fprintf(stderr, "BIND_OPCODE_SET_DYLIB_ORDINAL_ULEB(%lld)\n", it->operand1);
						}
					...
					case BIND_OPCODE_SET_DYLIB_SPECIAL_IMM:
						if (log)
						{
							fprintf(stderr, "BIND_OPCODE_SET_DYLIB_SPECIAL_IMM(%lld)\n", it->operand1);
						}
					...
					case BIND_OPCODE_SET_SYMBOL_TRAILING_FLAGS_IMM:
						if (log)
						{
							fprintf(stderr, "BIND_OPCODE_SET_SYMBOL_TRAILING_FLAGS_IMM(0x%0llX, %s)\n", it->operand1, it->name);
						}
					...
					case BIND_OPCODE_SET_TYPE_IMM:
						if (log)
						{
							fprintf(stderr, "BIND_OPCODE_SET_TYPE_IMM(%lld)\n", it->operand1);
						}
					...
					case BIND_OPCODE_SET_ADDEND_SLEB:
						if (log)
						{
							fprintf(stderr, "BIND_OPCODE_SET_ADDEND_SLEB(%lld)\n", it->operand1);
						}
					...
					case BIND_OPCODE_SET_SEGMENT_AND_OFFSET_ULEB:
						if (log)
						{
							fprintf(stderr, "BIND_OPCODE_SET_SEGMENT_AND_OFFSET_ULEB(%lld, 0x%llX)\n", it->operand1, it->operand2);
						}
					...
					case BIND_OPCODE_ADD_ADDR_ULEB:
						if (log)
						{
							fprintf(stderr, "BIND_OPCODE_ADD_ADDR_ULEB(0x%llX)\n", it->operand1);
						}
					...
					case BIND_OPCODE_DO_BIND:
						if (log)
						{
							fprintf(stderr, "BIND_OPCODE_DO_BIND()\n");
						}
					...
					case BIND_OPCODE_DO_BIND_ADD_ADDR_ULEB:
						if (log)
						{
							fprintf(stderr, "BIND_OPCODE_DO_BIND_ADD_ADDR_ULEB(0x%llX)\n", it->operand1);
						}
					...
					case BIND_OPCODE_DO_BIND_ADD_ADDR_IMM_SCALED:
						if (log)
						{
							fprintf(stderr, "BIND_OPCODE_DO_BIND_ADD_ADDR_IMM_SCALED(%lld=0x%llX)\n", it->operand1, it->operand1 * sizeof(pint_t));
						}
					...
					case BIND_OPCODE_DO_BIND_ULEB_TIMES_SKIPPING_ULEB:
						if (log)
						{
							fprintf(stderr, "BIND_OPCODE_DO_BIND_ULEB_TIMES_SKIPPING_ULEB(%lld, %lld)\n", it->operand1, it->operand2);
						}
			...
			if (log)
			{
				fprintf(stderr, "total binding info size = %ld\n", this->_encodedData.size());
			}
		}

		template <typename A>
		void
		BindingInfoAtom<A>::encodeV2() const
		{
			...
			if (log)
			{
				fprintf(stderr, "BIND_SUBOPCODE_THREADED_SET_BIND_ORDINAL_TABLE_SIZE_ULEB(%lld)\n", numBinds + 1);
			}
			...
			for (typename std::vector<binding_tmp>::iterator it = mid.begin(); !done && it != mid.end() ; ++it)
			{
				switch (it->opcode)
				{
					...
					case BIND_OPCODE_THREADED | BIND_SUBOPCODE_THREADED_APPLY:
						...
						if (log)
						{
							fprintf(stderr, "BIND_SUBOPCODE_THREADED_APPLY()\n");
						}
						break;
				}
			}
		...
		class WeakBindingInfoAtom : public LinkEditAtom
		{
			...
			private:
				...
				{
					bool
					operator()(const OutputFile::BindingInfo &left, const OutputFile::BindingInfo &right)
					{
		...
		template <typename A>
		void
		WeakBindingInfoAtom<A>::encode() const
		{
			// sort by symbol, type, address
			...
			if (log)
			{
				fprintf(stderr, "total weak binding info size = %ld\n", this->_encodedData.size());
			}
		}
		...
		template <typename A>
		void
		LazyBindingInfoAtom<A>::encode() const
		{
		...
		template <typename A>
		void
		ChainedInfoAtom<A>::encode() const
		{
			...
			for (OutputFile::ChainedFixupSegInfo &segInfo : _writer._chainedFixupSegments)
			{
				if (!segInfo.pages.empty())
				{
					...
					this->_encodedData.pad_to_size(8);
					dyld_chained_starts_in_image *segHeader = (dyld_chained_starts_in_image *)(this->_encodedData.start() +
					        segsHeaderOffset);
		...
		template <typename A>
		const ld::Atom *
		ExportInfoAtom<A>::stubForResolverFunction(const ld::Atom *resolver) const
		{
		...
		ExportInfoAtom<A>::encode() const
		{
			...
			for (std::vector<const ld::Atom *>::const_iterator it = exports.begin(); it != exports.end(); ++it)
			{
				...
				uint64_t flags = (atom->contentType() == ld::Atom::typeTLV) ? EXPORT_SYMBOL_FLAGS_KIND_THREAD_LOCAL :
				...
				if (atom->definition() == ld::Atom::definitionProxy)
				{
					...
					{
						warning("not adding explict export for symbol %s because it is already re-exported from dylib %s", entry.name,
						        atom->safeFilePath());
						continue;
					}
					if (atom->isAlias())
		...
		template <>
		void
		SplitSegInfoV1Atom<x86_64>::addSplitSegInfo(uint64_t address, ld::Fixup::Kind kind, uint32_t extra) const
		{
			switch (kind)
			{
		...
		template <>
		void
		SplitSegInfoV1Atom<x86>::addSplitSegInfo(uint64_t address, ld::Fixup::Kind kind, uint32_t extra) const
		{
			switch (kind)
			{
				case ld::Fixup::kindStoreLittleEndian32:
		...
		template <>
		void
		SplitSegInfoV1Atom<arm>::addSplitSegInfo(uint64_t address, ld::Fixup::Kind kind, uint32_t extra) const
		{
			switch (kind)
			{
				case ld::Fixup::kindStoreLittleEndian32:
		...
		template <>
		void
		SplitSegInfoV1Atom<arm64>::addSplitSegInfo(uint64_t address, ld::Fixup::Kind kind, uint32_t extra) const
		{
			switch (kind)
			{
		...
		template <>
		void
		SplitSegInfoV1Atom<arm64_32>::addSplitSegInfo(uint64_t address, ld::Fixup::Kind kind, uint32_t extra) const
		{
			switch (kind)
			{
				case ld::Fixup::kindStoreARM64Page21:
		...
		template <typename A>
		void
		SplitSegInfoV1Atom<A>::uleb128EncodeAddresses(const std::vector<uint64_t> &locations) const
		{
		...
		template <typename A>
		void
		SplitSegInfoV1Atom<A>::encode() const
		{
		...
		template <typename A>
		void
		SplitSegInfoV2Atom<A>::encode() const
		{
		...
		template <typename A>
		void
		FunctionStartsAtom<A>::encode() const
		{
			this->_encodedData.reserve(8192);
			const uint64_t badAddress = 0xFFFFFFFFFFFFFFFF;
			uint64_t addr = badAddress;
			// delta compress all function addresses
			for (std::vector<ld::Internal::FinalSection *>::iterator it = this->_state.sections.begin();
			        it != this->_state.sections.end(); ++it)
			{
		...
		class DataInCodeAtom : public LinkEditAtom
		{
			...
			private:
				...
				{
					bool
					operator()(const ld::Fixup *left, const ld::Fixup *right)
					{
				...
				void
				encodeEntry(uint32_t startImageOffset, int len, ld::Fixup::Kind kind) const
				{
		...
		class OptimizationHintsAtom : public LinkEditAtom
		{
			public:
				...
				name() const
				{
					return "linker optimization hints";
				}
				// overrides of LinkEditAtom
				virtual void								encode() const;
		...
		template <typename A>
		void
		OptimizationHintsAtom<A>::encode() const
		{
		...
		void
		CodeSignatureAtom::encode() const
		{
			...
			if (libcd_set_hash_types_for_platform_version(_sigRef, (int)sig_platform,
			        (int)sig_min_version) != LIBCD_SET_HASH_TYPE_SUCCESS)
			{
		...
		void
		CodeSignatureAtom::hash(uint8_t *wholeFileBuffer) const
		{
			Internal::FinalSection *codeSignSect = _state.sections.back();
			assert(codeSignSect->atoms[0] == this);
